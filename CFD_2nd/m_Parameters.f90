module m_Parameters
  ...
  real(RK),public:: PrGradAver=0.0_RK
  real(RK),public:: PrGradNow=0.0_RK
...
contains
  ...
  subroutine PMcoeUpdate(ns)
    ...
    if((.not. RestartFlag) .and. ischeme==FI_AB2 .and. itime==ifirst) then
      pmGamma = dt
      pmTheta = 0.0_RK   
    endif
    ...
    pmBetaT = 0.5_RK*pmAlphaConst(ns) * dt  ! for LES purpose only
    if(ns==1) PrGradAver=0.0_RK
