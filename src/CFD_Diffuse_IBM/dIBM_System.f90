...
module dIBM_system
  ...
  use f2_BC_and_Halo
  use dIBM_IBM
  use dIBM_BC_and_Halo,only: SetBC_and_UpdateHalo_VelIBM
  implicit none
  private
...
!#define Double_FIMP_IBM
      ...
      IF((itime==ifirst .or. mod(itime, Cmd_LFile_Freq)==0) ) THEN
        ...
        uxm = CalcUxAver(ux)
        if(nrank==0) then
          call MainLog%OutInfo("CFD_Diffuse_IBM performed "//strip(num2str(itime))//" iterations up to here!",1)
