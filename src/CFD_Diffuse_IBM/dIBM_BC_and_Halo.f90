...
module dIBM_BC_and_Halo
  use MPI
  use mc_TypeDef
  use mc_Decomp2d,only:nrank,y1start,y1end,HaloInfo,myProcNghBC,update_halo,sum_halo
  use f2_Parameters
  ...
  public:: SetBC_and_UpdateHalo_VelIBM, SetBC_and_UpdateHalo_VolForce, Gather_Halo_IBMForce
contains
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  ! SetBC_and_UpdateHalo_VelIBM
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90   
  subroutine SetBC_and_UpdateHalo_VelIBM(ux,uy,uz); implicit none
  ...
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  ! SetBC_and_UpdateHalo_VolForce
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90   
  subroutine SetBC_and_UpdateHalo_VolForce(VolForce_x,VolForce_y,VolForce_z); implicit none
    ...
    if(BcOption(yp_dir) /= BC_PERIOD) then
      do kc=y1start(3),y1end(3)
        do ic=y1start(1),y1end(1)
          VolForce_x(ic,nyp,  kc)= VolForce_x(ic, nyc, kc)
          VolForce_y(ic,nyp+1,kc)= VolForce_y(ic, nyp, kc)
          VolForce_z(ic,nyp,  kc)= VolForce_z(ic, nyc, kc)
        enddo
      enddo     
    endif
    ...
    if(BcOption(ym_dir) /= BC_PERIOD) then
      do kc=y1start(3),y1end(3)
        do ic=y1start(1),y1end(1)
          VolForce_x(ic, 0, kc) = VolForce_x(ic, 1, kc)
          VolForce_y(ic, 0, kc) = VolForce_y(ic, 1, kc)
          VolForce_z(ic, 0, kc) = VolForce_z(ic, 1, kc)
        enddo
      enddo    
    endif
    ...
    if(myProcNghBC(y_pencil, 3) < 0) then
      VolForce_x(nxp+1,0:nyp,y1start(3):y1end(3))= VolForce_x(nxp, 0:nyp,y1start(3):y1end(3))
      VolForce_y(nxp,  0:nyp,y1start(3):y1end(3))= VolForce_y(nxc, 0:nyp,y1start(3):y1end(3))
      VolForce_z(nxp,  0:nyp,y1start(3):y1end(3))= VolForce_z(nxc, 0:nyp,y1start(3):y1end(3))       
    endif
    
    ! xm-dir
    if(myProcNghBC(y_pencil, 4) < 0) then
      VolForce_x(0,0:nyp,y1start(3):y1end(3)) = VolForce_x(1,0:nyp,y1start(3):y1end(3))
      VolForce_y(0,0:nyp,y1start(3):y1end(3)) = VolForce_y(1,0:nyp,y1start(3):y1end(3))
      VolForce_z(0,0:nyp,y1start(3):y1end(3)) = VolForce_z(1,0:nyp,y1start(3):y1end(3))       
    endif 
       
    ! zp-dir
    if(myProcNghBC(y_pencil, 1) < 0) then
      VolForce_x(y1start(1):y1end(1), 0:nyp, nzp  ) = VolForce_x(y1start(1):y1end(1), 0:nyp, nzc)
      VolForce_y(y1start(1):y1end(1), 0:nyp, nzp  ) = VolForce_y(y1start(1):y1end(1), 0:nyp, nzc)
      VolForce_z(y1start(1):y1end(1), 0:nyp, nzp+1) = VolForce_z(y1start(1):y1end(1), 0:nyp, nzp)
    endif         
    
    ! zm-dir
    if(myProcNghBC(y_pencil, 2) < 0) then
      VolForce_x(y1start(1):y1end(1), 0:nyp, 0) = VolForce_x(y1start(1):y1end(1), 0:nyp, 1) 
      VolForce_y(y1start(1):y1end(1), 0:nyp, 0) = VolForce_y(y1start(1):y1end(1), 0:nyp, 1)
      VolForce_z(y1start(1):y1end(1), 0:nyp, 0) = VolForce_z(y1start(1):y1end(1), 0:nyp, 1)   
    endif     
    ...
    hi_interp%xmh=0;  hi_interp%xph=1
    hi_interp%zmh=1;  hi_interp%zph=1
    call update_halo(VolForce_x, mb1, hi_interp)

    hi_interp%xmh=1;  hi_interp%xph=1
    hi_interp%zmh=0;  hi_interp%zph=1
    call update_halo(VolForce_z, mb1, hi_interp)
    ...
    hi_interp%xmh=1;  hi_interp%xph=1
    hi_interp%zmh=1;  hi_interp%zph=1    
    call update_halo(VolForce_y, mb1, hi_interp)
#else
    if(BcOption(ym_dir)==BC_PERIOD) then
      hi_interp%ymh=1;  hi_interp%yph=1    
    endif
    ...
    call update_halo(VolForce_y, mb1, hi_interp)
#endif
  end subroutine SetBC_and_UpdateHalo_VolForce
  
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  ! Gather_Halo_IBMForce
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine Gather_Halo_IBMForce(VolForce_x,VolForce_y,VolForce_z); implicit none
    real(RK),dimension(mb1%xmm:mb1%xpm,mb1%ymm:mb1%ypm,mb1%zmm:mb1%zpm),intent(inout)::VolForce_x,VolForce_y,VolForce_z
