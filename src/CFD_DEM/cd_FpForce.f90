...
module cd_FpForce
  ...
  use f2_Variables,only:mb1,hi1,mb_dist,hi_dist
  use f2_Variables,only:FpForce_x,FpForce_y,FpForce_z  
  use sp_Property
  use sp_Variables
  use sp_parameters
  use sp_Decomp_2d,only:DEM_decomp
  implicit none
  private
...
#include "clc_FluidAccForce_inc.f90"
  ...
  ! InitDistribute
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine InitDistribute(); implicit none

    ! locals
    type(real4)::pos
    type(real3)::RatioYp,RatioYc
    real(RK)::maxR,y1cord,y2cord,y3cord
    integer::i,j,k,ic,jc,kc,idxp_interp,idyp_interp,idzp_interp,idxc_interp,idyc_interp,idzc_interp,js,je,ierr,ierrTmp
    ...
    if(nrank==0 .and. (nxc>huge(int(0,2))-20 .or. nyc>huge(int(0,2))-20 .or. nzc>huge(int(0,2))-20)) then
    ...
    if(is_clc_Basset) then
      ...
      if(is_clc_Basset_fixed .and. (.not.DEM_Opt%RestartFlag) .and. mlocalFix>0) then
        allocate (GPFix_BassetData(GPrtcl_BassetSeq%nDataLen, mlocalFix), Stat= ierr)
        if(ierr /= 0 ) call MainLog%CheckForError(ErrT_Abort,"InitDistribute","allocate wrong1")
    ...
    ! fixed particle variables
    if(mlocalFix>0) then
      allocate(idxcFixed(mlocalFix),        Stat= ierrTmp); ierr =ierr +abs(ierrTmp)
      allocate(idycFixed(mlocalFix),        Stat= ierrTmp); ierr =ierr +abs(ierrTmp)
      allocate(idzcFixed(mlocalFix),        Stat= ierrTmp); ierr =ierr +abs(ierrTmp)
      allocate(indxyzFixed(6,mlocalFix),    Stat= ierrTmp); ierr =ierr +abs(ierrTmp)
      allocate(RYpFixed_interp(mlocalFix),  Stat= ierrTmp); ierr =ierr +abs(ierrTmp)
      allocate(RYcFixed_interp(mlocalFix),  Stat= ierrTmp); ierr =ierr +abs(ierrTmp)
      allocate(GPFix_FpForce(mlocalFix),    Stat= ierrTmp); ierr =ierr +abs(ierrTmp)
      allocate(GPFix_VolFluid(mlocalFix),   Stat= ierrTmp); ierr =ierr +abs(ierrTmp)
      allocate(GPFix_nFluidCell(mlocalFix), Stat= ierrTmp); ierr =ierr +abs(ierrTmp)
      if(ierr /= 0) call MainLog%CheckForError(ErrT_Abort,"InitDistribute","allocate wrong2")
    ...
    DO pid=1,mlocalFix
      ...
      idxcFixed(pid)= int(ic,2)
      idycFixed(pid)= int(jc,2)
      idzcFixed(pid)= int(kc,2)
      ...
      indxyzFixed(1,pid)= int(idxc_interp,2)
      indxyzFixed(2,pid)= int(idxp_interp,2)
      indxyzFixed(3,pid)= int(idyc_interp,2)
      indxyzFixed(4,pid)= int(idyp_interp,2)
      indxyzFixed(5,pid)= int(idzc_interp,2)
      indxyzFixed(6,pid)= int(idzp_interp,2)
      ...
      GPFix_nFluidCell(pid)= int(nFluidCell,2)
    ENDDO
  ...
  ! PrepareDistribute
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine PrepareDistribute(RatioYp_interp,RatioYc_interp); implicit none
  ...
  ! FinalDistribute
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine FinalDistribute(); implicit none
    integer::pid
  ...
  ! clc_FpForce
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine clc_FpForce(ux,uy,uz,pressure,RatioYp_interp,RatioYc_interp); implicit none
...
#define DragType1
    ...
    if(is_clc_Basset)   call clc_BassetForce()
  ...
  ! dist_FpForce 
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine distribute_FpForce(); implicit none
    ...
    DO pid=1,GPrtcl_list%nlocal
      ...
      FpForce= (massFH/dt)*(GPrtcl_linVel(1,pid)-GPrtcl_linVelOld(pid)) -GPrtcl_FpForce(pid)
  ...
  ! Gather_Halo_dist
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine Gather_Halo_dist(mat); implicit none
    ...
    integer:: i,j,k,icount,ierr,requests(2),ProcNgh(4)
    ...
    if(ProcNgh(3)==nrank) then  ! neighbour is nrank itself, and ProcNgh(4)==nrank also !!!
      ...
      call MPI_IRECV( xmhalo, icount,real_type,ProcNgh(4),1,MPI_COMM_WORLD,requests(1),ierr)
      call MPI_ISSEND(xmhalos,icount,real_type,ProcNgh(3),1,MPI_COMM_WORLD,requests(2),ierr)
      call MPI_WAITALL(2,requests,SRstatus,ierr)
    endif
    if(ProcNgh(4)>=0) then
    ...
    if(ProcNgh(4)==nrank) then  ! neighbour is nrank itself, and ProcNgh(3)==nrank also !!!
      ...
      call MPI_IRECV( xphalo, icount,real_type,ProcNgh(3),2,MPI_COMM_WORLD,requests(1),ierr)
      call MPI_ISSEND(xphalos,icount,real_type,ProcNgh(4),2,MPI_COMM_WORLD,requests(2),ierr)
      call MPI_WAITALL(2,requests,SRstatus,ierr)
    endif
    if(ProcNgh(3)>=0) then
      do k=zm_dist, ze_dist
    ...
    if(ProcNgh(1)==nrank) then  ! neighbour is nrank itself, and ProcNgh(2)==nrank also !!!
      ...
      call MPI_IRECV( zmhalo,          icount,real_type,ProcNgh(2),3,MPI_COMM_WORLD,requests(1),ierr)
      call MPI_ISSEND(mat(xs1,ys1,zs1),icount,real_type,ProcNgh(1),3,MPI_COMM_WORLD,requests(2),ierr)
      call MPI_WAITALL(2,requests,SRstatus,ierr)
    endif
    if(ProcNgh(2)>=0) then
    ...
    if(ProcNgh(2)==nrank) then  ! neighbour is nrank itself, and ProcNgh(1)==nrank also !!!
      ...
      icount = (xs2-xs1+1)*(ys2-ys1+1)*distwz
      call MPI_IRECV( zphalo,          icount,real_type,ProcNgh(1),4,MPI_COMM_WORLD,requests(1),ierr)
      call MPI_ISSEND(mat(xs1,ys1,zs1),icount,real_type,ProcNgh(2),4,MPI_COMM_WORLD,requests(2),ierr)
      call MPI_WAITALL(2,requests,SRstatus,ierr)
    endif
    if(ProcNgh(1)>=0) then
  ...
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine SetBC_and_UpdateHalo_dist(mat); implicit none
    real(RK),dimension(mb_dist%xmm:mb_dist%xpm,mb_dist%ymm:mb_dist%ypm,mb_dist%zmm:mb_dist%zpm),intent(inout)::mat
