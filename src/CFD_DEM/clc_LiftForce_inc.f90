...
subroutine clc_LiftForce(ux,uy,uz,RatioYp_interp,RatioYc_interp); implicit none
  ...
  ! locals
  type(real4)::pos
  type(real3)::FpVel,SaffmanLift,vorticity_interp,MagnusLift
  type(haloInfo):: hi_vorx_interp, hi_vory_interp, hi_vorz_interp
  integer::ic,jc,kc,im,jm,km,id,jd,kd,pid,i,j,k,nlocal
  ...
  real(RK)::SaffmanCoe,c_saff,MagnusCoe
...
#ifdef SaffManType1
  real(RK)::beta_saff,magVorticity
#else
  integer::itype
#endif
#ifdef CLCMagnusForce
  type(real3)::PRotVel
  real(RK)::c_magnus_veldiff,MagRotvel
#endif
  ...
  call update_halo(vorx, mb1, hi_vorx_interp)
  ...
  call update_halo(vory, mb1, hi_vory_interp)
  ...
  call update_halo(vorz, mb1, hi_vorz_interp)
  ...
  DO pid=1,nlocal
    ...
    IF(magVorticity >1.0E-10_RK) THEN
      beta_saff = diam*sqrt(magVorticity/(2.0_RK*xnu))
      if(rep<0.5_RK) then
        c_saff= 1.0_RK-0.1_RK *rep + 0.03314_RK*sqrt(rep)*beta_saff
      elseif(rep<=40_RK) then
        c_saff= exp(-0.1_RK*rep) +0.3314_RK*beta_saff/sqrt(rep)*(1.0_RK-exp(-0.1_RK*rep))
      else
        c_saff= 0.0524_RK*beta_saff
      endif
      c_saff=min(1.0_RK,c_saff) ! c_saff SMALLER THEN unity (according to [3])
    ...
    if(MagRotvel >1.0E-10_RK) then
      c_magnus_veldiff=0.45_RK*veldiff + (diam*MagRotvel -0.45_RK*veldiff) &
                       *exp(-0.075_RK*((0.5_RK*diam*diam*MagRotvel/xnu)**0.4_RK)*(rep**0.3_RK))
      PRotVel = (1.0_RK/MagRotvel)*(PRotVel)  ! normalized rotational vector
