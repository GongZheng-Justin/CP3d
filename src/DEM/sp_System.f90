...
contains
  ...
  !    predefined plane 
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine DEMS_Initialize(this,chDEMPrm); implicit none
    class(DEMSystem)::this
    character(len=*),intent(in)::chDEMPrm
    
    ! locals
    integer:: ierr
    character(len=:),allocatable:: chStr
    ...
    if(nrank==0) then
      call mkdir(DEM_Opt%ResultsDir, ierr); if(ierr<0) then; print*,'Cannot create folder ResultsDir'; stop; endif
      call mkdir(DEM_Opt%RestartDir, ierr); if(ierr<0) then; print*,'Cannot create folder RestartDir'; stop; endif
    endif
    ...
    if(nrank==0) then
      ...
      call DEMLogInfo%OutInfo("Physical properties contains "// strip( num2str(DEM_Opt%numPrtcl_Type ) ) // &
                              " particle types and "//strip( num2str(DEM_Opt%numWall_type ) )// " wall types.",2)
    endif
    ...
    if(nrank==0) then
      ...
      call DEMLogInfo%OutInfo("Geometry Contains "//strip(num2str(DEMGeometry%num_pWall))//" Plane walls.", 2)
    endif

    ! Step3: initilize all the particle variables and IO
    call GPrtcl_list%AllocateAllVar()
    call Prtcl_Init_visu(chDEMPrm,1)
    t_restart1=MPI_WTIME()
#ifdef CFDDEM
    if(.not.DEM_Opt%RestartFlag) then
      if(DEM_Opt%numPrtcl>0) call GPrtcl_list%ReadInitialCoord()
      if(nrank==0) then
        call DEMLogInfo%OutInfo("Step3: Initial Particle coordinates are READING into DEMSystem ...", 1 )
        call DEMLogInfo%OutInfo("Number of particles avaiable in the system:"//strip(num2str(DEM_Opt%numPrtcl)),2)
      endif
      DEM_Opt%np_InDomain = DEM_Opt%numPrtcl
      if(DEM_Opt%numPrtclFix>0) call Prtcl_ReadFixedCoord()
    else
      if(DEM_Opt%numPrtcl>0) then
        call Prtcl_Read_Restart()
      else
        DEM_Opt%np_InDomain=0
      endif
      if(nrank==0) then
        call DEMLogInfo%OutInfo("Step3: Particles are READING from the Restarting file ...", 1 )
        call DEMLogInfo%OutInfo("Number of particles available in domain:"//strip(num2str(DEM_Opt%np_InDomain)),2)
      endif
      if(DEM_Opt%numPrtclFix>0) call Prtcl_ReadFixedRestart()
    endif
#else
    if(.not.DEM_Opt%RestartFlag) then
      ...
      if(nrank==0) then
        ...
        call DEMLogInfo%OutInfo("Number of particles avaiable in domain:"//strip(num2str(DEM_Opt%np_InDomain)),2)
      endif
    endif
    if(DEM_Opt%numPrtclFix>0) call Prtcl_ReadFixedCoord()
#endif
    ...
    call Prtcl_Init_visu(chDEMPrm,2)
#ifdef CFDDEM
    ...
    if(DEM_Opt%RestartFlag) then
      if(DEM_Opt%np_InDomain>0) call Prtcl_Restart_ContactList()
    endif
    call MPI_BARRIER(MPI_COMM_WORLD,ierr)
    t_restart2=MPI_WTIME(); t_res_tot=t_restart2-t_restart1+t_res_tot
    if(nrank==0 .and. DEM_Opt%RestartFlag) call DEMLogInfo%OutInfo("Restart time [sec] :"//strip(num2str(t_res_tot)),2)
    
    if(nrank==0) then
      call DEMLogInfo%OutInfo("Step5: Initializing contact list and contact force models . . . ", 1 )
      if( DEM_Opt%CF_Type == DEM_LSD ) then
        chStr = "linear spring-dashpot with limited tangential displacement"
      elseif( DEM_Opt%CF_Type == DEM_nLin ) then
        chStr = "non-linear visco-elastic model with limited tangential displacement"
      endif
      call DEMLogInfo%OutInfo("Contact force model is "//chStr, 2)

      if(DEM_Opt%PI_Method==PIM_FE) then
        chStr = "Forward Euler"
      elseif(DEM_Opt%PI_Method==PIM_AB2) then
        chStr = "Adams Bashforth: 2nd Order"
      elseif(DEM_Opt%PI_Method==PIM_AB3) then
        chStr = "Adams Bashforth: 3nd Order"
      endif
      call DEMLogInfo%OutInfo("Linear   movement Integration scheme is : "//chStr,2)
      ...
      call DEMLogInfo%OutInfo("Rotating movement Integration scheme is : "//chStr,2)
    endif
...
#ifdef CFDDEM
    call Prtcl_Dump_Visu((DEM_Opt%ifirst-1)/icouple)            
#else
    call Prtcl_Dump_Visu(DEM_Opt%ifirst-1)
#endif
  ...
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine DEMS_iterate(this,itime); implicit none
    class(DEMSystem):: this
    integer,intent(in)::itime

    ! locals
    integer:: Consv_Cont(2),Consv_Cont1(2),ierr,npwcs(4)
  ...
  ENDIF
    ...
    call MPI_ALLREDUCE(GPrtcl_list%nlocal, DEM_Opt%np_InDomain, 1, int_type,MPI_SUM,MPI_COMM_WORLD,ierr)
#ifdef CFDDEM
    if( mod(this%IterNumber,DEM_Opt%SaveVisu)== 0)   call Prtcl_Dump_Visu(itime/icouple)
    if( mod(this%IterNumber,DumpPrtclFreq)== 0)      call WriteDumpCache(itime)
#else
    if( mod(this%IterNumber,DEM_Opt%SaveVisu)== 0)   call Prtcl_Dump_Visu(itime)
#endif
    if( mod(this%IterNumber,DEM_Opt%BackupFreq)== 0 .or. itime==DEM_Opt%ilast) then
      call Prtcl_Write_Restart(itime)
#ifdef CFDDEM
      call Prtcl_WriteFixedRestart(itime)
      call PrtclVarDump(itime)
#endif
      call Prtcl_Delete_Prev_Restart(itime)
    endif
  ...
  IF(UpdateDEMflag) THEN
    ...
    IF((this%IterNumber==DEM_Opt%ifirst .or. mod(this%IterNumber,DEM_Opt%Cmd_LFile_Freq)==0) ) THEN
      call get_num_Cnsv_cntct_PP(Consv_Cont1)
      call MPI_REDUCE(Consv_Cont1, Consv_Cont,       2,int_type,MPI_SUM,0,MPI_COMM_WORLD,ierr)
      call MPI_REDUCE(GPPW_CntctList%numCntcts,npwcs,4,int_type,MPI_SUM,0,MPI_COMM_WORLD,ierr)
      if(nrank/=0) return
    
      ! command window and log file output
      call DEMLogInfo%OutInfo("DEM performed "//strip(num2str(this%IterNumber))//" iterations up to here!",1)

      call DEMLogInfo%OutInfo("Execution time [tot, last, ave] [sec]: "//strip(num2str(this%m_total_timer%tot_time))//", "// &
      strip(num2str(this%m_total_timer%last_time ))//", "//strip(num2str(this%m_total_timer%average())),2)
      call DEMLogInfo%OutInfo("PreItertion time [tot, ave]        : "//strip(num2str(this%m_pre_iter_timer%tot_time))//", "// &
      strip(num2str(this%m_pre_iter_timer%average())),3)
      call DEMLogInfo%OutInfo("Comm_For_Contact [tot, ave]        : "//strip(num2str(this%m_comm_cs_timer%tot_time))//", "// &
      strip(num2str(this%m_comm_cs_timer%average())), 3)
      call DEMLogInfo%OutInfo("CS and CF P-P time [tot, ave]      : "//strip(num2str(this%m_CSCF_PP_timer%tot_time))//", "// &
      strip(num2str(this%m_CSCF_PP_timer%average())), 3)
      call DEMLogInfo%OutInfo("CS and CF P-W time [tot, ave]      : "//strip(num2str(this%m_CSCF_PW_timer%tot_time))//", "// &
      strip(num2str(this%m_CSCF_PW_timer%average())), 3)
      call DEMLogInfo%OutInfo("Integration time [tot, ave]        : "//strip(num2str(this%m_integration_timer%tot_time))//", "// &
      strip(num2str(this%m_integration_timer%average())), 3)
      call DEMLogInfo%OutInfo("Comm_For_Exchange [tot, ave]       : "//strip(num2str(this%m_comm_exchange_timer%tot_time))//", "// &
      strip(num2str(this%m_comm_exchange_timer%average())), 3)
      call DEMLogInfo%OutInfo("Write to file time [tot, ave]      : "//strip(num2str(this%m_write_prtcl_timer%tot_time))//", "// &
      strip(num2str(this%m_write_prtcl_timer%average())), 3)
     
      call DEMLogInfo%OutInfo("Particle number in  domain:  "//strip(num2str(DEM_Opt%np_InDomain)), 2)        
      call DEMLogInfo%OutInfo("Contact information", 2)
      call DEMLogInfo%OutInfo("No. consrvtv. contacts, same level | cross level: " // strip(num2str(Consv_Cont(1))) // " | " // strip(num2str(Consv_Cont(2))), 3)
      call DEMLogInfo%OutInfo("No. exact contacts P-P | P-GP | P-FP | P-W : " // strip(num2str(npwcs(1))) // " | " // &
                     strip(num2str(npwcs(2))) // " | " // strip(num2str(npwcs(3))) // " | " // strip(num2str(npwcs(4))), 3)
    ENDIF
#ifdef CFDDEM
  ENDIF
#endif
  ...
  ! DEMS_preIteration
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine DEMS_preIteration(this); implicit none
    class(DEMSystem)::this
  ...
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine Write_DEM_Opt_to_Log(); implicit none

    ! locals
    logical::RestartFlag
    real(RK)::dtDEM,Wall_neighbor_ratio,Prtcl_cs_ratio
    character(len=:),allocatable::RunName,ResultsDir,RestartDir,Geom_Dir
    ...
    write(DEMLogInfo%iUnit, nml=DEMOptions)
#ifdef CFDDEM
    write(DEMLogInfo%iUnit, nml=CFDDEMCoupling)
    write(DEMLogInfo%iUnit, nml=BassetOptions)
#endif
  end subroutine Write_DEM_Opt_to_Log

end module sp_System
