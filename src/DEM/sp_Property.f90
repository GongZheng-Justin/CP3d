...
#else
  use sp_decomp_2d,only: nrank
#endif
  use sp_Parameters
#ifdef CFDDEM
  use f2_Parameters,only: FluidDensity
#elif  CFDACM
  use f2_Parameters,only: FluidDensity,xnu
  use f2_MeshAndMetries,only: dx,dyUniform,dz
#endif
  implicit none
  private
...
contains
  ...
  subroutine InitPrtclProperty(this,chFile); implicit none
    ...
    if(ierr /= 0 ) call DEMLogInfo%CheckForError(ErrT_Abort,"InitPrtclProperty","Cannot open file:"//strip(chFile))
    read(iUnit, nml=ParticlePhysicalProperty)
    if(nrank==0) write(DEMLogInfo%iUnit, nml=ParticlePhysicalProperty)
#ifdef CFDACM
    ...
    rewind(iUnit)
    read(iUnit, nml=ParticleIBMProperty)
    if(nrank==0)write(DEMLogInfo%iUnit, nml=ParticleIBMProperty)
#endif
  ...
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine InitWallProperty(this,chFile); implicit none
    class(PhysicalProperty)::this
    character(len=*),intent(in)::chFile
        
    ! locals
    type(PureProperty)pari,wall
    integer::i,j,nPType,nWType,iUnit,ierr
    ...
    if(ierr /= 0 .and. nrank==0) then
      call DEMLogInfo%CheckForError(ErrT_Abort,"InitWallProperty: " ,"Cannot open file:"//strip(chFile))
    endif
    read(iUnit, nml=WallPhysicalProperty)
    if(nrank==0)write(DEMLogInfo%iUnit, nml=WallPhysicalProperty)
    close(iUnit)        
  ...
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  function clc_BnryPrtcl_Prop(pari,parj,FrictionCoe_s,FrictionCoe_k,RestitutionCoe_n,iswall) result(Bnry); implicit none
    ...
    !Vel_Crit= 6.0_RK*PI*St_Crit*FluidDensity*xnu*Bnry%RadEff*Bnry%RadEff/Bnry%MassEff ! Yang & Hunt, PoF, 2006.
