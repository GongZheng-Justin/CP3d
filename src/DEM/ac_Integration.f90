...
contains
  ...
  ! Prtcl_Integrate
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine Prtcl_Integrate(iCountACM); implicit none
    integer,intent(in)::iCountACM
    
    ! locals
    integer::pid,itype
    type(real3)::linVel1,rotVel1,FpLinAcc,FpRotAcc
    ...
    DO pid =1,GPrtcl_list%nlocal
      ...
      if(GPrtcl_HighSt(pid)=="Y") then   ! Turn off fluid forces for large St collisions
        ...
        FpLinAcc= (MassInFluid*MassEff)*DEM_Opt%Gravity
      else
        MassEff = 1.0_RK/PrtclIBMProp(itype)%MassEff
        FpLinAcc= (MassInFluid*MassEff)*DEM_Opt%Gravity +MassEff*GPrtcl_FpForce(pid)
      endif
