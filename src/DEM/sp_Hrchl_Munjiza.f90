...
contains
  ...
  ! NBS_Munjiza_Hrchl
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine Init_Munjiza_Hrchl(this); implicit none
    class(NBS_Munjiza_Hrchl):: this

    type(integer3)::numCell
    real(RK):: minD, maxD, minD_lvl, maxD_lvl, cell_len,Diam, xed_cs,yed_cs,zed_cs
    integer:: numLevels, idh, nx,ny,nz, dmax_min,numPrtcl_lvl,i, id_level,nLevel_final,ierr,ierrTmp
    ...
    if(nrank==0) then
      ...
      call DEMLogInfo%OutInfo(" Number of levels is :" // strip( num2str(nLevel_final)),3)
    endif

    ierr = 0
    allocate( lvls_Munjiza(nLevel_final),  Stat = ierrTmp); ierr =ierr +abs(ierrTmp)
    allocate( box_index(this%mbox), Stat = ierrTmp); ierr =ierr +abs(ierrTmp)
    allocate( NextX( this%mbox ),   Stat = ierrTmp); ierr =ierr +abs(ierrTmp)
    allocate( NextY( this%mbox ),   Stat = ierrTmp); ierr =ierr +abs(ierrTmp)
    allocate( NextZ( this%mbox ),   Stat = ierrTmp); ierr =ierr +abs(ierrTmp) 
    if(ierr /= 0) then
    ...
    DO idh = 1, numLevels
      ...
      if(numPrtcl_lvl>0) then
        ...
        allocate(lvls_Munjiza(id_level)%HeadY( 0:ny+1), Stat = ierrTmp); ierr =ierr +abs(ierrTmp)
        allocate(lvls_Munjiza(id_level)%HeadX0(0:nx+1), Stat = ierrTmp); ierr =ierr +abs(ierrTmp)
        allocate(lvls_Munjiza(id_level)%HeadX( 0:nx+1), Stat = ierrTmp); ierr =ierr +abs(ierrTmp)
        allocate(lvls_Munjiza(id_level)%HeadX2(0:nx+1), Stat = ierrTmp); ierr =ierr +abs(ierrTmp)
        allocate(lvls_Munjiza(id_level)%HeadZ0(0:2,0:nz+1), Stat = ierrTmp); ierr =ierr +abs(ierrTmp)
        allocate(lvls_Munjiza(id_level)%HeadZ( 0:2,0:nz+1), Stat = ierrTmp); ierr =ierr +abs(ierrTmp)
        allocate(lvls_Munjiza(id_level)%HeadZ2(0:2,0:nz+1), Stat = ierrTmp); ierr =ierr +abs(ierrTmp)
        if(ierr/= 0) then
        ...
        call DEMLogInfo%OutInfo("Level "//strip( num2str(id_level)), 4)
        call DEMLogInfo%OutInfo("   Cell size is [m]: "// strip(num2str(cell_len)), 4)
        call DEMLogInfo%OutInfo("   number of cells (x,y,z) :"//strip(num2str(numCell)),4,.true.)
        call DEMLogInfo%OutInfo("   number of particles : "//strip(num2str(numPrtcl_lvl)),4,.true.)
      endif
  ...
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine NBSMH_Grow_Box_And_Next(this,nbox); implicit none
    class(NBS_Munjiza_Hrchl):: this 
    ...
    call DEMLogInfo%OutInfo("Previous matirx length is :"//strip(num2str(this%mbox)),3)
  ...
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine NBSMH_ContactSearch(this); implicit none
    class(NBS_Munjiza_Hrchl) :: this
    integer :: idh
  ...
  ! Calculating integer coordinates of all boxes
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90    
  subroutine NBSMH_clcBoxIndex_and_BuildYList(this); implicit none
    class(NBS_Munjiza_Hrchl)::this
  ...
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine NBSMH_BuildXList(base_lvl,iy); implicit none
  ...
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine NBSMH_BuildXList2(base_lvl,iy); implicit none
    type(lvl_Munjiza):: base_lvl
    integer,intent(in)::iy ! row index
    integer:: n,ix
  ...
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine NBSMH_BuildZList0(base_lvl, ix,m,lcheck); implicit none
    type(lvl_Munjiza):: base_lvl
  ...
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine NBSMH_BuildZList(base_lvl,ix,m,lcheck); implicit none
    type(lvl_Munjiza):: base_lvl
  ...
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine NBSMH_BuildZList2(base_lvl,ix, m,lcheck); implicit none
    type(lvl_Munjiza):: base_lvl
    integer,intent(in) :: ix ! col index
    integer,intent(in) :: m  ! the column location with resect to ix 
    logical,optional,intent(in):: lcheck
  ...
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine OneLevelBroadSearch(this,base_lvl); implicit none
    class(NBS_Munjiza_Hrchl):: this
    type(lvl_Munjiza) base_lvl
    integer:: idh,lvlCoe
    ...
    DO iy=1,base_lvl%ny_lvl
      call NBSMH_BuildXList(base_lvl, iy)
      ...
      DO idh=base_lvl%lvl-1, 1, -1
        ...
        if(crs_indy == 1) then
          ...
          call NBSMH_BuildXList(lvls_Munjiza(idh), 1)
        endif
        call NBSMH_BuildXList2(lvls_Munjiza(idh),  crs_indy+1)
      ENDDO
                        
      ! if row is non-empty
      IF(base_lvl%HeadY(iy)  /=  -1 ) THEN
        base_lvl%HeadZ(0,:) = -1
        base_lvl%HeadZ0(0,:)= -1
               
        ! Creating the zlist of the lower row and current ix (column)
        call NBSMH_BuildZList0(base_lvl, 1, 1)
        do ix = 1,base_lvl%nx_lvl
          call NBSMH_BuildZList(base_lvl, ix, 1)
          call NBSMH_BuildZList0(base_lvl,ix, 2)
          ...
          do idh =base_lvl%lvl-1, 1, -1
            ...
            if(crs_indx == 1 ) then
              ...
              call NBSMH_BuildZList0(lvls_Munjiza(idh),crs_indx,1, .true. )    
              call NBSMH_BuildZList(lvls_Munjiza(idh), crs_indx,1, .true. )    
              call NBSMH_BuildZList2(lvls_Munjiza(idh),crs_indx,1, .true. )
            endif
                        
            call NBSMH_BuildZList0(lvls_Munjiza(idh),crs_indx,2, .true. )    
            call NBSMH_BuildZList(lvls_Munjiza(idh), crs_indx,2, .true. )    
            call NBSMH_BuildZList2(lvls_Munjiza(idh),crs_indx,2, .true. )  
          enddo                        
                              
          if(base_lvl%Headx(ix) /= -1) then
  ...
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine NBSMH_LoopNBSMask(this, base_lvl, iz); implicit none
    class(NBS_Munjiza_Hrchl):: this
    type(lvl_Munjiza):: base_lvl
  ...
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90    
  subroutine NBSMH_FineSearch(pid1,pid2); implicit none
  ...
  ! CrossMunjiza level
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine NBSMH_Loop_CrossMask(this, base_lvl, iz, crs_iz, CrossMunjiza); implicit none
    class(NBS_Munjiza_Hrchl):: this
    type(lvl_Munjiza)::base_lvl
