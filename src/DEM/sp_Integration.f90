...
module sp_Integration
  use mc_TypeDef
  use sp_Property
  use sp_Variables
  use sp_Parameters
#ifdef CFDDEM
  use f2_Parameters,only:PrGradData,IsUxConst,FluidDensity
#endif
  implicit none
  private 
...
contains
  ...
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
#ifdef CFDDEM
  subroutine clc_Acceleration(); implicit none

    ! locals
    integer:: i,itype
    type(real3):: Fpforce,PrGradVec
    real(RK)::Mass,Inertia,MassInFluid,MassTot,Vol
    
    PrGradVec = zero_r3
    if(IsAddFluidPressureGradient .and. IsUxConst) PrGradVec%x = FluidDensity * PrGradData(2)
    ...
    do i=1,GPrtcl_list%nlocal
      ...
      GPrtcl_linAcc(1,i) = (1.0_RK/MassTot)*(GPrtcl_cntctForce(i)+Fpforce +Vol*PrGradVec)+ (MassInFluid/MassTot)*DEM_Opt%Gravity
  ...
  ! calculating acceleration of particles (linear and angular) 
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine Prtcl_Integrate(iCountDEM); implicit none
