...
contains
  ...
  ! eq_sphere
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine eq_sphere(Point); implicit none
    ...
    real(RK)::PI,rx,ry,rz,rnorm,r_regions,a_top,a_bot,area_tot,Psi,Phi,aTemp
    real(RK)::area_cap,c_polar,discrepancy,a_fitting,area_top,area_bot,offset

    PI=3.141592653589793238462643383279502884_RK
  ...
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  function gcd(m1,n1) result(r); implicit none
