...
module sp_System
  ...
  contains
    ...
    procedure,nopass:: Write_Restart => DEMS_WriteRestart
    
    ! performing pre-iterations
    procedure,private:: preIteration => DEMS_preIteration
  ...
  subroutine DEMS_Initialize(this,chDEMPrm); implicit none
    ...
    if(.not.DEM_Opt%RestartFlag) then
      ...
      if(nrank==0) then
        ...
        call DEMLogInfo%OutInfo("Number of particles available in the system:"//strip(num2str(DEM_Opt%numPrtcl)),2)
      endif
    ...
    if(nrank==0) then
      ...
      if(DEM_Opt%CF_Type == ACM_LSD ) then
        chStr = "Adaptive linear spring-dashpot model"
      elseif(DEM_Opt%CF_Type == ACM_nLin ) then
        chStr = "Adaptive non-linear visco-elastic model"
      elseif(DEM_Opt%CF_Type == DEM_LSD ) then
        chStr = "Typical linear spring-dashpot model"
      elseif(DEM_Opt%CF_Type == DEM_nLin ) then
        chStr = "Typical non-linear visco-elastic model"
      endif
...
#ifdef CFDACM
    NAMELIST/CFDACMCoupling/UpdateACMflag,IBM_Scheme,icouple,Klub_pp,Klub_pw,Lub_ratio, &
...
#ifdef CFDACM
    write(DEMLogInfo%iUnit, nml=CFDACMCoupling)
#endif
