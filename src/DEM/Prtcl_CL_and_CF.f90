...
contains
  ...
  subroutine CL_Count_Cntctlink(this,pid,ncv)
    ...
    do while(n>0)
      ...
      if(ncv > DEM_Opt%ncvAllowed) call DEMLogInfo%CheckForError(ErrT_Abort,"CL_Count_Cntctlink","so big ncv")
      n = Next(n)
    enddo        
  ...
  subroutine CL_Gather_Cntctlink_Restart(this,pid,ncv,CntctVec)
    ...
    do while(n>0)
      ncv = ncv + 1
      ! for monosize particles, a particle can contact with NO MORE THAN 12 neighbor particles.
      if(ncv > DEM_Opt%ncvAllowed) call DEMLogInfo%CheckForError(ErrT_Abort,"CL_Gather_Cntctlink_Restart","so big ncv")
