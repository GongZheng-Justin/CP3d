...
contains
  ...
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  logical function PW_isInContact(this, box,ovrlp,nv ); implicit none
  ...
  ! determining if an sphere (box) has a contact with this wall
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  logical function PW_isInContact(this, box,ovrlp,nv,clcLubFlag); implicit none
    class(PlaneWall)::this
    type(real4),intent(in):: box
    real(RK),intent(out):: ovrlp
    type(real3),intent(out)::nv
  ...
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  function PW_IsInDomain(this,dpmin, dpmax) result(res); implicit none
    class(PlaneWall)::this
  ...
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  logical function PW_IsInPlane( p1,p2,p3,p4, cp); implicit none
  ...
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  logical function Line_point_check(lp1,lp2, dpos, nv,ovrlp); implicit none
  ...
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90    
  function PointIsInDomain(point, dpmin, dpmax) result(res); implicit none
  ...
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  function IsReflectionOvlp(fp1,fp2,fp3,fp4,sp1,sp2,sp3,sp4) result(res); implicit none
  ...
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90    
  function IsLineInsertInner(L1_p1, L1_p2, L2_p1, L2_p2) result(res); implicit none
  ...
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine G_InitAllocate(this); implicit none
    class(Geometry):: this
    integer:: iUnit,ierr
    ...
    if(nrank/=0) return
    chFile = strip(DEM_opt%ResultsDir) // "WallsFor" // strip(DEM_opt%RunName) // ".backup"
    open(newunit=iUnit, file=chfile,status='replace',form='formatted',IOSTAT=ierr)
    if(ierr/=0.and.nrank==0) call DEMLogInfo%CheckForError(ErrT_Abort,"G_InitAllocate","Cannot open file: "//chFile)
  ...
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine G_add_PlaneWall(this, p1, p2, p3, p4 , user_id ,prop_type,  both, infinite, t_vel_t); implicit none
    class(Geometry):: this
    ...
    IF(this%nPW_local == MaxWallSize) THEN
      ...
      allocate(this%pWall(MaxWallSize),Stat=ierr) 
      if(ierr/=0) call DEMLogInfo%CheckForError(ErrT_Abort,"G_add_PlaneWall","Reallocation failed, 2 ")
    ...
    IF(wall%IsInDomain(pmin, pmax)) THEN
      ...
      if(nrank/=0) return
      chFile = strip(DEM_opt%ResultsDir)//"WallsFor"//strip(DEM_opt%RunName)//".backup"
      open(newunit=iUnit, file=chFile, status='old',position='append',form='formatted',IOSTAT=ierr )
      if(ierr/=0 .and. nrank==0) call DEMLogInfo%CheckForError(ErrT_Abort,"G_add_PlaneWall","Cannot open file: "//chFile)
      write(iUnit,* ) p1
      write(iUnit,* ) p2
      write(iUnit,* ) p3
      write(iUnit,* ) p4
    ...
    ELSE
      ...
      call DEMLogInfo%OutInfo("   Point 1: "//strip(num2str(p1%x))//'  '//strip(num2str(p1%y))//'  '//strip(num2str(p1%z)), 3, .true.)
      call DEMLogInfo%OutInfo("   Point 2: "//strip(num2str(p2%x))//'  '//strip(num2str(p2%y))//'  '//strip(num2str(p2%z)), 3, .true.)
      call DEMLogInfo%OutInfo("   Point 3: "//strip(num2str(p3%x))//'  '//strip(num2str(p3%y))//'  '//strip(num2str(p3%z)), 3, .true.)
      call DEMLogInfo%OutInfo("   Point 4: "//strip(num2str(p4%x))//'  '//strip(num2str(p4%y))//'  '//strip(num2str(p4%z)), 3, .true.)
    ENDIF
  ...
  ! MakeGeometry
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90     
  subroutine G_MakeGeometry(this,chFile); implicit none
    class(Geometry)::this
    character(len=*),intent(in)::chFile
        
    !locals
    integer:: i,nplane,iUnit,ierr
    ...
    elseif(DEM_Opt%GeometrySource ==1) then  ! add the geometry from the NAMELIST "&GeometryMakingParam"
      ...
      if(ierr /= 0 .and. nrank==0) call DEMLogInfo%CheckForError(ErrT_Abort,"G_MakeGeometry","Cannot open file:"//strip(chFile))
      read(iUnit, nml=GeometryMakingNumPlane)
      if(nplane<1) then
        close(iUnit, IOSTAT=ierr);return
      endif
      ...
      rewind(iUnit)
      read(iUnit, nml=GeometryMakingParam)
      close(iUnit, IOSTAT=ierr)
      do i=1,nplane
