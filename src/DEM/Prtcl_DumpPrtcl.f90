...
#define Prtcl_Dump_Flag 93
  ...
  subroutine WriteDumpCache(itime)
    ...
    do pid=1,nlocal
...
#ifdef OnlyDumpFpForce
      DumpRealMat(k,nDumpPrtclSize)=real(GPrtcl_FpForce(pid)%x, RKP);    k=k+1 ! 04 
      DumpRealMat(k,nDumpPrtclSize)=real(GPrtcl_FpForce(pid)%y, RKP);    k=k+1 ! 05 
      DumpRealMat(k,nDumpPrtclSize)=real(GPrtcl_FpForce(pid)%z, RKP);    k=k+1 ! 06       
      DumpRealMat(k,nDumpPrtclSize)=real(GPrtcl_FpTorque(pid)%x, RKP);   k=k+1 ! 07
      DumpRealMat(k,nDumpPrtclSize)=real(GPrtcl_FpTorque(pid)%y, RKP);   k=k+1 ! 08
      DumpRealMat(k,nDumpPrtclSize)=real(GPrtcl_FpTorque(pid)%z, RKP);   k=k+1 ! 09
#else
      ...
      DumpRealMat(k,nDumpPrtclSize)=real(GPrtcl_FpTorque(pid)%x, RKP);   k=k+1 ! 13
      DumpRealMat(k,nDumpPrtclSize)=real(GPrtcl_FpTorque(pid)%y, RKP);   k=k+1 ! 14
      DumpRealMat(k,nDumpPrtclSize)=real(GPrtcl_FpTorque(pid)%z, RKP);   k=k+1 ! 15
      DumpRealMat(k,nDumpPrtclSize)=real(GPrtcl_CntctForce(pid)%x, RKP); k=k+1 ! 16
      DumpRealMat(k,nDumpPrtclSize)=real(GPrtcl_CntctForce(pid)%y, RKP); k=k+1 ! 17
      DumpRealMat(k,nDumpPrtclSize)=real(GPrtcl_CntctForce(pid)%z, RKP); k=k+1 ! 18
