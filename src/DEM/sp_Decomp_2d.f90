...
module sp_Decomp_2d
  ...
  use f2_MeshAndMetries,only:dx,dz
  use f2_Parameters,only:p_row,p_col
#endif
  use sp_Parameters
  implicit none
  private
...
contains
  ...
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
#if !defined(CFDDEM) && !defined(CFDACM)
  subroutine PDI_Init_DECOMP(this,chFile,RowCol); implicit none
    class(Prtcl_DECOMP_INFO)::this
    character(len=*),intent(in)::chFile
    integer,intent(in),optional::RowCol(2)
#else
  subroutine PDI_Init_DECOMP(this); implicit none
    class(Prtcl_DECOMP_INFO)::this
#endif
    ...
    if(ierr /= 0 .and. nrank==0) then
      print*, "  PDI_Init_DECOMP: Cannot open file"//strip(chFile)
      STOP
    endif
    read(iUnit, nml=PrtclDomainDecomp)
  ...
  subroutine Init_Prctl_MPI_TYPE(); implicit none
    ...
    call MPI_TYPE_SIZE(real_type,  real_byte,  ierr)
