...
module sp_Variables
  ...
  contains
    ...
    procedure:: ReadInitialCoord => GL_ReadInitialCoord
#if !defined(CFDDEM) && !defined(CFDACM)
...
contains
  ...
  ! GL_AllocateAllVar
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine GL_AllocateAllVar(this); implicit none
    class(VarList)::this

    ! locals
    type(real3)::SimLen
    real(RK)::xst,xed,yst,yed,zst,zed,vol_tot,vol_local
    integer:: mlocal,numPrtcl,mlocalFix,numPrtclFix,ierrTmp,ierr
    ...
    ierr=0
    allocate(GPrtcl_id(mlocal),                Stat=ierrTmp); ierr=ierr+abs(ierrTmp)
    allocate(GPrtcl_pType(mlocal),             Stat=ierrTmp); ierr=ierr+abs(ierrTmp)
    allocate(GPrtcl_usrMark(mlocal),           Stat=ierrTmp); ierr=ierr+abs(ierrTmp)
    allocate(GPrtcl_PosR(mlocal),              Stat=ierrTmp); ierr=ierr+abs(ierrTmp)
    allocate(GPrtcl_linVel(this%tsize,mlocal), Stat=ierrTmp); ierr=ierr+abs(ierrTmp)
    allocate(GPrtcl_linAcc(this%tsize,mlocal), Stat=ierrTmp); ierr=ierr+abs(ierrTmp)
    allocate(GPrtcl_theta(mlocal),             Stat=ierrTmp); ierr=ierr+abs(ierrTmp)
    allocate(GPrtcl_rotVel(this%rsize,mlocal), Stat=ierrTmp); ierr=ierr+abs(ierrTmp)
    allocate(GPrtcl_rotAcc(this%rsize,mlocal), Stat=ierrTmp); ierr=ierr+abs(ierrTmp)
    allocate(GPrtcl_cntctForce(mlocal),        Stat=ierrTmp); ierr=ierr+abs(ierrTmp)
    allocate(GPrtcl_torque(mlocal),            Stat=ierrTmp); ierr=ierr+abs(ierrTmp)
    if(ierr/=0) call DEMLogInfo%CheckForError(ErrT_Abort,"GL_AllocateAllVar: ","Allocation failed 1")
    ...
    allocate(GPFix_id(mlocalFix),   Stat=ierrTmp); ierr=ierr+abs(ierrTmp)
    allocate(GPFix_pType(mlocalFix),Stat=ierrTmp); ierr=ierr+abs(ierrTmp)
    allocate(GPFix_PosR(mlocalFix), Stat=ierrTmp); ierr=ierr+abs(ierrTmp)
    if(ierr/=0) call DEMLogInfo%CheckForError(ErrT_Abort,"GL_AllocateAllVar: ","Allocation failed 2")
#ifdef CFDDEM
    allocate(GPrtcl_FpForce(mlocal),    Stat=ierrTmp); ierr=ierr+abs(ierrTmp)
    allocate(GPrtcl_FpForce_old(mlocal),Stat=ierrTmp); ierr=ierr+abs(ierrTmp)
    allocate(GPrtcl_linVelOld(mlocal),  Stat=ierrTmp); ierr=ierr+abs(ierrTmp)
    allocate(GPrtcl_Vfluid(2,mlocal),   Stat=ierrTmp); ierr=ierr+abs(ierrTmp)
    if(ierr/=0) call DEMLogInfo%CheckForError(ErrT_Abort,"GL_AllocateAllVar: ","Allocation Fpforce failed")
    GPrtcl_FpForce     = zero_r3
    GPrtcl_FpForce_old = zero_r3
    GPrtcl_linVelOld   = zero_r3
    GPrtcl_Vfluid      = zero_r3
    if(Is_clc_Basset) then
      allocate(GPrtcl_BassetData(GPrtcl_BassetSeq%nDataLen, mlocal), Stat=ierrTmp);ierr=ierr+abs(ierrTmp)
      if(ierr/=0) call DEMLogInfo%CheckForError(ErrT_Abort,"GL_AllocateAllVar: ","Allocation GPrtcl_BassetData failed")
      GPrtcl_BassetData= zero_r3
    endif
#endif
#ifdef CFDACM
    allocate(GPrtcl_FpForce(mlocal),         Stat=ierrTmp); ierr=ierr+abs(ierrTmp)
    allocate(GPrtcl_FpTorque(mlocal),        Stat=ierrTmp); ierr=ierr+abs(ierrTmp)
    allocate(GPrtcl_FluidIntegrate(2,mlocal),Stat=ierrTmp); ierr=ierr+abs(ierrTmp)
    allocate(GPrtcl_FluidIntOld(2,mlocal),   Stat=ierrTmp); ierr=ierr+abs(ierrTmp)
    allocate(GPrtcl_HighSt(mlocal),          Stat=ierrTmp); ierr=ierr+abs(ierrTmp)
    if(IBM_Scheme==2) allocate(GPrtcl_PosOld(mlocal),Stat=ierrTmp); ierr=ierr+abs(ierrTmp)
    if(ierr/=0) call DEMLogInfo%CheckForError(ErrT_Abort,"GL_AllocateAllVar","Allocation Fpforce failed")
    GPrtcl_FpForce  = zero_r3
...
#define DEM_NumRead 100
  subroutine GL_ReadInitialCoord(this); implicit none
    class(VarList)::this
    ...
    integer::i,pid,ierr,nLeft,nRead,numPrtcl,nlocal,nlocal_sum,pType,iUnit,pbyte,Init_SpheresCoord_Type
    real(RK),dimension(5,DEM_NumRead)::InitPrtclIn
    logical::lexist
    ...
    chFile = strip(DEM_Opt%RestartDir)//"SpheresCoord.bin"
    inquire(file=chFile, exist=lexist)   ! Try to find the binary file firstly.
    if(.not. lexist) then
      chFile = strip(DEM_Opt%RestartDir)//"SpheresCoord.txt"
      inquire(file=chFile, exist=lexist) ! Try to find the plain text file secondly.
      if(.not. lexist) then
        if(nrank==0) call DEMLogInfo%CheckForError(ErrT_Abort,"GL_ReadInitialCoord","Cannot open particle Coordinate file")
      else
    ...
    IF(Init_SpheresCoord_Type == 0) THEN
      ...
      if(ierr /=0 .and. nrank==0) call DEMLogInfo%CheckForError(ErrT_Abort,"GL_ReadInitialCoord","Cannot open file: "//chFile)
      inquire(unit=iUnit,Pos=disp); disp=disp-1_8; 
      rewind(unit=iUnit,IOSTAT=ierr)
      byte_total=int(pbyte,8)*int(numPrtcl,8)
      if(disp/=byte_total .and. nrank==0) call DEMLogInfo%CheckForError(ErrT_Abort,"GL_ReadInitialCoord","file byte wrong")
    ELSE
      chFile = strip(DEM_Opt%RestartDir)//"SpheresCoord.txt"
    ...
    DO
      ...
      IF(Init_SpheresCoord_Type == 0) THEN
        read(iUnit,pos=disp,IOSTAT=ierr)InitPrtclIn(:,1:nRead)
      ELSE
        do i=1,nRead
          read(unit=iUnit,fmt=*,IOSTAT=ierr) InitPrtclIn(:,i)
          if(ierr /= 0 .and. nrank==0) call DEMLogInfo%CheckForError(ErrT_Abort,"GL_ReadInitialCoord", "Read plain text wrong") 
        enddo      
      ENDIF
      ...
      do i=1,nRead
        ...
        pType=nint(InitPrtclIn(5,i))
        radius= DEMProperty%Prtcl_PureProp(pType)%Radius
        if(abs(2.0_RK*radius/diam -1.0_RK)>1.0E-6 .and. nrank==0 )then
          call DEMLogInfo%CheckForError(ErrT_Abort,"GL_ReadInitialCoord","Diameter not coordinate")
        endif
    ...
    if(nlocal_sum/= numPrtcl .and. nrank==0) then
      call DEMLogInfo%CheckForError(ErrT_Abort,"GL_ReadInitialCoord","nlocal_sum/= numPrtcl " )
    endif
  end subroutine GL_ReadInitialCoord
#undef DEM_NumRead
  ...
  ! GL_MakingAllPrtcl
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine GL_MakingAllPrtcl(this, chFile); implicit none
    class(VarList)::this
    character(len=*),intent(in)::chFile
        
    ! locals
    logical::IsReadInitialCoord
    real(RK)::Distance_Ratio,VelMag
    character(len=1),dimension(3)::Fill_order
    real(RK),dimension(3)::MkPrtclMinpoint,MkPrtclMaxpoint
    type(real3):: MkPrtclMinpoint_real3,MkPrtclMaxpoint_real3
    NAMELIST/ParticleMakingOption/ IsReadInitialCoord, MkPrtclMinpoint,MkPrtclMaxpoint, Fill_order, Distance_Ratio, VelMag
    ...
    integer::i,j,k,iTV(8),bin_id,numPrtcl,iUnit,ierr,nlocal,nlocal_sum
   
    IsReadInitialCoord = .false.
    open(newunit=iUnit, file=chFile,status='old',form='formatted',IOSTAT=ierr)
    if(ierr/=0) call DEMLogInfo%CheckForError(ErrT_Abort,"MakingAllPrtcl", "Cannot open file: "//strip(chFile))
    read(iUnit, nml=ParticleMakingOption)
    if(nrank==0)write(DEMLogInfo%iUnit, nml=ParticleMakingOption)
    close(iUnit,IOSTAT=ierr)
    ...
    IF(IsReadInitialCoord) THEN
      call this%ReadInitialCoord()
    ELSE
      nlocal = 0
      bin_id = 1
  ...
  ! Fill order Vector 
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine Fill_Vectors(fill_order , l1 , l2, l3); implicit none
    character(len=1),dimension(3) :: fill_order
  ...
  ! copy i2 to i1
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine copy_GPrtcl(i1,i2); implicit none
...
#endif
  end subroutine copy_GPrtcl

  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  ! Reallocate particle varaibles 
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine GL_ReallocatePrtclVar(this,np_new); implicit none
    class(VarList)::this
    ...
    allocate(GPrtcl_id(sizen),stat=ierrTmp);ierr=ierr+abs(ierrTmp)
    GPrtcl_id(1:sizep)=IntVec
    GPrtcl_id(sizep+1:sizen)=0

    call move_alloc(GPrtcl_pType, IntVec)
    allocate(GPrtcl_pType(sizen),stat=ierrTmp);ierr=ierr+abs(ierrTmp)
    GPrtcl_pType(1:sizep)=IntVec
    GPrtcl_pType(sizep+1:sizen)=1

    call move_alloc(GPrtcl_usrMark, IntVec)
    allocate(GPrtcl_usrMark(sizen),stat=ierrTmp);ierr=ierr+abs(ierrTmp)
    ...
    allocate(GPrtcl_FpTorque(sizen),stat=ierrTmp);ierr=ierr+abs(ierrTmp)
    ...
    allocate(GPrtcl_cntctForce(sizen),stat=ierrTmp);ierr=ierr+abs(ierrTmp)

    deallocate(GPrtcl_torque)
    allocate(GPrtcl_torque(sizen),stat=ierrTmp);ierr=ierr+abs(ierrTmp)
    
    ! ======= real3 matrix part =======
    call move_alloc(GPrtcl_linVel,Real3Arr)
    allocate(GPrtcl_linVel(this%tsize,sizen),stat=ierrTmp);ierr=ierr+abs(ierrTmp)
    ...
    allocate(GPrtcl_linAcc(this%tsize,sizen),stat=ierrTmp);ierr=ierr+abs(ierrTmp)
    ...
    allocate(GPrtcl_rotVel(this%rsize,sizen),stat=ierrTmp);ierr=ierr+abs(ierrTmp)
    ...
    allocate(GPrtcl_rotAcc(this%rsize,sizen),stat=ierrTmp);ierr=ierr+abs(ierrTmp)
    ...
    allocate(GPrtcl_Vfluid(2,sizen),stat=ierrTmp);ierr=ierr+abs(ierrTmp)
    GPrtcl_Vfluid(1:2,1:sizep)=Real3Arr
    GPrtcl_Vfluid(1:2,sizep+1:sizen)=zero_r3

    if(is_clc_Basset) then
      call move_alloc(GPrtcl_BassetData,Real3Arr)
      allocate(GPrtcl_BassetData(GPrtcl_BassetSeq%nDataLen ,sizen),stat=ierrTmp);ierr=ierr+abs(ierrTmp)
    ...
    allocate(GPrtcl_HighSt(sizen),stat=ierrTmp);ierr=ierr+abs(ierrTmp)
