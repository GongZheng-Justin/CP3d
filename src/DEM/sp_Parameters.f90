...
module sp_Parameters
...
#endif
#ifdef CFDDEM
  use f2_Parameters,only: dtMax,ifirst,ilast,BackupFreq,SaveVisu,xlx,yly,zlz,BcOption
#endif
#ifdef CFDACM
  use f2_Parameters,only: dtMax,ifirst,ilast,BackupFreq,SaveVisu,xlx,yly,zlz,BcOption,ischeme,FI_AB2,FI_RK2,FI_RK3
#endif
  implicit none
  private
  ...
  integer,public::icouple,idem_advance_start(3),idem_advance_end(3)
#endif
#ifdef CFDDEM
  ...
  type DEMS_Options
    ...
    character(len=:),allocatable :: Geom_Dir
...
contains
  ...
  ! DO_ReadDEMOption
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine DO_ReadDEMOption(this, chFile); implicit none
    class(DEMS_Options):: this
    character(len=*),intent(in)::chFile
           
    ! locals
    logical::RestartFlag
    character(512)::RunName,ResultsDir,RestartDir,Geom_Dir
    ...
    integer::numPrtcl_Type,numWall_type,CntctList_Size,CS_numlvls,iUnit,ierr
        
    logical::IsPeriodic(3)=.false.
    real(RK)::dtDEM,minpoint(3),maxpoint(3)
    integer:: ifirstDEM,ilastDEM,BackupFreqDEM,SaveVisuDEM, len_str
    ...
    read(iUnit, nml=DEMOptions)

#ifdef CFDDEM 
    rewind(iUnit)
    read(iUnit, nml=CFDDEMCoupling)
    rewind(iUnit)
    read(iUnit, nml=BassetOptions )
    ...
    if( this%GeometrySource==2 ) this%Geom_Dir = strip(Geom_Dir)
