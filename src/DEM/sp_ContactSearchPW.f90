...
module sp_ContactSearchPW
  use MPI
  use mc_TypeDef
  use mc_LogInfo
#if defined(CFDDEM) || defined(CFDACM)
  use mc_Decomp2d,only: nrank
  use sp_Decomp_2d,only:int_type,real_type
#else
  use sp_Decomp_2d,only:nrank,int_type,real_type
#endif
  use sp_Geometry
  use sp_Property
  use sp_Variables
  ...
  type(ContactSearchPW)::DEMContactSearchPW
  
  public:: DEMContactSearchPW, FindContactsPW
contains

  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  ! Initializing the object
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine InitContactSearchPW(this); implicit none
    class(ContactSearchPW)::this
    type(real3):: wallvel
    integer:: i,nw, ierr,ierrTmp
    ...
    ierr = 0
    allocate(Bucket_PWCS(this%mHead), Stat=ierrTmp); ierr = ierr +abs(ierrTmp)
    allocate(id_Wall(this%max_nearPrtcl_pWall ), Stat=ierrTmp); ierr = ierr +abs(ierrTmp)
    allocate(Next_PWCS(this%max_nearPrtcl_pWall ), Stat=ierrTmp); ierr = ierr +abs(ierrTmp)
    if(ierr /=0 ) call DEMLogInfo%CheckForError(ErrT_Abort,"Initialize Contact List","Allocation failed ")
    ...
    enddo
    call MPI_ALLREDUCE(MaxWallVel, maxvel1,1, real_type,MPI_MAX,MPI_COMM_WORLD,ierr)
  ...
  ! Init_PWCS_List
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine Init_PWCS_List(this); implicit none
    class(ContactSearchPW) :: this 
    integer::i
  ...
  ! Reallocate_Bucket
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90 
  subroutine Reallocate_Bucket(this); implicit none
    class(ContactSearchPW)::this
    ...
    sizep= this%mHead
    sizen= GPrtcl_list%mlocal ! 2024-08-09, Zheng Gong
    this%mHead = sizen

    call move_alloc(Bucket_PWCS, IntVec)
    allocate(Bucket_PWCS(sizen), stat=ierr)
    Bucket_PWCS(1:sizep)=IntVec
    Bucket_PWCS(sizep+1:sizen)=0     ! Added at 22:37, 2020-11-06, Gong Zheng
    deallocate(IntVec)
    
    if(ierr/=0) then
      call DEMLogInfo%CheckForError(ErrT_Abort,"Reallocate_Bucket "," Reallocate wrong!")
  ...
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90    
  subroutine Reallocate_PWCS_List(this); implicit none
    class(ContactSearchPW)::this 
  ...
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine UpdateNearPrtclsPW(this,iterNumber); implicit none
    class(ContactSearchPW) :: this
    integer,intent(in):: iterNumber

    ! locals
    real(RK)::max_v,max_a,t,maxreal 
    integer:: nw,i,numIter,wid,nNear,nextI,ierr
    ...
    enddo
    call MPI_ALLREDUCE(maxreal,max_v,1,real_type,MPI_MAX,MPI_COMM_WORLD,ierr)
    ...
    enddo
    call MPI_ALLREDUCE(maxreal,max_a,1,real_type,MPI_MAX,MPI_COMM_WORLD,ierr)
    ...
    if(nrank==0) then
      t = (sqrt(max_v*max_v+ 2.0_RK*max_a*this%DeltaXmax1)-max_v)/max_a
      numIter = max(int(t/DEM_opt%dt), 1)
      numIter = min(numIter, DEM_opt%Wall_max_update_iter)
    endif
    call MPI_BCAST(numIter,1,int_type,0,MPI_COMM_WORLD,ierr)
    ...
    if(nrank==0 .and. numIter<DEM_opt%Wall_max_update_iter ) then
      ...
      call DEMLogInfo%OutInfo("This process is repeated in Iteration :"//strip(num2str(DEMContactSearchPW%Next_iter_update)),4)
    endif    
  ...
  ! CSPW_copy
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine CSPW_copy(this,id1,id2); implicit none
    class(ContactSearchPW):: this
  ...
  ! InsertNearPW
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine InsertNearPW(this,pid); implicit none
    class(ContactSearchPW):: this
