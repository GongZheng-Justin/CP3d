...
#ifdef CFDACM
  logical,dimension(3),public::pbc
  real(RK),dimension(6),public::dx_pbc
  real(RK),dimension(6),public::dy_pbc
  real(RK),dimension(6),public::dz_pbc
...
#ifdef CFDACM
  public:: reallocate_ghost_for_Cntct
#endif
contains
  ...
  subroutine PC_InitComm(this); implicit none
    ...
    Prtcl_Exchange_size = 9 + 6*(GPrtcl_list%tsize + GPrtcl_list%rsize)
#ifdef CFDDEM
    ! (GPrtcl_FpForce 3)  +(GPrtcl_FpForce_old 3) +(GPrtcl_Vfluid 3+3) +(GPrtcl_linVelOld 3)=15
    Prtcl_Exchange_size = Prtcl_Exchange_size + 15
    if(is_clc_Basset) Prtcl_Exchange_size= Prtcl_Exchange_size+ 3*GPrtcl_BassetSeq%nDataLen
#endif
#ifdef CFDACM
    ! FpForce(3)+ FpTorque(3)+ FluidIntOld( 2*3)= 6+ 6 =12
    Prtcl_Exchange_size= Prtcl_Exchange_size +12
    ! PosOld(3)
    if(IBM_Scheme==2) Prtcl_Exchange_size= Prtcl_Exchange_size +3
#endif
    ...
    ierr=0
    allocate(GhostPFix_id(mFixedCS),   Stat=ierrTmp); ierr=ierr+abs(ierrTmp)
    allocate(GhostPFix_pType(mFixedCS),Stat=ierrTmp); ierr=ierr+abs(ierrTmp)
    allocate(GhostPFix_PosR(mFixedCS), Stat=ierrTmp); ierr=ierr+abs(ierrTmp)
    allocate(sendlist(msend),          Stat=ierrTmp); ierr=ierr+abs(ierrTmp)
    if(ierr/=0) call DEMLogInfo%CheckForError(ErrT_Abort,"PC_InitComm","Allocation failed-1")
    ...
    this%msend=msend
    if(allocated(sendlist)) deallocate(sendlist);
    allocate(GhostP_id(mCs),    Stat=ierrTmp); ierr=ierr+abs(ierrTmp)
    allocate(GhostP_pType(mCs), Stat=ierrTmp); ierr=ierr+abs(ierrTmp)
    allocate(GhostP_PosR(mCs),  Stat=ierrTmp); ierr=ierr+abs(ierrTmp)
    allocate(GhostP_linVel(mCs),Stat=ierrTmp); ierr=ierr+abs(ierrTmp)
    allocate(GhostP_rotVel(mCS),Stat=ierrTmp); ierr=ierr+abs(ierrTmp)
    allocate(sendlist(msend),   Stat=ierrTmp); ierr=ierr+abs(ierrTmp)
    if(ierr/=0) call DEMLogInfo%CheckForError(ErrT_Abort,"PC_InitComm","Allocation failed-2")
  end subroutine PC_InitComm

  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  ! Prtcl_Comm_For_Cntct
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine PC_Comm_For_Cntct(this); implicit none
    class(Prtcl_Comm_info)::this
    ...
    CASE(x_axis)     ! ccccccccccccccccccccccccc  x-axis  ccccccccccccccccccccccccccc
      ...
      IF(pbc(1)) THEN
        do i=1,GPrtcl_list%nlocal
          ...
          if(px<=xst2_cs) then
            ng=ng+1
            if(ng>GPrtcl_list%mGhost_CS) call reallocate_ghost_for_Cntct(ng)
      ...
      if(nsend>0) then
        ...
        call packForContact(buf_send,nsendg,nsend,yp_axis)
      ...
      if(nrecv>0) then
        ...
        ng=ng+nrecv
        if(ng>GPrtcl_list%mGhost_CS) call reallocate_ghost_for_Cntct(ng)
        call unpackForContact(buf_recv,ngp+1,ng)
      endif
      ...
      if(nsend>0) then
        nsend2=nsend*this%GhostCS_size
        allocate(buf_send(nsend2))
        call packForContact(buf_send,nsendg,nsend,ym_axis)
      ...
      if(nsend>0) then
        nsend2=nsend*this%GhostCS_size
        allocate(buf_send(nsend2))
        call packForContact(buf_send,nsendg,nsend,zp_axis)
      ...
      if(nsend>0) then
        nsend2=nsend*this%GhostCS_size
        allocate(buf_send(nsend2))
        call packForContact(buf_send,nsendg,nsend,zm_axis)
    ...
    CASE(y_axis)     ! ccccccccccccccccccccccccc  y-axis  ccccccccccccccccccccccccccc
      ...
      if(nsend>0) then
        nsend2=nsend*this%GhostCS_size
        allocate(buf_send(nsend2))
        call packForContact(buf_send,nsendg,nsend,xp_axis)
        call MPI_SEND(buf_send,nsend2,real_type,DEM_decomp%ProcNgh(3),2,MPI_COMM_WORLD,ierr)
      endif
      if(nrecv>0) then
        call MPI_WAIT(request(1),SRstatus,ierr)
      ...
      if(nsend>0) then
        nsend2=nsend*this%GhostCS_size
        allocate(buf_send(nsend2))
        call packForContact(buf_send,nsendg,nsend,xm_axis)
        call MPI_SEND(buf_send,nsend2,real_type,DEM_decomp%ProcNgh(4),4,MPI_COMM_WORLD,ierr)
      endif
      if(nrecv>0) then
        call MPI_WAIT(request(2),SRstatus,ierr)
  ...
  ! packForContact
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine packForContact(buf_send,nsendg,nsend,dir); implicit none
    ...
    do i=1,nsendg
      id=sendlist(i)
      buf_send(m)=real(GhostP_id(id),RK);     m=m+1 ! 01
    ...
    do i=nsendg+1,nsend
      id=sendlist(i)
      buf_send(m)=real(GPrtcl_id(id),RK);    m=m+1 ! 01
    ...
    enddo
  end subroutine packForContact

  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  ! unpackForContact
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine unpackForContact(buf_recv,n1,n2); implicit none
    ...
    enddo
  end subroutine unpackForContact

  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  ! PC_Comm_For_Exchange
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine PC_Comm_For_Exchange(this); implicit none
    class(Prtcl_Comm_info)::this

    ! locals
    integer::i,ierr,request(4),nlocal,nlocalp
    ...
    CASE(x_axis)     ! ccccccccccccccccccccccccc  x-axis  ccccccccccccccccccccccccccc
      ...
      ELSE
        ...
        do while(i<=nlocal)
          ...
          if(px >= xed1_cs .or. px < xst1_cs) then
            call copy_GPrtcl(i,nlocal)
            call copy_ContactList(i,nlocal)
      ...
      ELSE
        do i=1,nlocal
          ...
          if(py >= yed1_cs) then
            ...
            sendlist(nsend(1)) = i
            nlink = getPrtcl_nlink(i)
            nsend(2)= nsend(2) +nlink*5+ Prtcl_Exchange_size+1 ! The final "1" stands for END_PRTCL flag
          endif
        enddo
      ENDIF
      call MPI_SENDRECV(nsend, 2, int_type, DEM_decomp%ProcNgh(3), 9, &
                        nrecv, 2, int_type, DEM_decomp%ProcNgh(4), 9, MPI_COMM_WORLD,SRstatus,ierr)
      if(nrecv(2)>0) then
        allocate(buf_recv(nrecv(2)))
        call MPI_IRECV(buf_recv,nrecv(2),real_type,DEM_decomp%ProcNgh(4),10,MPI_COMM_WORLD,request(1),ierr)
      endif
      if(nsend(2)>0) then
        allocate(buf_send(nsend(2)))
        nlocalp= nlocal;  nlocal = nlocalp - nsend(1)
        call pack_For_Exchange(buf_send,nsend(1),nlocalp,yp_axis)
        call MPI_SEND(buf_send,nsend(2),real_type,DEM_decomp%ProcNgh(3),10,MPI_COMM_WORLD,ierr)
      endif
      if(nrecv(2)>0) then
        call MPI_WAIT(request(1),SRstatus,ierr)
        if(nlocal + nrecv(1)>=GPrtcl_list%mlocal) then
          call GPrtcl_list%ReallocatePrtclVar(nlocal + nrecv(1))
          call GPPW_CntctList%reallocateCL()
          call DEMContactSearchPW%Reallocate_Bucket()
        endif
        call unpack_For_Exchange(buf_recv,nrecv(1),nlocal,yp_axis)
      endif
      if(allocated(buf_send))deallocate(buf_send) 
      if(allocated(buf_recv))deallocate(buf_recv)
      ...
      call MPI_SENDRECV(nsend, 2, int_type, DEM_decomp%ProcNgh(4),11, &
                        nrecv, 2, int_type, DEM_decomp%ProcNgh(3),11, MPI_COMM_WORLD,SRstatus,ierr)
      if(nrecv(2)>0) then
        allocate(buf_recv(nrecv(2)))
        call MPI_IRECV(buf_recv,nrecv(2),real_type,DEM_decomp%ProcNgh(3),12,MPI_COMM_WORLD,request(2),ierr)
      endif
      if(nsend(2)>0) then
        allocate(buf_send(nsend(2)))
        nlocalp= nlocal;  nlocal = nlocalp - nsend(1)
        call pack_For_Exchange(buf_send,nsend(1),nlocalp,ym_axis)
        call MPI_SEND(buf_send,nsend(2),real_type,DEM_decomp%ProcNgh(4),12,MPI_COMM_WORLD,ierr)
      endif
      if(nrecv(2)>0) then
        call MPI_WAIT(request(2),SRstatus,ierr)
        if(nlocal + nrecv(1)>=GPrtcl_list%mlocal) then
        ...
        call unpack_For_Exchange(buf_recv,nrecv(1),nlocal,ym_axis)
      endif
      if(allocated(buf_send))deallocate(buf_send) 
      if(allocated(buf_recv))deallocate(buf_recv)
      ...
      call MPI_SENDRECV(nsend, 2, int_type, DEM_decomp%ProcNgh(1),13, &
                        nrecv, 2, int_type, DEM_decomp%ProcNgh(2),13, MPI_COMM_WORLD,SRstatus,ierr)
      if(nrecv(2)>0) then
        allocate(buf_recv(nrecv(2)))
        call MPI_IRECV(buf_recv,nrecv(2),real_type,DEM_decomp%ProcNgh(2),14,MPI_COMM_WORLD,request(3),ierr)
      endif
      if(nsend(2)>0) then
        allocate(buf_send(nsend(2)))
        nlocalp= nlocal;  nlocal = nlocalp - nsend(1)
        call pack_For_Exchange(buf_send,nsend(1),nlocalp,zp_axis)
        call MPI_SEND(buf_send,nsend(2),real_type,DEM_decomp%ProcNgh(1),14,MPI_COMM_WORLD,ierr)
      endif
      if(nrecv(2)>0) then
        call MPI_WAIT(request(3),SRstatus,ierr)
        if(nlocal + nrecv(1)>=GPrtcl_list%mlocal) then
        ...
        call unpack_For_Exchange(buf_recv,nrecv(1),nlocal,zp_axis)
      endif
      if(allocated(buf_send))deallocate(buf_send) 
      if(allocated(buf_recv))deallocate(buf_recv)
      ...
      call MPI_SENDRECV(nsend, 2, int_type, DEM_decomp%ProcNgh(2),15, &
                        nrecv, 2, int_type, DEM_decomp%ProcNgh(1),15, MPI_COMM_WORLD,SRstatus,ierr)
      if(nrecv(2)>0) then
        allocate(buf_recv(nrecv(2)))
        call MPI_IRECV(buf_recv,nrecv(2),real_type,DEM_decomp%ProcNgh(1),16,MPI_COMM_WORLD,request(4),ierr)
      endif
      if(nsend(2)>0) then
        allocate(buf_send(nsend(2)))
        nlocalp= nlocal;  nlocal = nlocalp - nsend(1)
        call pack_For_Exchange(buf_send,nsend(1),nlocalp,zm_axis)
        call MPI_SEND(buf_send,nsend(2),real_type,DEM_decomp%ProcNgh(2),16,MPI_COMM_WORLD,ierr)
      endif
      if(nrecv(2)>0) then
        call MPI_WAIT(request(4),SRstatus,ierr)
        if(nlocal + nrecv(1)>=GPrtcl_list%mlocal) then
        ...
        call unpack_For_Exchange(buf_recv,nrecv(1),nlocal,zm_axis)
      endif
      if(allocated(buf_send))deallocate(buf_send) 
      if(allocated(buf_recv))deallocate(buf_recv)
    ...
    CASE(y_axis)     ! ccccccccccccccccccccccccc  y-axis  ccccccccccccccccccccccccccc
      ...
      if(nsend(2)>0) then
        allocate(buf_send(nsend(2)))
        nlocalp= nlocal;  nlocal = nlocalp - nsend(1)
        call pack_For_Exchange(buf_send,nsend(1),nlocalp,xp_axis)
        call MPI_SEND(buf_send,nsend(2),real_type,DEM_decomp%ProcNgh(3),10,MPI_COMM_WORLD,ierr)
      endif
      if(nrecv(2)>0) then
        call MPI_WAIT(request(1),SRstatus,ierr)
        ...
        call unpack_For_Exchange(buf_recv,nrecv(1),nlocal,xp_axis)
      endif
      if(allocated(buf_send))deallocate(buf_send) 
      if(allocated(buf_recv))deallocate(buf_recv)
      ...
      if(nsend(2)>0) then
        allocate(buf_send(nsend(2)))
        nlocalp= nlocal;  nlocal = nlocalp - nsend(1)
        call pack_For_Exchange(buf_send,nsend(1),nlocalp,xm_axis)
        call MPI_SEND(buf_send,nsend(2),real_type,DEM_decomp%ProcNgh(4),12,MPI_COMM_WORLD,ierr)
      endif
      if(nrecv(2)>0) then
        call MPI_WAIT(request(2),SRstatus,ierr)
        ...
        call unpack_For_Exchange(buf_recv,nrecv(1),nlocal,xm_axis)
      endif
      if(allocated(buf_send))deallocate(buf_send) 
      if(allocated(buf_recv))deallocate(buf_recv)
  ...
  ! pack_For_Exchange
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine pack_For_Exchange(buf_send,nsend,nlocalp,dir); implicit none
    real(RK),dimension(:),intent(out)::buf_send
    ...
    IF(is_clc_Basset)  THEN
      DO i=1,nsend
        ...
        buf_send(m)=real(GPrtcl_id(id),RK);      m=m+1 ! 01
        buf_send(m)=real(GPrtcl_pType(id),RK);   m=m+1 ! 02
        buf_send(m)=real(GPrtcl_usrMark(id),RK); m=m+1 ! 03
        ...
        enddo
        call gather_ContactList(id,buf_send,m)         ! contact list part
        buf_send(m) = END_OF_PRTCL;              m=m+1 ! END_PRTCL flag
      ENDDO
    ELSE
      DO i=1,nsend
        id = sendlist(i)
   ...
   DO i=1,nsend
    ...
    DO i=nsend,1,-1
      id = sendlist(i)
      call copy_GPrtcl(id,nlocal)
      call copy_ContactList(id,nlocal)
      call DEMContactSearchPW%copy(id,nlocal)
      nlocal = nlocal -1
    ENDDO
  end subroutine pack_For_Exchange

  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  ! unpack_For_Exchange
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine unpack_For_Exchange(buf_recv,nrecv,nlocal,dir); implicit none
    real(RK),dimension(:),intent(in)::buf_recv
    ...
    If (is_clc_Basset) then
      DO i=1,nrecv
        if( .not.(IsPrtcl_InThisProc(buf_recv,m,dir)) ) cycle
    ...
    ENDDO
#endif
  end subroutine unpack_For_Exchange

  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  ! IsPrtcl_InThisProc
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  function IsPrtcl_InThisProc(buf_recv,m,dir) result(res); implicit none
    real(RK),dimension(:),intent(in)::buf_recv
    ...
    call DEMLogInfo%CheckForError(ErrT_Pass," IsPrtcl_InThisProc"," The following particle is deleted: ")
    call DEMLogInfo%OutInfo(" Exchange direction  is :"//strip(num2str(dir)  ),3)
    call DEMLogInfo%OutInfo("   The particle id   is :"//strip(num2str( nint(buf_recv(m)  ))),3)
    call DEMLogInfo%OutInfo("       particle type is :"//strip(num2str( nint(buf_recv(m+1)))),3)
    call DEMLogInfo%OutInfo("       x-coordinate  is :"//strip(num2str( buf_recv(m+3) )), 3)
    call DEMLogInfo%OutInfo("       y-coordinate  is :"//strip(num2str( buf_recv(m+4) )), 3)
    call DEMLogInfo%OutInfo("       z-coordinate  is :"//strip(num2str( buf_recv(m+5) )), 3)
    call DEMLogInfo%OutInfo(" Present processor   is :"//strip(num2str(nrank)), 3)

    m = m + Prtcl_Exchange_size
    do while( buf_recv(m) /= END_OF_PRTCL )
      m=m+1
    enddo
    m=m+1
  end function IsPrtcl_InThisProc

  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  ! reallocate_ghost_for_Cntct
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90  
  subroutine reallocate_ghost_for_Cntct(ng); implicit none
    ...
    allocate(GhostP_id(sizen),stat=ierrTmp);ierr=ierr+abs(ierrTmp)
    GhostP_id(1:sizep)=IntVec

    call move_alloc(GhostP_pType,IntVec)
    allocate(GhostP_pType(sizen),stat=ierrTmp);ierr=ierr+abs(ierrTmp)
    GhostP_pType(1:sizep)=IntVec
    deallocate(IntVec)  

    ! ======= real3 vercor part =======
    call move_alloc(GhostP_linVel,Real3Vec)
    allocate(GhostP_linVel(sizen),stat=ierrTmp);ierr=ierr+abs(ierrTmp)
    GhostP_linVel(1:sizep)=Real3Vec
 
    call move_alloc(GhostP_rotVel,Real3Vec)
    allocate(GhostP_rotVel(sizen),stat=ierrTmp);ierr=ierr+abs(ierrTmp)
    GhostP_rotVel(1:sizep)=Real3Vec
    deallocate(Real3Vec)

    ! ======= real4 vercor part =======
    call move_alloc(GhostP_PosR,Real4Vec)
    allocate(GhostP_PosR(sizen),stat=ierrTmp);ierr=ierr+abs(ierrTmp)
    ...
    !call DEMLogInfo%OutInfo("The present processor  is :"//strip(num2str(nrank)),3)
    !call DEMLogInfo%OutInfo("Previous matirx length is :"//strip(num2str(sizep)),3)
    !call DEMLogInfo%OutInfo("Updated  matirx length is :"//strip(num2str(sizen)),3)
  end subroutine reallocate_ghost_for_Cntct

  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  ! PC_reallocate_sendlist
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90  
  subroutine PC_reallocate_sendlist(this,ns); implicit none
    class(Prtcl_Comm_info)::this
  ...
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine PC_Comm_For_Cntct_fixed(this); implicit none
    class(Prtcl_Comm_info)::this
    ...
    CASE(x_axis)     ! ccccccccccccccccccccccccc  x-axis  ccccccccccccccccccccccccccc
      ...
      if(nsend>0) then
        ...
        allocate(buf_send(nsend2))
        call packForContact_fixed(buf_send,nsendg,nsend,yp_axis)
        call MPI_SEND(buf_send,nsend2,real_type,DEM_decomp%ProcNgh(3),2,MPI_COMM_WORLD,ierr)
      endif
      if(nrecv>0) then
        call MPI_WAIT(request(1),SRstatus,ierr)
        ng=ng+nrecv
        if(ng>GPrtcl_list%nGhostFix_CS) call reallocate_ghostFix_for_Cntct(ng)
        call unpackForContact_fixed(buf_recv,ngp+1,ng)
      endif
      if(allocated(buf_send))deallocate(buf_send) 
      if(allocated(buf_recv))deallocate(buf_recv)
      ...
      if(nsend>0) then
        nsend2=nsend*GhostFixedCS_size
        allocate(buf_send(nsend2))
        call packForContact_fixed(buf_send,nsendg,nsend,ym_axis)
        call MPI_SEND(buf_send,nsend2,real_type,DEM_decomp%ProcNgh(4),4,MPI_COMM_WORLD,ierr)
      endif
      if(nrecv>0) then
        call MPI_WAIT(request(2),SRstatus,ierr)
      ...
      if(nsend>0) then
        nsend2=nsend*GhostFixedCS_size
        allocate(buf_send(nsend2))
        call packForContact_fixed(buf_send,nsendg,nsend,zp_axis)
        call MPI_SEND(buf_send,nsend2,real_type,DEM_decomp%ProcNgh(1),6,MPI_COMM_WORLD,ierr)
      endif
      if(nrecv>0) then
        call MPI_WAIT(request(3),SRstatus,ierr)
      ...
      if(nsend>0) then
        nsend2=nsend*GhostFixedCS_size
        allocate(buf_send(nsend2))
        call packForContact_fixed(buf_send,nsendg,nsend,zm_axis)
        call MPI_SEND(buf_send,nsend2,real_type,DEM_decomp%ProcNgh(2),8,MPI_COMM_WORLD,ierr)
      endif
      if(nrecv>0) then
        call MPI_WAIT(request(4),SRstatus,ierr)
    ...
    CASE(y_axis)     ! ccccccccccccccccccccccccc  y-axis  ccccccccccccccccccccccccccc
      ...
      if(nsend>0) then
        nsend2=nsend*GhostFixedCS_size
        allocate(buf_send(nsend2))
        call packForContact_fixed(buf_send,nsendg,nsend,xp_axis)
        call MPI_SEND(buf_send,nsend2,real_type,DEM_decomp%ProcNgh(3),2,MPI_COMM_WORLD,ierr)
      endif
      if(nrecv>0) then
        call MPI_WAIT(request(1),SRstatus,ierr)
      ...
      if(nsend>0) then
        nsend2=nsend*GhostFixedCS_size
        allocate(buf_send(nsend2))
        call packForContact_fixed(buf_send,nsendg,nsend,xm_axis)
        call MPI_SEND(buf_send,nsend2,real_type,DEM_decomp%ProcNgh(4),4,MPI_COMM_WORLD,ierr)
      endif
      if(nrecv>0) then
        call MPI_WAIT(request(2),SRstatus,ierr)
  ...
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  ! packForContact_fixed
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine packForContact_fixed(buf_send,nsendg,nsend,dir); implicit none
    real(RK),dimension(:),intent(out)::buf_send
    integer,intent(in)::nsendg,nsend,dir
    ...
    do i=1,nsendg
      id=sendlist(i)
      buf_send(m)=real(GhostPFix_id(id),RK);    m=m+1 ! 01
      buf_send(m)=real(GhostPFix_pType(id),RK); m=m+1 ! 02
    ...
    do i=nsendg+1,nsend
      id=sendlist(i)
      buf_send(m)=real(GPFix_id(id),RK);        m=m+1 ! 01
      buf_send(m)=real(GPFix_pType(id),RK);     m=m+1 ! 02
      buf_send(m)=GPFix_PosR(id)%x+dx;          m=m+1 ! 03
      buf_send(m)=GPFix_PosR(id)%y+dy;          m=m+1 ! 04
      buf_send(m)=GPFix_PosR(id)%z+dz;          m=m+1 ! 05
    enddo
  end subroutine packForContact_fixed

  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  ! unpackForContact_fixed
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine unpackForContact_fixed(buf_recv,n1,n2); implicit none
    real(RK),dimension(:),intent(in)::buf_recv
    integer,intent(in)::n1,n2
    ...
    enddo
  end subroutine unpackForContact_fixed

  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  ! reallocate_ghostFix_for_Cntct
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90  
  subroutine reallocate_ghostFix_for_Cntct(ng); implicit none
    integer,intent(in)::ng
