...
module sp_NBS_Munjiza
  ...
  integer:: NBSM_nx         ! number of divisions in x direction
  integer:: NBSM_ny         ! number of divisions in y direction
  integer:: NBSM_nz         ! number of divisions in z direction  
...
contains
  ...
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine Init_NBSM(this); implicit none
    ...
    NBSM_nx = int((xed_cs-xst_cs)/this%cell_len)+1
    NBSM_ny = int((yed_cs-yst_cs)/this%cell_len)+1
    NBSM_nz = int((zed_cs-zst_cs)/this%cell_len)+1
        
    numcell = integer3(NBSM_nx, NBSM_ny, NBSM_nz)
    if(nrank==0) then
      call DEMLogInfo%OutInfo("Contact search method is NBS Munjiza", 3 )
      call DEMLogInfo%OutInfo("Cell size is [m]: "// strip(num2str(this%cell_len)), 4)
      call DEMLogInfo%OutInfo("Number of cells considered is (x,y,z) :"//strip(num2str(numCell)),4) 
    endif       
    ...
    allocate(HeadY(NBSM_ny),         STAT=iErr2)
    allocate(HeadX(NBSM_nx),         STAT=iErr3)
    allocate(HeadX0(NBSM_nx),        STAT=iErr4)
    allocate(HeadZ(0:1,0:NBSM_nz+1), STAT=iErr5)
    allocate(HeadZ0(0:2,0:NBSM_nz+1),STAT=iErr6)
  ...
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine NBSM_ContactSearch(this); implicit none
    class(NBS_Munjiza):: this      
    ...
    call NBSM_BuildYlist()

    HeadX0= -1
    DO iy = 1, NBSM_ny
      call NBSM_BuildXList(iy)
      if(HeadY(iy) /= -1 ) then
        HeadZ(0,:)  = -1
        HeadZ0(0,:) = -1
        call NBSM_BuildZlist0(1,1)
        do ix = 1, NBSM_nx
          call NBSM_BuildZlist(ix,1)
          call NBSM_BuildZlist0(ix,2)
  ...
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine NBSM_clcBoxIndex(this); implicit none
    class(NBS_Munjiza):: this

    ! locals
    real(RK)::rpdx,rpdy,rpdz
    integer::i,m,sizen,ierrTmp,ierr=0
    ...
    if(nNeedCSTotoal>this%mbox) then
      ...
      allocate(box_index(sizen),stat=ierrTmp);ierr=ierr+abs(ierrTmp)
      if(nFixed>0)box_index(1:nFixed)= Int3Vec(1:nFixed)
      deallocate(Int3Vec)

      deallocate(NextX); allocate(NextX(sizen),stat=ierrTmp);ierr=ierr+abs(ierrTmp)
      deallocate(NextY); allocate(NextY(sizen),stat=ierrTmp);ierr=ierr+abs(ierrTmp)
      deallocate(NextZ); allocate(NextZ(sizen),stat=ierrTmp);ierr=ierr+abs(ierrTmp)
      if(ierr/=0) then
  ...
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine NBSM_BuildYList(); implicit none
    integer:: i,iy
  ...
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine NBSM_BuildXList(iy); implicit none
  ...
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine NBSM_BuildZList(ix,m); implicit none
  ...
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine NBSM_BuildZList0(ix,m); implicit none
  ...
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine NBSM_LoopNBSMask(this, iz); implicit none
    class(NBS_Munjiza) this
    ...
    DO WHILE(m/=-1)
      ...
      ELSE                             !==================================
       ...
       !over particles in the same cell but not the same particle (to prevent self-contact)
        ...
        DO WHILE(n/=-1)
          if(n<nFixedP) then
            call NBSM_FineSearch1(m,n)
          elseif(n>nFixedAndlocal) then
            call NBSM_FineSearch3(m,n)
          else
            call NBSM_FineSearch2(m,n)
          endif
          this%num_Cnsv_cntct = this%num_Cnsv_cntct + 1
          n = NextZ(n)
        ENDDO
  ...
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90    
  subroutine NBSM_FineSearch1(pid1,pid2); implicit none
  ...
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90    
  subroutine NBSM_FineSearch2(pid1,pid2); implicit none
    integer,intent(in):: pid1,pid2
  ...
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90    
  subroutine NBSM_FineSearch3(pid1,pid2); implicit none
    integer,intent(in):: pid1,pid2
