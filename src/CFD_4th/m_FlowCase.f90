...
#define nGradStat     98
    ...
    DO jc=1,nyc
      ...
      do kc=y1start(3),y1end(3)
        ...
        do ic=y1start(1),y1end(1)
          ...
          SumVec(31)=SumVec(31)+ (dudxC*(dvdxU+dvdx)+ (dudyU+dudy)*(uy_xp-uy_xc)*caj)*0.5_RK !2021-12-14
          ...
          SumVec(45)=SumVec(45)+ 0.5_RK*(FpForce_y(ic,jc,kc)+FpForce_y(ic,jp,kc))*uxCell+ &
  ...
  subroutine clcSpectraAndLCSR2D(ArrIN,m1,m2)
...
#else
    allocate(ArrFFT(nxc,x1size(2),x1size(3)))
    ...
    deallocate(arrFFT)
#endif
    arrx_xhzf(1,:,:)=0.0_RK; arrx_xhzf(nxhp,:,:)=0.0_RK
    do ic=2,nxh
      ict=nxc+2-ic
  ...
  subroutine clcCosSpectra2D(ArrIN1,ArrIN2,m,IsSymmetry)
    ...
    deallocate(arrFFT)
#endif   
    arrx_xhzf1(1,:,:)=0.0_RK; arrx_xhzf1(nxhp,:,:)=0.0_RK
    do ic=2,nxh
      ict=nxc+2-ic
    ...
    deallocate(arrFFT)
#endif        
    arrx_xhzf2(1,:,:)=0.0_RK; arrx_xhzf2(nxhp,:,:)=0.0_RK
    do ic=2,nxh
      ict=nxc+2-ic
