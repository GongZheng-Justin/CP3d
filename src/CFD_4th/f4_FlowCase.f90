...
#undef  EnergySpectra_staggered_4th
  ...
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
#ifdef ScalarFlow
  subroutine InitFlowField(ux,uy,uz,Deviation,scalar); implicit none
    real(RK),dimension(mb1%xmm:mb1%xpm,mb1%ymm:mb1%ypm,mb1%zmm:mb1%zpm),intent(out)::ux,uy,uz,scalar
#else
  subroutine InitFlowField(ux,uy,uz,Deviation); implicit none
    ...
    ! locals
    integer ::ic,jc,kc,m1,m2,iTV(8),ierr
  ...
  ! Update_uy_ym
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90   
  subroutine Update_uy_ym(uy_ym, duy_ym); implicit none
  ...
  ! InitStatVar
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine InitStatVar(ChannelPrm); implicit none 
    character(len=*),intent(in)::ChannelPrm

    ! locals
    character(len=:),allocatable::filename
    integer::ierr,iUnit,plan_type,nySpec2D
    ...
    if(ierr/=0 .and. nrank==0) call MainLog%CheckForError(ErrT_Abort,"InitStatVar", "Cannot open file: "//strip(ChannelPrm))
    read(iUnit, nml=SpectraOptions)
    close(iUnit,IOSTAT=ierr)
    
    if(nrank==0) then
      write(MainLog%iUnit, nml=SpectraOptions)
      ...
      if(IsUxConst)then
        ...
        if(ierr/=0) call MainLog%CheckForError(ErrT_Abort,"InitCAStatistics","Cannot open file: "//filename)
        close(iUnit,IOSTAT=ierr)
      endif
    endif
    allocate(SumStat(NCHASTAT,nyp),Stat=ierr)
    if(ierr/=0) call MainLog%CheckForError(ErrT_Abort,"InitStatVar: ","Allocation failed 1")  
    nfstime=0; nSpectime=0; SumStat=0.0_RK; PrGradsum=0.0_RK
#ifdef HighOrderGradStat
    allocate(SumGrad(nGradStat,nyp),Stat=ierr)
    if(ierr/=0) call MainLog%CheckForError(ErrT_Abort,"InitStatVar: ","Allocation failed 2")  
    SumGrad=0.0_RK
#endif
    ...
    IF(clcSpectra2D) THEN
      ...
      allocate(EnergySpec2D(decomp_xhzh%y2sz(1),nySpec2D,decomp_xhzh%y2sz(3),NEnergySpec2D),Stat=ierr)
      if(ierr/=0) call MainLog%CheckForError(ErrT_Abort,"InitStatVar","Allocation failed For Spectra2D")
      EnergySpec2D=0.0_RK
    ENDIF
  ...
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
#ifdef ScalarFlow
  subroutine clcStat(ux,uy,uz,pr,scalar,ArrTemp1,ArrTemp2); implicit none
    real(RK),dimension(mb1%xmm:mb1%xpm,mb1%ymm:mb1%ypm,mb1%zmm:mb1%zpm),intent(in)::ux,uy,uz,pr,scalar
#else
  subroutine clcStat(ux,uy,uz,pr,ArrTemp1,ArrTemp2); implicit none
    ...
    integer::ic,jc,kc,im,jm,km,ip,jp,kp,it,jt,kt,ierr,ids,is,ks,iu,ku,nrankX,nrankZ,iUnit
...
#undef  EnergySpectra_staggered_4th
    ...
    ! Write statistics
    call MPI_REDUCE(SumStat,SumStatR,NCHASTAT*nyp,real_type,MPI_SUM,0,MPI_COMM_WORLD,ierr)
    if(nrank==0) then
      infstime = 1.0_RK/real(nfstime,RK)
      filename = strip(ResultsDir_) // 'stats' // int2str(itime,10)
      open(newunit=iUnit,file=filename,status='replace',form='formatted',IOSTAT=ierr)
      IF(ierr/=0) THEN
        call MainLog%CheckForError(ErrT_Pass,"clcStat","Cannot open file: "//filename)
      ELSE
        ...
        write(iUnit,'(A)')'  '
        if(IsUxConst) then
          write(iUnit,'(A)')'  Constant velocity in x-dir by adding a pressure gradient.'
          write(iUnit,'(A, ES24.15)')'    time averaged pressure gradient is: ',PrGradsum*infstime
        else
          ...
          write(iUnit,'(A, ES24.15)')'  Variable velocity in x-dir while adding a constant body force.',xnu*dudy/yly
        endif
        write(iUnit,'(A)')'  '
        ...
        do jc=1,nyp
          write(iUnit,FormatStr)SumStatR(1:NCHASTAT,jc)*infstime
        enddo
        End Block
      ENDIF
      close(iUnit,IOSTAT=ierr)
...
#ifdef HighOrderGradStat
    call MPI_REDUCE(SumGrad,SumGradR,nGradStat*nyp,real_type,MPI_SUM,0,MPI_COMM_WORLD,ierr)
    if(nrank==0) then
      infstime = 1.0_RK/real(nfstime,RK)
      filename = strip(ResultsDir_) // 'vGrad' // int2str(itime,10)
      open(newunit=iUnit,file=filename,status='replace',form='formatted',IOSTAT=ierr)
      IF(ierr/=0) THEN
        call MainLog%CheckForError(ErrT_Pass,"clcStat","Cannot open file: "//filename)
      ELSE
        ...
        do jc=1,nyp
          write(iUnit,FormatStr)SumGradR(1:nGradStat,jc)*infstime
        enddo
        End Block
      ENDIF
      close(iUnit,IOSTAT=ierr)  
