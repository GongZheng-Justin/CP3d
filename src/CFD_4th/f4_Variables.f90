...
contains  
  ...
  subroutine AllocateVariables(); implicit none
    ...
    scalar=Scalar_InitValue; RealArrC=0.0_RK; HistCOld=0.0_RK
#endif
    !-------------------------------------------------
    ...
    allocate(uy_ym(y1start(1):y1end(1),  y1start(3):y1end(3)), Stat=ierrTmp);ierr=ierr+abs(ierrTmp)
    allocate(duy_ym(y1start(1):y1end(1), y1start(3):y1end(3)), Stat=ierrTmp);ierr=ierr+abs(ierrTmp)
    if(ierr /= 0) call MainLog%CheckForError(ErrT_Abort,"AllocateVariables","Allocation failed")    
    ...
    uy_ym=0.0_RK;      duy_ym=0.0_RK
      
  end subroutine AllocateVariables
    
end module f4_Variables
