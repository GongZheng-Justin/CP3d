...
#ifdef CFDLPT_TwoWay
  use f4_Variables,only: FpForce_x,FpForce_y,FpForce_z
#endif
  use f4_Tools,only: InverseTridiagonal,InversePeriodicTridiagonal,InversePTriFixedCoe
  implicit none
  private
...
contains    
  ...
  subroutine InitTimeScheme(); implicit none
    ...
    ! ux/uz Laplacian metries in y-dir (for Crank-Nicolson scheme purpose)    
    allocate(ap2cForCN(1:nyc),ac2cForCN(1:nyc),am2cForCN(1:nyc), Stat=ierr)
...
#ifdef ScalarFlow
    ! scalar Laplacian metries in y-dir (for Crank-Nicolson scheme purpose)
    allocate(ap2cScCN(1:nyc),ac2cScCN(1:nyc),am2cScCN(1:nyc), Stat=ierr)
    block
    integer::j
    do j=1,nyc
    ...
    CASE(-1)
      am2cScCN(1)= 8.0_RK*rdyc(1)/( dyc(1)+2.0_RK*dyc(2) )
      ap2cScCN(1)= 4.0_RK*rdyc(2)/( dyc(1)+2.0_RK*dyc(2) )
      ac2cScCN(1)= -(8.0_RK*rdyc(1)+4.0_RK*rdyc(2))/( dyc(1)+2.0_RK*dyc(2) )
    CASE(-2)
    ...
    CASE(-3)
      ap2cScCN(1)= rdyp(1)*rdyc(2)
      ac2cScCN(1)= ac2cScCN(1)+ am2cScCN(1)*(2.0_RK+ScalarBcValues(1)*dyc(1))/(2.0_RK-ScalarBcValues(1)*dyc(1))
      am2cScCN(1)= 0.0_RK
    END SELECT
    SELECT CASE(ScalarBcOption(2))
    CASE(-1)
      am2cScCN(nyc)=  4.0_RK*rdyc(nyc)/( dyc(nyp)+2.0_RK*dyc(nyc) )
      ap2cScCN(nyc)=  8.0_RK*rdyc(nyp)/( dyc(nyp)+2.0_RK*dyc(nyc) )
      ac2cScCN(nyc)=-(8.0_RK*rdyc(nyp)+4.0_RK*rdyc(nyc))/( dyc(nyp)+2.0_RK*dyc(nyc) )
    CASE(-2)
    ...
    CASE(-3)
      am2cScCN(nyc)= rdyp(nyc)*rdyc(nyc)
      ac2cScCN(nyc)= ac2cScCN(nyc)+ ap2cScCN(nyc)*(2.0_RK-ScalarBcValues(2)*dyc(nyp))/(2.0_RK+ScalarBcValues(2)*dyc(nyp))
      ap2cScCN(nyc)= 0.0_RK
    END SELECT
#endif    
  ...
  ! clcPrSrc
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90  
  subroutine clcPrSrc(ux,uy,uz,prsrc,divmax); implicit none
    ...
    ! locals
    integer::ic,jc,kc,ip,jp,kp,ierr,km,im,ku,iu
    real(RK)::sucaj,rdiv,divmax1,dudx,dvdy,dwdz
    ...
    DO kc=y1start(3),y1end(3)
      ...
      do jc=y1start(2),y1end(2)
        ...
        do ic=y1start(1),y1end(1)
          ...
          ux(ic,jc,kc)=ux(ic,jc,kc) -dpdx
          uy(ic,jc,kc)=uy(ic,jc,kc) -dpdy
          uz(ic,jc,kc)=uz(ic,jc,kc) -dpdz               
        enddo
      enddo
    ENDDO
  ...
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
#ifdef ScalarFlow
  subroutine clcRhsXYZ(ux,uy,uz,scalar,RhsX,RhsY,RhsZ,RhsC,HistXOld,HistYOld,HistZOld,HistCOld,pressure); implicit none
...
#else
  subroutine clcRhsXYZ(ux,uy,uz,RhsX,RhsY,RhsZ,HistXOld,HistYOld,HistZOld,pressure); implicit none
...
#endif
    integer::im,ic,ip,jc,jm,jp,km,kc,kp,is,ks,iu,ku,ierr
    real(RK)::uxhyp,uxhyc,uxhym,uxhyu,uzhym,uzhyc,uzhyp,uzhyu,gradp1,gradp2,gradp3
    real(RK)::d11q1,d22q1,d33q1,d11q2,d22q2,d33q2,d11q3,d22q3,d33q3,dcq13,ForcedXnu
    real(RK)::h11,h12,h13,h21,h22,h23,h31,h32,h33,InterpY1,InterpY2,InterpY3,InterpY4
    real(RK)::sucaj,Forced,Forced_tot,ForcedCoe,dp1ns,convEd1,convEd2,convEd3,sucac,qsucac
    ...
    DO kc=y1start(3),y1end(3)
      ...
      jc=nyp
      do ic=y1start(1),y1end(1)
        uyhx(ic,jc,kc)=0.0_RK; uyhz(ic,jc,kc)=0.0_RK;
      enddo
    enddo
    call update_halo(uxhx, mb1, hi_uxhx)
    call update_halo(uyhx, mb1, hi_uyhx)
    call update_halo(uzhx, mb1, hi_uzhx)
    call update_halo(uxhz, mb1, hi_uxhz)
    call update_halo(uyhz, mb1, hi_uyhz)
    call update_halo(uzhz, mb1, hi_uzhz)
    ...
    DO kc=y1start(3),y1end(3)
      ...
      enddo
    ENDDO
    call update_halo(shx, mb1, hi_shx)
    call update_halo(shz, mb1, hi_shz)
    ...
    DO kc=y1start(3),y1end(3)
      ...
      do jc=y1start(2),y1end(2)
        ...
        do ic=y1start(1),y1end(1)
          ...
          Forced=Forced +ForcedCoe*convEd1
          ...
          RhsC(ic,jc,kc)= pmGamma*convEdc+ pmTheta*HistCOld(ic,jc,kc)+ 2.0_RK*pmBetaSc*d22sc
    ...
    IF(IsUxConst) THEN
      call MPI_ALLREDUCE(Forced,Forced_tot,1,real_type,MPI_SUM,MPI_COMM_WORLD,ierr)
      Forced= -Forced_tot/(real(nxc*nzc,kind=RK))/yly -gravity(1)
      dp1ns = pmGamma*Forced +pmTheta*PrGradData(4)
      PrGradData(4) =Forced
      do kc=y1start(3),y1end(3)
        do jc=y1start(2),y1end(2)
          do ic=y1start(1),y1end(1)
  ...
  ! clcU1Hat
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90    
  subroutine clcU1Hat(ux,RhsX); implicit none
    ...
    do jc=y1start(2),y1end(2)
      do ic=y1start(1),y1end(1)
        ...
        tridcj(ic,jc) = -pmBeta*ac2cForCN(jc)+1.0_RK
  ...
  ! clcU2Hat
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90  
  subroutine clcU2Hat(uy,RhsY,duy_ym); implicit none
  ...
  ! clcU3Hat
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90  
  subroutine clcU3Hat(uz,RhsZ); implicit none
  ...
  ! clcScalar
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90  
  subroutine clcScalar(scalar,RhsC); implicit none
    real(RK),dimension(mb1%xmm:mb1%xpm,mb1%ymm:mb1%ypm,mb1%zmm:mb1%zpm),intent(inout)::RhsC
    ...
    do jc=y1start(2),y1end(2)
      do ic=y1start(1),y1end(1)
        ...
        tridcj(ic,jc) = -pmBetaSc*ac2cScCN(jc)+1.0_RK
  ...
  subroutine PressureUpdate(pressure, prphiHalo); implicit none
    ...
    DO kc=y1start(3),y1end(3)
      do jc=y1start(2),y1end(2)
        jp=jc+1;  jm=jc-1
        pjc= xnuhm*ap2Pr(jc)
        cjc= xnuhm*ac2Pr(jc) +rpmAlpha
        mjc= xnuhm*am2Pr(jc)
        do ic=y1start(1),y1end(1)
          pressure(ic,jc,kc)= pressure(ic,jc,kc)+ pjc*prphiHalo(ic,jp,kc)+ cjc*prphiHalo(ic,jc,kc)+ &
                                                  mjc*prphiHalo(ic,jm,kc)
        enddo
      enddo
    ENDDO
  end subroutine PressureUpdate

end module f4_TScheme
