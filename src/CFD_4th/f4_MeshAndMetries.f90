...
contains
  ...
  subroutine InitMeshAndMetries(ChannelPrm); implicit none 
...
#ifdef ScalarFlow
    allocate(ap2cSc(1:nyc), Stat=ierrTmp);ierr=ierr+abs(ierrTmp)
    allocate(ac2cSc(1:nyc), Stat=ierrTmp);ierr=ierr+abs(ierrTmp)
    allocate(am2cSc(1:nyc), Stat=ierrTmp);ierr=ierr+abs(ierrTmp)
#endif
    ...
    do j=0,nyp
      DeltaCell(j)=(dyp(j)*dx*dz)**(0.33333333333333333333_RK)
    enddo
    ...
    if(ScalarBcOption(1)==-1) then
      am2cSc(1)= 4.0_RK*rdyc(1)/( dyc(1)+2.0_RK*dyc(2) )
      ap2cSc(1)= 4.0_RK*rdyc(2)/( dyc(1)+2.0_RK*dyc(2) )
    endif
    if(ScalarBcOption(2)==-1) then
      am2cSc(nyc)= 4.0_RK*rdyc(nyc)/( dyc(nyp)+2.0_RK*dyc(nyc) )
      ap2cSc(nyc)= 4.0_RK*rdyc(nyp)/( dyc(nyp)+2.0_RK*dyc(nyc) )
    endif
  ...
  ! clcYcoord
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine clcYcoord(ycoord,nidY,ySectionCoord,StretType,cStret,StretOption); implicit none
    ...
    CASE(1) ! Tangent hyperbolic function
      ...
      do j=0,m
        ...
        y0= tanh(cStret*(xi-1.0_RK))/tstr + 1.0_RK  ! For j: [0,m],  y0: [0, 1]
    ...
    CASE(2) ! Sine/cosine function
      ...
      do j=0,m
        ...
        y0= sin(cStret*xi*PI*0.5_RK)/tstr + 1.0_RK ! For j: [0,m],  y0: [0, 1]
        if(StretOption==0) then  ! bottom
          jt=nidY(1)+j
          ycoord(jt)=ySectionCoord(1) + y0*yLen
        else                     ! top
