...
contains    
  ...
  subroutine CalcMaxCFL(ux,uy,uz,uddxmax); implicit none
    ...
    call MPI_ALLREDUCE(vcf,uddxmax,1,real_type,MPI_MAX,MPI_COMM_WORLD,ierr)
  end subroutine CalcMaxCFL
    
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  ! CheckDivergence
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90     
  subroutine CheckDivergence(ux,uy,uz, divmax); implicit none
    real(RK),dimension(mb1%xmm:mb1%xpm,mb1%ymm:mb1%ypm,mb1%zmm:mb1%zpm),intent(in)::ux,uy,uz
  ...
  ! CalcVmax
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90  
  function CalcVmax(ux,uy,uz) result(vmaxabs); implicit none
    real(RK),dimension(mb1%xmm:mb1%xpm,mb1%ymm:mb1%ypm,mb1%zmm:mb1%zpm),intent(in)::ux,uy,uz
    ...
    do kc=y1start(3),y1end(3)
      ...
      enddo
    enddo
    call MPI_REDUCE(vmaxabs1,vmaxabs,3,real_type,MPI_MAX,0,MPI_COMM_WORLD,ierr)

  end function CalcVmax

  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  ! InversePeriodicTridiagonal: periodic tridiagonal solvers (variable coefficients)
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine InversePeriodicTridiagonal(aj,bj,cj,fj,m,n); implicit none
  ...
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  ! InversePTriFixedCoe: periodic tridiagonal solvers (fixedcoefficients)
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine InversePTriFixedCoe(ajf,bjf,cjf,fj,m,n); implicit none
    integer,intent(in)::m,n
  ...
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  ! InverseTridiagonal: tridiagonal solvers (variable coefficients)
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90  
  subroutine InverseTridiagonal(aj,bj,cj,fj,m,n); implicit none
    integer,intent(in)::m,n
    real(RK),dimension(m,n),intent(in):: aj,bj,cj
    real(RK),dimension(m,n),intent(inout)::fj
  ...
  ! CalcUxAver
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  function CalcUxAver(ux) result(res); implicit none
    ...
    do kc=y1start(3),y1end(3)
      ...
      enddo
    enddo
    call MPI_REDUCE(res1,res,1,real_type,MPI_SUM,0,MPI_COMM_WORLD,ierr)
  ...
  ! ClcMeanValue
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  function ClcMeanValue(ux,scalar) result(res); implicit none
    ...
    do kc=y1start(3),y1end(3)
      ...
      enddo
    enddo
    call MPI_REDUCE(res1,res,3,real_type,MPI_SUM,0,MPI_COMM_WORLD,ierr)
    res=res/(real(nxc*nzc,kind=RK))/yly
  ...
  ! Clc_Q_vor
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine Clc_Q_vor(ux,uy,uz,Q_vor); implicit none
    real(RK),dimension(mb1%xmm:mb1%xpm,mb1%ymm:mb1%ypm,mb1%zmm:mb1%zpm),intent(in)::ux,uy,uz
  ...
  ! Clc_lamda2
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine Clc_lamda2(ux,uy,uz,lamda2_vor); implicit none
    real(RK),dimension(mb1%xmm:mb1%xpm,mb1%ymm:mb1%ypm,mb1%zmm:mb1%zpm),intent(in)::ux,uy,uz
  ...
  ! CalcDissipationRate
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine CalcDissipationRate(ux,uy,uz,dissp); implicit none
    real(RK),dimension(mb1%xmm:mb1%xpm,mb1%ymm:mb1%ypm,mb1%zmm:mb1%zpm),intent(in)::ux,uy,uz
  ...
  ! CubicRoot
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine CubicRoot(b,c,d,Root,AllRealFlag); implicit none
    ...
    elseif(t<SMALL) then
      ...
      Root(3)= (0.5_RK*sqrt(3.0_RK))*(eta-rho)
    endif
