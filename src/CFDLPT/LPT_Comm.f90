...
contains
  ...
  function PC_ISInThisProc(this,buf_recv,m,dir) result(res)
    ...
    call LPTLogInfo%OutInfo("   The particle id   is :"//trim(num2str( int(buf_recv(m)  +0.2))),3)
    call LPTLogInfo%OutInfo("       particle type is :"//trim(num2str( int(buf_recv(m+1)+0.2))),3)
