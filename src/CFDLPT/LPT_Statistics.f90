...
!********************************************************************************
  ...
  subroutine ResetStatVar()
    ...
    ufsum=zero;      vfsum=zero;      wfsum=zero
    upupsum=zero;    vpvpsum=zero;    wpwpsum=zero;    upvpsum=zero;
    ufufsum=zero;    vfvfsum=zero;    wfwfsum=zero;    ufvfsum=zero;
        ...
        if(ierror /= 0) then
          ...
          do jc=1,nslab
            if(npsslot(itype,jc)==0) then
              write(nUnit,'(20ES24.15)')(ypForPs(jc)+ypForPs(jc+1))*half,   & ! 1
