...
contains
  ...
  subroutine G_InitAllocate(this)
    ...
    pmin = LPT_opt%SimDomain_min - LenExp * one_r3
    pmax = LPT_opt%SimDomain_max + LenExp * one_r3
    pmin_local =real3(LPT_decomp%xSt,LPT_decomp%ySt,LPT_decomp%zSt)-LenExp*one_r3
    pmax_local =real3(LPT_decomp%xEd,LPT_decomp%yEd,LPT_decomp%zEd)+LenExp*one_r3
  ...
  subroutine G_MakeGeometry(this)
    ...
    p01= real3( zero,  zero,  zero)
    p02= real3( zero,  zero,   zlz)
    p03= real3(  xlx,  zero,   zlz)
    p04= real3(  xlx,  zero,  zero)
    call this%add_PlaneWall( p01, p02, p03, p04, 1, 1, infinite=.true. )
    p01= real3( zero,  yly,  zero)
    p02= real3(  xlx,  yly,  zero)
    p03= real3(  xlx,  yly,   zlz)
    p04= real3( zero,  yly,   zlz)
    call this%add_PlaneWall( p01, p02, p03, p04, 1, 1, infinite=.true. )
