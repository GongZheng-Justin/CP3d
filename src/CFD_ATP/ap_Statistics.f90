...
module ap_Statistics
  ...
  use f2_Parameters,only: ivstats,saveStat,nyc,itime,yly,ilast 
#else
  use f4_MeshAndMetries,only:yp
  use f4_Parameters,only: ivstats,saveStat,nyc,itime,yly,ilast
#endif
  use ap_Property
  use ap_Decomp_2d
  use ap_Variables
...
!^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine InitATPStatistics(chFile); implicit none
    ...
    if(nrank/=0) return 
    filename = strip(ATP_Opt%ResultsDir) // 'shannon' // int2str(ilast,10)
    open(newunit=iUnit,file=filename,status='replace',form='formatted',IOSTAT=ierr)
    if(ierr /= 0) call ATPLogInfo%CheckForError(ErrT_Abort,"InitATPStatistics","Cannot open file: "//filename)
    close(iUnit,IOSTAT=ierr)
    do j=1,npType
      filename = strip(ATP_Opt%ResultsDir)//'MeanSquare'//int2str(ilast,10)//'_set'//int2str(j,4)
      open(newunit=iUnit,file=filename,status='replace',form='formatted',IOSTAT=ierr)
      if(ierr /= 0) call ATPLogInfo%CheckForError(ErrT_Abort,"InitATPStatistics","Cannot open file: "//filename)
      close(iUnit,IOSTAT=ierr)    
    enddo
  ...
  subroutine ClcATPStatistics(); implicit none
    ...
    integer::pid,nlocal,itype,js,je,jc,ierr,iUnit,nBin
    ...
    do jc=1,nslab
      ...
      enddo
    enddo
    call MPI_REDUCE(npsslot,npsslotR, npType*nShannon,int_type,MPI_SUM,0,MPI_COMM_WORLD,ierr)
    ...
    IF(nrank==0) THEN
      ...
      if(ierr /= 0) then
        ...
        write(iUnit,'(I10,10ES24.15)')itime,shannon_entropy
      endif
      close(iUnit,IOSTAT=ierr)
    ENDIF
    ...
    block
      ...
      enddo
      call MPI_REDUCE(SumDist,SumDistR, npType*6,real_type,MPI_SUM,0,MPI_COMM_WORLD,ierr)
      IF(nrank==0) THEN
        DO itype=1,npType
          nBin=ATPProperty%nPrtcl_in_Bin(itype)
          do jc=1,6
          ...
          enddo
          filename = strip(ATP_Opt%ResultsDir)//'MeanSquare'//int2str(ilast,10)//'_set'//int2str(itype,4)
          open(newunit=iUnit,file=filename,status='old',position='append',form='formatted',IOSTAT=ierr)
          if(ierr /= 0) then
            call ATPLogInfo%CheckForError(ErrT_Pass,"ClcATPStatistics","Cannot open file: "//filename)
          else  
            write(iUnit,'(I10,6ES24.15)')itime,SumDistR(:,itype)
          endif
          close(iUnit,IOSTAT=ierr)        
        ENDDO
      ENDIF      
    ...
    call MPI_REDUCE(sumStat,sumStatR,14*npType*nslab, real_type,MPI_SUM,0,MPI_COMM_WORLD,ierr)
    ...
    if(nrank==0) then
      ...
      do itype=1,npType
        ...
        if(ierr /= 0) then
          ...
          write(iUnit,*)
          write(iUnit,'(A)')'  yp, np, up, vp, wp, upup, vpvp, wpwp, upvp, uf, vf, wf, ufuf, vfvf, wfwf, ufvf' 
          do jc=1,nslab
            if(npsslot(itype,jc)==0) then
              write(iUnit,'(20ES24.15)')(ypForPs(jc)+ypForPs(jc+1))*0.5_RK,   & ! 1
