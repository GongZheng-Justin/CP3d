...
module ap_Fpforce
  ...
  integer:: xmp_interp, xep_interp, xmc_interp, xec_interp ! index constraints for interpolation
  integer:: ymp_interp, yep_interp, ymc_interp, yec_interp ! index constraints for interpolation
  integer:: zmp_interp, zep_interp, zmc_interp, zec_interp ! index constraints for interpolation
...
contains
  ...
  ! InitFpForce
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine InitFpForce(); implicit none
    ...
    ymp_interp=0;          if(.not. ATP_Opt%IsPeriodic(2)) ymp_interp=1
    yep_interp=nyc;        if(.not. ATP_Opt%IsPeriodic(2)) yep_interp=nyc -1     ! Modified by Zheng Gong,2021-09-23
    ymc_interp=0;          if(.not. ATP_Opt%IsPeriodic(2)) ymc_interp=1
    yec_interp=nyc-1;      if(.not. ATP_Opt%IsPeriodic(2)) yec_interp=nyc -2   
  ...
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine PrepareInterpolation(); implicit none
    ...
    DO pid=1,nlocal
      ...
      idyc_interp=max(ymc_interp,jc-1)          ! Modified by Zheng Gong,2024-06-06
      idyc_interp=min(yec_interp,idyc_interp)   ! Modified by Zheng Gong,2024-06-06
      if(pos%y>yc(jc)) then
        idyp_interp=min(jc,  yep_interp)
      else
        idyp_interp=max(jc-1,ymp_interp)
      endif
  ...
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine clc_FluidInterpolation(ux,uy,uz); implicit none
    ...
    real(RK)::RatioSwim
#ifdef AddFallingVelocity
#ifdef LinearChangeSwimVelocity 
    real(RK)::RatioYt, RyChange
#else
    real(RK)::DistToSurface, Radius
#endif
#endif
    ...
    if(ATP_Opt%IsPeriodic(2)) then
      jc = nyc+2
      do kc=mb1%zmm,mb1%zpm
        do ic=mb1%xmm,mb1%xpm
          uy(ic,jc,kc)=uy(ic,2,kc)
        enddo
      enddo
    endif
#endif      
    ...
    DO pid=1,nlocal
...
#ifdef AddFallingVelocity
      SwimDir=real3(0.0_RK,1.0_RK,0.0_RK) ! Attention here. 2024-03-10
#endif      
      ...
      RatioSwim = 1.0_RK
#ifdef AddFallingVelocity
#define LinearChangeSwimVelocity
#ifdef LinearChangeSwimVelocity      
      RyChange = 0.8_RK
      RatioYt =  pos%y/yly
      if(RatioYt >=1.0_RK) then
        RatioSwim=0.0_RK
      elseif(RatioYt > RyChange) then
        RatioSwim = (1.0_RK -RatioYt)/(1.0_RK -RyChange)
      endif
#else
      Radius =  0.5_RK*ATPProperty%Prtcl_PureProp(itype)%Diameter
      DistToSurface = yly-pos%y -Radius
      if(DistToSurface > 0.0_RK) then
        DistToSurface = Radius/DistToSurface
        DistToSurface = max(DistToSurface, 0.2820_RK)
        DistToSurface = min(DistToSurface, 1.0E+3_RK)
        RatioSwim = 1.0_RK/(DistToSurface +0.2_RK*log(DistToSurface) +0.9713_RK)
      else
        RatioSwim = 0.0_RK
      endif
#endif
#endif
      GPrtcl_linVel(1,pid) =real3(SumXDir,SumYDir,SumZDir)+ SwimVelocityMag*SwimDir*RatioSwim
