...
module ap_Integration
  ...
  logical::AddRandomDiffusion=.false.
...
contains
!#define AddFallingVelocity
#define PreferredDirection
  
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  ! Prtcl_Integrate
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90  
  subroutine Prtcl_Integrate(iCountATP); implicit none
    ...
    if(ATP_Opt%PI_Method==PIM_AB2) then
      DO pid=1,nlocal
        ...
        GPrtcl_SwimDir(pid)=(1.0_RK/normValue)*SwimOld
      ENDDO
    ...
    if(AddRandomDiffusion) then
      allocate(randTmp1(nlocal))
      call normrnd(randTmp1)
      DiffR=1.0E-6_RK
      DO pid=1,nlocal
        GPrtcl_PosR(pid)%y =GPrtcl_PosR(pid)%y + sqrt(2.0_RK*dt*DiffR)*randTmp1(pid)
      Enddo
      deallocate(randTmp1)
    endif
#else
    ...
    if(nlocal<1) return
    allocate(randTmp1(nlocal),randTmp2(nlocal))
    call random_number(randTmp1)
    call random_number(randTmp2)
    DO pid=1,nlocal
      Theta=pi*randTmp1(pid)
      Phi=twopi*randTmp2(pid)
      real3T%x=sin(Theta)*cos(Phi)
      real3T%y=sin(Theta)*sin(Phi)
      real3T%z=cos(Theta)
      normValue=norm(real3T)
#ifdef AddFallingVelocity
      GPrtcl_SwimDir(pid)=(1.0_RK/normValue)*real3T +real3(0.0_RK,-1.0_RK,0.0_RK)
#else
      GPrtcl_SwimDir(pid)=(1.0_RK/normValue)*real3T   
#endif
    ENDDO
    if(AddRandomDiffusion) then
      call normrnd(randTmp1)
  ...
  ! generateCoord
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine generateCoord(exPrime,eyPrime,ezPrime); implicit none
  ...
  ! normrnd
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine normrnd(x); implicit none
