...
module ap_Variables
  ...
  use f2_Parameters,only:xlx,yly,zlz
#else
  use f4_Parameters,only:xlx,yly,zlz
#endif
  use ap_Property
  use ap_decomp_2d
  use ap_Parameters
...
contains
  ...
  subroutine GL_AllocateAllVar(this); implicit none
    ...
    ! locals
    type(real3)::SimLen
    integer::mlocal,numPrtcl,ierrTmp,ierr
    ...
    allocate(GPrtcl_SwimAcc(this%tsize,mlocal),Stat=ierrTmp);ierr=ierr+abs(ierrTmp)
    allocate(GPrtcl_PosOld(mlocal),            Stat=ierrTmp);ierr=ierr+abs(ierrTmp)
    allocate(GPrtcl_SwimDir(mlocal),           Stat=ierrTmp);ierr=ierr+abs(ierrTmp)
    allocate(GPrtcl_MoveDistance(mlocal),       Stat=ierrTmp);ierr=ierr+abs(ierrTmp)
    if(ierr/=0) call ATPLogInfo%CheckForError(ErrT_Abort,"GL_AllocateAllVar","Allocation failed 1")   
  ...
  ! GL_MakingAllPrtcl
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine GL_MakingAllPrtcl(this); implicit none
    class(VarList)::this
        
    ! locals
    type(real3)::cntr
    real(RK)::xst,xed,yst,yed,zst,zed,realt(3),MagTmp,randbox
    integer::i,j,k,iTV(8),numPrtcl,ierr,nlocal,nlocal_sum,nPrtlBin
    ...
    nPrtlBin=ATPProperty%nPrtcl_in_Bin(1)
    if(nrank==0) then
      do j=2,ATP_opt%numPrtcl_Type
        if(ATPProperty%nPrtcl_in_Bin(j)/=ATPProperty%nPrtcl_in_Bin(1)) then     
          call ATPLogInfo%CheckForError(ErrT_Abort,"MakingAllPrtcl: "," nPrtcl_in_Bin wrong" )        
        endif
      enddo
    endif
    
    nlocal=0; k=0
    randbox=0.00_RK*yly ! 0.02_RK*yly
    do i=1,nPrtlBin 
      call random_number(realt)
      realt=2.0_RK*realt-1.0_RK
      cntr%x= 0.5_RK*xlx +randbox*realt(1)
      cntr%y= 0.5_RK*yly +randbox*realt(2)
      cntr%z= 0.5_RK*zlz +randbox*realt(3)
      ...
      if(cntr%x>=xst.and.cntr%x<xed.and.cntr%y>=yst.and.cntr%y<yed.and.cntr%z>=zst.and.cntr%z<zed) then
        do j=1,ATP_opt%numPrtcl_Type
          ...
          nlocal=nlocal+1
          GPrtcl_id(nlocal) = (i-1)*ATP_opt%numPrtcl_Type+j
          GPrtcl_pType(nlocal)= j
  ...
  ! copy i2 to i1
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine GL_copy(i1,i2); implicit none
