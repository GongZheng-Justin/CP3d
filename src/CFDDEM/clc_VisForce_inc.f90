...
subroutine clc_VisForce(ux,uy,uz,RatioYp_interp,RatioYc_interp)
  ...
  IF(myProcNghBC(y_pencil,4)<0) THEN  !xm_dir
    ...
    DO kc=y1start(3),y1end(3)
      ...
      do jc=y1start(2),y1end(2)
        ...
        d11q1= (ux(3,jc,kc)-two*ux(2,jc,kc)+ux(1,jc,kc))*rdx2
        d22q1= ap2c(jc)*ux(ic,jp,kc) + ac2c(jc)*ux(ic,jc,kc)+ am2c(jc)*ux(ic,jm,kc)                
        d33q1= ap3c(kc)*ux(ic,jc,kp) + ac3c(kc)*ux(ic,jc,kc)+ am3c(kc)*ux(ic,jc,km)
        visx(ic,jc,kc)=d11q1+d22q1+d33q1
      enddo
  ...
  IF(myProcNghBC(y_pencil,3)<0) THEN  !xp_dir
    ...
    DO kc=y1start(3),y1end(3)
      ...
      do jc=y1start(2),y1end(2)
        ...
        d11q1= (ux(nxp,jc,kc)-two*ux(nxc,jc,kc)+ux(nxc-1,jc,kc))*rdx2
        d22q1= ap2c(jc)*ux(ic,jp,kc) + ac2c(jc)*ux(ic,jc,kc)+ am2c(jc)*ux(ic,jm,kc)                
        d33q1= ap3c(kc)*ux(ic,jc,kp) + ac3c(kc)*ux(ic,jc,kc)+ am3c(kc)*ux(ic,jc,km)
        visx(ic,jc,kc)=d11q1+d22q1+d33q1
      enddo
  ...
  ! Here I set zero gradient Bcs for visx TERM for all the non-periodic directions !!!
  ! yp-dir and ym_dir
  do kc=y1start(3),y1end(3)
  ...
  call myupdate_halo(visx, mb1, hi_visx_interp) 
  ...
  DO kc=y1start(3),y1end(3)
    ...
    do ic=y1start(1),y1end(1)
      im=ic-1; ip=ic+1  
      d11q2= ap1c(ic)*uy(ip,jc,kc)+ac1c(ic)*uy(ic,jc,kc)+am1c(ic)*uy(im,jc,kc)
      d22q2= two*rdyp(2)*( (uy(ic,3,kc)-uy(ic,1,kc))/(yp(3)-yp(1))           &
  ...
  DO kc=y1start(3),y1end(3)
    ...
    do ic=y1start(1),y1end(1)
      im=ic-1; ip=ic+1  
      d11q2= ap1c(ic)*uy(ip,jc,kc)+ac1c(ic)*uy(ic,jc,kc)+am1c(ic)*uy(im,jc,kc)
      d22q2= two*rdyp(nyc-1)*( (uy(ic,nyp,kc)-uy(ic,nyc,  kc))*rdyp(nyc)     &
  ...
  ! Here I set zero gradient Bcs for visy TERM for all the non-periodic directions !!!
  ! xp-dir and xm_dir
  ...
  !!! update Halo here !!!
  call myupdate_halo(visy, mb1, hi1)
  ...
  IF(myProcNghBC(y_pencil,2)<0) THEN  !zm_dir
    ...
    do jc=y1start(2),y1end(2)
      ...
      do ic=y1start(1),y1end(1)
        ...
        d11q3= ap1c(ic)*uz(ip,jc,kc)+ac1c(ic)*uz(ic,jc,kc)+am1c(ic)*uz(im,jc,kc)
        d22q3= ap2c(jc)*uz(ic,jp,kc)+ac2c(jc)*uz(ic,jc,kc)+am2c(jc)*uz(ic,jm,kc)                
        d33q3= (uz(ic,jc,3)-two*uz(ic,jc,2)+uz(ic,jc,1))*rdz2
        visz(ic,jc,kc)=d11q3+d22q3+d33q3
      enddo
    enddo
  ELSE
  ...
  IF(myProcNghBC(y_pencil,1)<0) THEN  !zp_dir
    ...
    do jc=y1start(2),y1end(2)
      ...
      do ic=y1start(1),y1end(1)
        ...
        d11q3= ap1c(ic)*uz(ip,jc,kc)+ac1c(ic)*uz(ic,jc,kc)+am1c(ic)*uz(im,jc,kc)
        d22q3= ap2c(jc)*uz(ic,jp,kc)+ac2c(jc)*uz(ic,jc,kc)+am2c(jc)*uz(ic,jm,kc)                
        d33q3= (uz(ic,jc,nzp)-two*uz(ic,jc,nzc)+uz(ic,jc,nzc-1))*rdz2
        visz(ic,jc,kc)=d11q3+d22q3+d33q3
      enddo
    enddo
  ENDIF

  ! Here I set zero gradient Bcs for visz TERM for all the non-periodic directions !!!
  ! yp-dir and ym_dir
  do kc=y1start(3),y1end(3)
    do ic=y1start(1),y1end(1)
  ...
  call myupdate_halo(visz, mb1, hi_visz_interp)
