...
#include "clc_FluidAccForce_inc.f90"
  ...
  subroutine InitDistribute()
    ...
    distwx = ceiling(1.02_RK*RatioSR*maxR*rdx -half); distwx = max(distwx,1)
    distwz = ceiling(1.02_RK*RatioSR*maxR*rdz -half); distwz = max(distwz,1)
    ...
    call myallocate(FpForce_x, mb_dist, opt_global=.true.);  FpForce_x=zero
    call myallocate(FpForce_y, mb_dist, opt_global=.true.);  FpForce_y=zero
    call myallocate(FpForce_z, mb_dist, opt_global=.true.);  FpForce_z=zero
    ...
    if(mlocalFix>0) then
      ...
      GPFix_FpForce=zero_r3;   GPFix_VolFluid=zero;     GPFix_nFluidCell=0
    endif
        ...
        do k= kc-1,kc+1
          ...
          do j=jc-1,jc+1
            ...
            do i=ic-1,ic+1
              ...
              volt  = volt + VolCell(j)/(sixteen**dist2)
            enddo
          enddo
        enddo
      ENDIF
...
#define DragType1
    ...
    DragCoe=three*pi*FluidDensity*xnu
    DO pid=1,nlocal
      pos = GPrtcl_posR(pid)
      diam= pos%w*two 
...
#ifdef DragType1
      cd =(one+0.11037527593819_RK*sqrt(rep))**2                                 ! Ref.[1]
#else
      cd = one+0.150_RK*(rep**0.681_RK)+ 0.0169583333_RK*rep*rep/(rep+8710.0_RK) ! Ref.[2]
#endif
  ...
  subroutine distribute_FpForce()
    ...
    DO pid=1,GPrtcl_list%nlocal
      ...
      massFH = half*DEMProperty%Prtcl_PureProp(itype)%MassOfFluid
        ...
        do k= kc-1,kc+1
          ...
          do j=jc-1,jc+1
            ...
            do i=ic-1,ic+1
              distx = xc(i) - pos%x
              dist2 = (distx*distx+disty2+distz2)/dcell2
              xi =  one/((sixteen**dist2)*volt*FluidDensity)
    ...
    do k=mb_dist%zmm,mb_dist%zpm
      do i=mb_dist%xmm,mb_dist%xpm
        FpForce_x(i,1,k)=FpForce_x(i,1,k)+FpForce_x(i,0,k); FpForce_x(i,0,k)=zero
        FpForce_y(i,1,k)=FpForce_y(i,1,k)+FpForce_y(i,0,k); FpForce_y(i,0,k)=zero
        FpForce_z(i,1,k)=FpForce_z(i,1,k)+FpForce_z(i,0,k); FpForce_z(i,0,k)=zero
        FpForce_x(i,nyc,k)=FpForce_x(i,nyc,k)+FpForce_x(i,nyp,k); FpForce_x(i,nyp,k)=zero
        FpForce_y(i,nyc,k)=FpForce_y(i,nyc,k)+FpForce_y(i,nyp,k); FpForce_y(i,nyp,k)=zero
        FpForce_z(i,nyc,k)=FpForce_z(i,nyc,k)+FpForce_z(i,nyp,k); FpForce_z(i,nyp,k)=zero
      enddo
    enddo
#endif
  ...
  subroutine SetBC_and_UpdateHalo_dist(mat)
    ...
    ! step5: set Bc for mat, Here zero-gradient Bcs for all non-periodic directions
    ! yp-dir, and ym-dir
    ...
    ! step6: update halo
    call myupdate_halo(mat, mb_dist, hi_dist)
