#include "definitions_inc.f90"
  ...
  ! clcRhsX_FIMP
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90    
  subroutine clcRhsX_FIMP(ux,uy,uz,RhsX,HistXold,pressure); implicit none
  ...
  ! clcRhsY_FIMP
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90    
  subroutine clcRhsY_FIMP(ux,uy,uz,RhsY,HistYold,pressure); implicit none
    real(RK),dimension(mb1%xmm:mb1%xpm,mb1%ymm:mb1%ypm,mb1%zmm:mb1%zpm),intent(in)::ux,uy,uz,pressure 
  ...
  ! clcRhsZ_FIMP
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90 
  subroutine clcRhsZ_FIMP(ux,uy,uz,RhsZ,HistZold,pressure); implicit none
    real(RK),dimension(mb1%xmm:mb1%xpm,mb1%ymm:mb1%ypm,mb1%zmm:mb1%zpm),intent(in)::ux,uy,uz,pressure
  ...
  ! clcU1Hat_FIMP
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90    
  subroutine clcU1Hat_FIMP(ux,RhsX); implicit none
    ...
    ! locals
    integer::ic,jc,kc
    real(RK):: rTemp,mic,cic,pic,mjc,cjc,pjc,mkc,ckc,pkc
    ...
    ELSE
      ...
      ENDDO
    ENDIF
  end subroutine clcU1Hat_FIMP

  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  ! clcU2Hat_FIMP
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90    
  subroutine clcU2Hat_FIMP(uy,RhsY); implicit none
    ...
    ! locals
    integer::ic,jc,kc
    real(RK)::rTemp,mic,pic,cic,mjc,pjc,cjc,mkc,pkc,ckc
    real(RK),dimension(x1size(2),x1size(1))::tridmi,tridci,tridpi,tridfi
    real(RK),dimension(z1size(1),z1size(3))::tridmk,tridck,tridpk,tridfk
    real(RK),dimension(y1start(1):y1end(1),y1start(2):y1end(2))::tridmj,tridcj,tridpj,tridfj
    real(RK),dimension(x1size(1),x1size(2),x1size(3))::arrx1
    ...
    ELSE
      ...
      ENDDO
    ENDIF
  end subroutine clcU2Hat_FIMP
  
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  ! clcU3Hat_FIMP
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90    
  subroutine clcU3Hat_FIMP(uz,RhsZ); implicit none
    ...
    ELSE
      ...
      ENDDO
    ENDIF
  end subroutine clcU3Hat_FIMP

  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  ! PressureUpdate_FIMP
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine PressureUpdate_FIMP(pressure, prphiHalo); implicit none
    ...
    DO kc=y1start(3),y1end(3)
      do jc=y1start(2),y1end(2)
        do ic=y1start(1),y1end(1)
          pressure(ic,jc,kc)= pressure(ic,jc,kc)+ prphiHalo(ic,jc,kc)/pmAlpha
        enddo
      enddo
    ENDDO
