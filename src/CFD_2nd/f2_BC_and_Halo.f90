...
module f2_BC_and_Halo
  use MPI
  use mc_TypeDef
  use mc_Decomp2d
  use f2_Parameters
  use f2_MeshAndMetries
  use f2_Variables,only: mb1,hi1,OutFlowInfoX,OutFlowInfoY
  implicit none
  private
...
contains
  ...
  subroutine clcOutFlowVelocity(ux,uy,uz); implicit none
    ...
    if(BcOption(yp_dir)==BC_OutFlow) then
      ...
      do kc=y1start(3),y1end(3)
        ...
        enddo
      enddo
      call MPI_ALLREDUCE(ConVectVel,rTemp,1,real_type,MPI_SUM,MPI_COMM_WORLD,ierr)
    ...
    if(myProcNghBC(y_pencil,3)==BC_outFlow) then  
      ...
      do kc=y1start(3),y1end(3)
        ...
        enddo
      enddo
      call MPI_ALLREDUCE(ConVectVel,rTemp,1,real_type,MPI_SUM,DECOMP_2D_COMM_ROW,ierr)
  ...
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine correctOutFlowFaceVelocity(ux,uy,uz); implicit none
    real(RK),dimension(mb1%xmm:mb1%xpm,mb1%ymm:mb1%ypm,mb1%zmm:mb1%zpm),intent(inout)::ux,uy,uz
    ...
    call MPI_ALLREDUCE(CorrectVel,rTemp,1,real_type,MPI_SUM,MPI_COMM_WORLD,ierr)
  ...
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90   
  subroutine SetBC_and_UpdateHalo(ux,uy,uz); implicit none
    real(RK),dimension(mb1%xmm:mb1%xpm,mb1%ymm:mb1%ypm,mb1%zmm:mb1%zpm),intent(inout)::ux,uy,uz
  ...
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90   
  subroutine SetBC_and_UpdateHaloForPrSrc(ux,uy,uz); implicit none
    real(RK),dimension(mb1%xmm:mb1%xpm,mb1%ymm:mb1%ypm,mb1%zmm:mb1%zpm),intent(inout)::ux,uy,uz
