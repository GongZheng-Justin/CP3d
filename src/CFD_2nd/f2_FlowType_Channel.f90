...
#endif
#ifdef CFDACM
  use f2_Variables,only:FluidIndicator
#endif
  implicit none
  private
  include "myfftw3.f03"
...
#undef  EnergySpectra_staggered_2nd
  ...
  ! InitVelocity_CH
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine InitVelocity_CH(ux,uy,uz,Deviation,chFile); implicit none
    ...
    ! locals
    integer::ierr,ic,jc,kc,m1,m2,iUnit,iTV(8)
  ...
  ! InitStatVar_CH
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine InitStatVar_CH(chFile); implicit none
    character(len=*),intent(in)::chFile
    ...
    open(newunit=iUnit, file=chFile, status='old',form='formatted',IOSTAT=ierr )
    if(ierr/=0 .and. nrank==0) then
      call MainLog%CheckForError(ErrT_Abort,"InitStatVar_CH","Cannot open file: "//strip(chFile))
    endif
    read(iUnit,nml=uBulk_Param)
    rewind(iUnit)
    ...
    if(nrank==0) then
      write(MainLog%iUnit, nml=uBulk_Param)
  ...
  ! clcStat_CH
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine clcStat_CH(ux,uy,uz,pr,ArrTemp1,ArrTemp2); implicit none
    ...
    integer::ic,jc,kc,im,jm,km,ip,jp,kp,it,jt,kt,ierr,ids,nrankX,nrankZ,iUnit
...
#undef  EnergySpectra_staggered_2nd
    ...
    if(nrank==0) then
      ...
      IF(ierr/=0) THEN
        call MainLog%CheckForError(ErrT_Pass,"clcStat_CH","Cannot open file: "//filename)
      ELSE
        ...
        if(IsUxConst) then
          ...
          write(iUnit,'(A)')'  Variable velocity in x-dir while adding a constant body force.'
        endif
        write(iUnit,'(A, 2ES24.15)')'    time averaged pressure gradient is: ', PrGradsum*infstime, xnu*dudy/yly
        write(iUnit,'(A)')'  '
...
#undef nGradStat
#endif
end module f2_FlowType_Channel
