  ...
  subroutine  clcRhsX_PIMP(ux,uy,uz,RhsX,HistXold,pressure)
    ...
    DO kc=y1start(3),y1end(3)
      ...
      do jc=y1start(2),y1end(2)
        ...
        if(yc(jc)<ybulk1 .or. yc(jc)>=ybulk2) s1totCoe=0.0_RK
...
#ifdef CFDDEM
          convEd1= -h11-h12-h13 + xnu*dcq13 + gravity(1)+0.5_RK*(FpForce_x(ic,jc,kc)+FpForce_x(im,jc,kc))
#elif CFDLPT_TwoWay
    ...
    DO kc=y1start(3),y1end(3)
      ...
      do jc=y1start(2),y1end(2)
        ...
        do ic=y1start(1),y1end(1)
          ...
          visa=0.25_RK*(nut(ic,jc,kc)+nut(im,jc,kc)+nut(ic,jm,kc)+nut(im,jm,kc))
          visb=0.25_RK*(nut(ic,jc,kc)+nut(im,jc,kc)+nut(ic,jp,kc)+nut(im,jp,kc))
          visc=0.25_RK*(nut(ic,jc,kc)+nut(im,jc,kc)+nut(ic,jc,km)+nut(im,jc,km))
          visd=0.25_RK*(nut(ic,jc,kc)+nut(im,jc,kc)+nut(ic,jc,kp)+nut(im,jc,kp))
          ...
          s1tot=s1tot+ux(ic,jc,kc)*s1totCoe        
#ifdef CFDDEM
          convEd1= -h11-h12-h13 +sgs1+sgs2+sgs3+dcq13+gravity(1)+0.5_RK*(FpForce_x(ic,jc,kc)+FpForce_x(im,jc,kc))
#elif CFDLPT_TwoWay
          convEd1= -h11-h12-h13 +sgs1+sgs2+sgs3+dcq13+gravity(1)+FpForce_x(ic,jc,kc)
#else
          convEd1= -h11-h12-h13 +sgs1+sgs2+sgs3+dcq13+gravity(1)
#endif
          RhsX(ic,jc,kc)=pmGamma*convEd1+ pmTheta*HistXold(ic,jc,kc)- pmAlpha*gradp1+ 2.0_RK*pmBetaT*d22q1
          HistXold(ic,jc,kc)=convEd1
        enddo
      enddo
    ENDDO
    ...
    DO kc=y1start(3),y1end(3)
      ...
      do jc=y1start(2),y1end(2)
        ...
        do ic=y1start(1),y1end(1)
          ...
          visa=0.25_RK*(nut(ic,jc,kc)+nut(im,jc,kc)+nut(ic,jm,kc)+nut(im,jm,kc))
          visb=0.25_RK*(nut(ic,jc,kc)+nut(ip,jc,kc)+nut(ic,jm,kc)+nut(ip,jm,kc))
          visc=0.25_RK*(nut(ic,jc,kc)+nut(ic,jm,kc)+nut(ic,jc,km)+nut(ic,jm,km))
          visd=0.25_RK*(nut(ic,jc,kc)+nut(ic,jm,kc)+nut(ic,jc,kp)+nut(ic,jm,kp))
          ...
          gradp2= (pressure(ic,jc,kc)-pressure(ic,jm,kc))*sucac
#ifdef CFDDEM
          convEd2= -h21-h22-h23+sgs1+sgs2+sgs3+dcq13+ gravity(2)+0.5_RK*(FpForce_y(ic,jc,kc)+FpForce_y(ic,jm,kc))
#elif CFDLPT_TwoWay
          convEd2= -h21-h22-h23+sgs1+sgs2+sgs3+dcq13+ gravity(2)+FpForce_y(ic,jc,kc)
#else
          convEd2= -h21-h22-h23+sgs1+sgs2+sgs3+dcq13+ gravity(2)
#endif
          RhsY(ic,jc,kc)=pmGamma*convEd2+ pmTheta*HistYold(ic,jc,kc)- pmAlpha*gradp2+ 2.0_RK*pmBetaT*d22q2
          HistYold(ic,jc,kc)=convEd2   
        enddo
      enddo
    ENDDO
    ...
    DO kc=y1start(3),y1end(3)
      ...
      do jc=y1start(2),y1end(2)
        ...
        do ic=y1start(1),y1end(1)
          ...
          visa=0.25_RK*(nut(ic,jc,km)+nut(im,jc,km)+nut(ic,jc,kc)+nut(im,jc,kc))
          visb=0.25_RK*(nut(ic,jc,km)+nut(ip,jc,km)+nut(ic,jc,kc)+nut(ip,jc,kc))
          visc=0.25_RK*(nut(ic,jc,km)+nut(ic,jm,km)+nut(ic,jm,kc)+nut(ic,jc,kc))
          visd=0.25_RK*(nut(ic,jc,km)+nut(ic,jp,km)+nut(ic,jp,kc)+nut(ic,jc,kc))
          ...
          gradp3= (pressure(ic,jc,kc)-pressure(ic,jc,km))*rdz 
#ifdef CFDDEM
          convEd3=-h31-h32-h33+sgs1+sgs2+sgs3+dcq13+gravity(3)+0.5_RK*(FpForce_z(ic,jc,kc)+FpForce_z(ic,jc,km))
#elif CFDLPT_TwoWay
          convEd3=-h31-h32-h33+sgs1+sgs2+sgs3+dcq13+gravity(3)+FpForce_z(ic,jc,kc)
#else
          convEd3=-h31-h32-h33+sgs1+sgs2+sgs3+dcq13+gravity(3)
#endif   
          RhsZ(ic,jc,kc)= pmGamma*convEd3+ pmTheta*HistZold(ic,jc,kc)- pmAlpha*gradp3+ 2.0_RK*pmBetaT*d22q3
          HistZold(ic,jc,kc)=convEd3
        enddo
      enddo
    ENDDO 
  ...
  subroutine clcU1Hat_PIMP_LES(ux,RhsX)
    ...
    DO kc=y1start(3),y1end(3)  
      do jc=y1start(2),y1end(2)
        ...
        do ic=y1start(1),y1end(1)
          im=ic-1
          visc= 0.25_RK*(nut(ic,jc,kc)+nut(im,jc,kc)+nut(ic,jm,kc)+nut(im,jm,kc))
          visd= 0.25_RK*(nut(ic,jc,kc)+nut(im,jc,kc)+nut(ic,jp,kc)+nut(im,jp,kc))
          rt1 = pmBetaT*visc*am2cForCN(jc)
          rt2 = pmBetaT*visd*ap2cForCN(jc)
          tridmj(ic,jc) = -rt1 
          tridcj(ic,jc) =  rt1+rt2+1.0_RK
          tridpj(ic,jc) = -rt2
          tridfj(ic,jc) =  RhsX(ic,jc,kc)
        enddo
      enddo  
  ...
  subroutine clcU3Hat_PIMP_LES(uz,RhsZ)
    ...
    DO kc=y1start(3),y1end(3)
      ...
      do jc=y1start(2),y1end(2)
        ...
        jp=jc+1
        do ic=y1start(1),y1end(1)
          visc=0.25_RK*(nut(ic,jc,km)+nut(ic,jm,km)+nut(ic,jc,kc)+nut(ic,jm,kc))
          visd=0.25_RK*(nut(ic,jc,km)+nut(ic,jp,km)+nut(ic,jc,kc)+nut(ic,jp,kc))
          rt1= pmBetaT*visc*am2cForCN(jc)
          rt2= pmBetaT*visd*ap2cForCN(jc)
          tridmj(ic,jc) = -rt1
          tridcj(ic,jc) =  rt1+rt2+1.0_RK
  ...
  subroutine PressureUpdate_PIMP_LES(pressure, prphiHalo)
    ...
    DO kc=y1start(3),y1end(3)
      do jc=y1start(2),y1end(2)
        ...
        do ic=y1start(1),y1end(1)
          visd= 0.5_RK*(nut(ic,jp,kc)+nut(ic,jc,kc))
          visc= 0.5_RK*(nut(ic,jc,kc)+nut(ic,jm,kc))
          pmBetap= -pmBetaT*metr2*visd
          pmBetam= -pmBetaT*metr1*visc
          pmBetac= -(pmBetap + pmBetam) +1.0_RK
          pressure(ic,jc,kc)= pressure(ic,jc,kc)+ pmBetap*prphiHalo(ic,jp,kc)+ pmBetac*prphiHalo(ic,jc,kc)+ &
                                                  pmBetam*prphiHalo(ic,jm,kc)
        enddo
      enddo
