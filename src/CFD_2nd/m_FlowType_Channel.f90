...
#define NCHASTAT 35
  ...
  subroutine InitVelocity_CH(ux,uy,uz,Deviation,chFile)
    ...
    real(RK)::ubulk,ubulkTemp,ybulk1,ybulk2,twopi
    ...
    DO jc=1,nyc
      ...
      do kc=y1start(3),y1end(3)
        ...
        do ic=y1start(1),y1end(1)
          ...
          SumVec(31)=SumVec(31)+ dudxC*(dvdxU+dvdx)*0.5_RK+ (dudyU+dudy)*(dvdyM+dvdy)*0.25_RK
          ...
          SumVec(42)=SumVec(42)+ 0.5_RK*(FpForce_y(ic,jc,kc)+FpForce_y(ic,jp,kc))*uxCell
          
          uyCellm=(uy(im,jc,kc)+ uy(im,jp,kc))*0.5_RK
          SumVec(42)=SumVec(42)+ FpForce_x(ic,jc,kc)*(uyCellm +uyCell)*0.5_RK
#endif
        enddo
      enddo
      do kc=1,NCHASTAT
