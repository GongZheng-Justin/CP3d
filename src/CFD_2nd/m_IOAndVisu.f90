module m_IOAndVisu
  ...
  integer:: Prev_BackUp_itime  = huge(1)
...
contains
  ...
  ! InitVisu
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine InitVisu(chFile); implicit none
    ...
    if(ierr/=0 .and. nrank==0) call MainLog%CheckForError(ErrT_Abort,"InitVisu", "Cannot openfile: "//chFile)
    read(iUnit, nml=IO_Options)
    close(iUnit,IOSTAT=ierr)
    if(nrank==0) write(MainLog%iUnit,nml=IO_Options)
  ...
  subroutine Delete_Prev_Restart(ntime); implicit none
    ...
    if((.not. delete_prev_restart_flag) .or. nrank /= 0) return
    ! 
    ...
    ! 
    chFile = strip(RestartDir_)//"OutFlowFor"//strip(RunName_)// int2str(Prev_BackUp_itime,10)
    open(newunit=iUnit,file=chFile,IOSTAT=ierr)
    close(unit=iUnit,status='delete',IOSTAT=ierr)
    !
  ...
  subroutine write_restart(ntime,ux,uy,uz,pressure,HistXOld,HistYOld,HistZOld); implicit none
    ...
    character(len=:),allocatable::chFile
    integer::fh,ierr,ic,jc,kc,newtype,sizes(3),subsizes(3),starts(3)
    ...
    if(BcOption(yp_dir)==BC_OutFlow) then
      ...
      call MPI_FILE_OPEN(MPI_COMM_WORLD, chFile, MPI_MODE_WRONLY, MPI_INFO_NULL, fh, ierr)
      ...
      call MPI_FILE_WRITE_ALL(fh,OutFlowInfoY(4:6,:,:),subsizes(1)*subsizes(2)*subsizes(3),real_type,MPI_STATUS_IGNORE,ierr)
      disp=disp+ int(sizes(1),8)*int(sizes(2),8)*int(sizes(3),8)*int(mytype_bytes,8)
      if(ischeme==FI_AB2 .and. WriteHistOld) then
        call MPI_FILE_WRITE_ALL(fh,OutFlowInfoY(1:3,:,:),subsizes(1)*subsizes(2)*subsizes(3),real_type,MPI_STATUS_IGNORE,ierr)
        disp=disp+ int(sizes(1),8)*int(sizes(2),8)*int(sizes(3),8)*int(mytype_bytes,8)
      endif
    ...
    if(myProcNghBC(y_pencil,3)==BC_OutFlow) then
      ...
      call MPI_FILE_OPEN(DECOMP_2D_COMM_ROW, chFile, MPI_MODE_WRONLY, MPI_INFO_NULL, fh, ierr)
      call MPI_TYPE_CREATE_SUBARRAY(3,sizes,subsizes,starts,MPI_ORDER_FORTRAN,real_type,newtype,ierr)
      call MPI_TYPE_COMMIT(newtype,ierr)
      call MPI_FILE_SET_VIEW(fh,disp,real_type,newtype,'native',MPI_INFO_NULL,ierr)
      call MPI_FILE_WRITE_ALL(fh,OutFlowInfoX(4:6,:,:),subsizes(1)*subsizes(2)*subsizes(3),real_type,MPI_STATUS_IGNORE,ierr)
      disp=disp+ int(sizes(1),8)*int(sizes(2),8)*int(sizes(3),8)*int(mytype_bytes,8)
      if(ischeme==FI_AB2 .and. WriteHistOld) then
        call MPI_FILE_WRITE_ALL(fh,OutFlowInfoX(1:3,:,:),subsizes(1)*subsizes(2)*subsizes(3),real_type,MPI_STATUS_IGNORE,ierr)
        disp=disp+ int(sizes(1),8)*int(sizes(2),8)*int(sizes(3),8)*int(mytype_bytes,8)    
      endif
      call MPI_TYPE_FREE(newtype,ierr)
      call MPI_FILE_CLOSE(fh,ierr)
  ...
  subroutine read_restart(ux,uy,uz,pressure,HistXOld,HistYOld,HistZOld); implicit none
    ...
    integer::fh,ierr,ntime,ic,jc,kc,newtype,sizes(3),subsizes(3),starts(3)
    ...
    if(BcOption(yp_dir)==BC_OutFlow) then
      ...
      call MPI_TYPE_COMMIT(newtype,ierr)
      call MPI_FILE_SET_VIEW(fh,disp,real_type,newtype,'native',MPI_INFO_NULL,ierr)
      call MPI_FILE_READ_ALL(fh,OutFlowInfoY(4:6,:,:),subsizes(1)*subsizes(2)*subsizes(3),real_type,MPI_STATUS_IGNORE,ierr)
      disp=disp+ int(sizes(1),8)*int(sizes(2),8)*int(sizes(3),8)*int(mytype_bytes,8)
      if(ischeme==FI_AB2 .and. WriteHistOld) then
        call MPI_FILE_READ_ALL(fh,OutFlowInfoY(1:3,:,:),subsizes(1)*subsizes(2)*subsizes(3),real_type,MPI_STATUS_IGNORE,ierr)
        disp=disp+ int(sizes(1),8)*int(sizes(2),8)*int(sizes(3),8)*int(mytype_bytes,8)
      endif
      call MPI_TYPE_FREE(newtype,ierr)
      call MPI_FILE_CLOSE(fh,ierr)
    ...
    if(myProcNghBC(y_pencil,3)==BC_OutFlow) then
      ...
      subsizes=[3,y1size(2),y1size(3)]
      starts=[0,y1start(2)-1,y1start(3)-1]
      call MPI_FILE_OPEN(DECOMP_2D_COMM_ROW, chFile, MPI_MODE_RDONLY, MPI_INFO_NULL, fh, ierr)
      call MPI_TYPE_CREATE_SUBARRAY(3,sizes,subsizes,starts,MPI_ORDER_FORTRAN,real_type,newtype,ierr)
      call MPI_TYPE_COMMIT(newtype,ierr)
      call MPI_FILE_SET_VIEW(fh,disp,real_type,newtype,'native',MPI_INFO_NULL,ierr)
      call MPI_FILE_READ_ALL(fh,OutFlowInfoX(4:6,:,:),subsizes(1)*subsizes(2)*subsizes(3),real_type,MPI_STATUS_IGNORE,ierr)
      disp=disp+ int(sizes(1),8)*int(sizes(2),8)*int(sizes(3),8)*int(mytype_bytes,8)
      if(ischeme==FI_AB2 .and. WriteHistOld) then
        call MPI_FILE_READ_ALL(fh,OutFlowInfoX(1:3,:,:),subsizes(1)*subsizes(2)*subsizes(3),real_type,MPI_STATUS_IGNORE,ierr)
        disp=disp+ int(sizes(1),8)*int(sizes(2),8)*int(sizes(3),8)*int(mytype_bytes,8)
      endif
      call MPI_TYPE_FREE(newtype,ierr)
      call MPI_FILE_CLOSE(fh,ierr)
