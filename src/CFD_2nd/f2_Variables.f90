...
module f2_Variables
  ...
  real(RK), public,allocatable, dimension(:,:,:):: uxStar, uyStar, uzStar
...
contains  
  ...
  ! InverseTridiagonal
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90 
  subroutine AllocateVariables(); implicit none
      
    ! locals
    integer::ierrTmp,ierr=0
    ...
    call myallocate(uxStar, mb1, opt_global=.true.)
    call myallocate(uyStar, mb1, opt_global=.true.)
    call myallocate(uzStar, mb1, opt_global=.true.)
    uxStar=0.0_RK; uyStar=0.0_RK; uzStar=0.0_RK
#endif
#ifdef CFDLPT_TwoWay
    ...
    !-------------------------------------------------
    allocate(HistxOld(y1start(1):y1end(1), y1start(2):y1end(2), y1start(3):y1end(3)),Stat=ierrTmp);ierr=ierr+abs(ierrTmp)
    allocate(HistyOld(y1start(1):y1end(1), y1start(2):y1end(2), y1start(3):y1end(3)),Stat=ierrTmp);ierr=ierr+abs(ierrTmp)
    allocate(HistzOld(y1start(1):y1end(1), y1start(2):y1end(2), y1start(3):y1end(3)),Stat=ierrTmp);ierr=ierr+abs(ierrTmp)
    allocate(RealArr1(y1start(1):y1end(1), y1start(2):y1end(2), y1start(3):y1end(3)),Stat=ierrTmp);ierr=ierr+abs(ierrTmp)
    allocate(RealArr2(y1start(1):y1end(1), y1start(2):y1end(2), y1start(3):y1end(3)),Stat=ierrTmp);ierr=ierr+abs(ierrTmp)
#ifdef CFDACM
    allocate(FluidIndicator(y1start(1):y1end(1), y1start(2):y1end(2), y1start(3):y1end(3)),Stat=ierrTmp);ierr=ierr+abs(ierrTmp)
#endif
    ...
    if(myProcNghBC(y_pencil,3)==BC_OutFlow) then
      allocate(OutFlowInfoX(6,y1start(2):y1end(2),y1start(3):y1end(3)),stat=ierr); OutFlowInfoX=0.0_RK
    endif
    if(ierr/=0) call MainLog%CheckForError(ErrT_Abort,"AllocateVariables","Allocation failed 1")
    
    ! yp - outflow
    if(BcOption(yp_dir)==BC_OutFlow) then
      allocate(OutFlowInfoY(6,y1start(1):y1end(1),y1start(3):y1end(3)),stat=ierr); OutFlowInfoY=0.0_RK    
    endif
    if(ierr/=0) call MainLog%CheckForError(ErrT_Abort,"AllocateVariables","Allocation failed 2")
  end subroutine AllocateVariables
    
end module f2_Variables
