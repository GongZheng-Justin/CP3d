...
contains
  ...
  subroutine InitFilterCoe()
    ...
    if(FilterType==0) then     ! 0:trapezoidal type
      Fcoe = [1.0_RK, 2.0_RK, 1.0_RK, 2.0_RK, 4.0_RK, 2.0_RK, 1.0_RK, 2.0_RK, 1.0_RK]/16.0_RK
    elseif(FilterType==1) then ! 1:Simpson type
      Fcoe = [1.0_RK, 4.0_RK, 1.0_RK, 4.0_RK, 16._RK, 4.0_RK, 1.0_RK, 4.0_RK, 1.0_RK]/36.0_RK
    else
    ...
    do kc=y1start(3),y1end(3)
      ...
      do jc=y1start(2),y1end(2)
        ...
        do ic=y1start(1),y1end(1)
          ...
          S11= dudx
          S12=(dudy+dvdx)*0.5_RK
          S13=(dudz+dwdx)*0.5_RK
          S22= dvdy
          S23=(dvdz+dwdy)*0.5_RK
          S33= dwdz
          SijSij= S11*S11+ S22*S22+ S33*S33+ 2.0_RK*(S12*S12+ S13*S13+ S23*S23)
          
          Sd11= dudx*dudx+ dudy*dvdx+ dudz*dwdx
          Sd12=(dudx*dudy+ dudy*dvdy+ dudz*dwdy+ dvdx*dudx+ dvdy*dvdx+ dvdz*dwdx)*0.5_RK
          Sd13=(dudx*dudz+ dudy*dvdz+ dudz*dwdz+ dwdx*dudx+ dwdy*dvdx+ dwdz*dwdx)*0.5_RK
          Sd22= dvdx*dudy+ dvdy*dvdy+ dvdz*dwdy
          Sd23=(dvdx*dudz+ dvdy*dvdz+ dvdz*dwdz+ dwdx*dudy+ dwdy*dvdy+ dwdz*dwdy)*0.5_RK
          Sd33= dwdx*dudz+ dwdy*dvdz+ dwdz*dwdz
          SdTrace=(Sd11+Sd22+Sd33)/3.0_RK
          Sd11=Sd11-SdTrace
          Sd22=Sd22-SdTrace
          Sd33=Sd33-SdTrace
          SdijSdij= Sd11*Sd11+ Sd22*Sd22+ Sd33*Sd33+ 2.0_RK*(Sd12*Sd12+ Sd13*Sd13+ Sd23*Sd23)
  ...
  subroutine Clc_SGS_MTS(ux,uy,uz,nut)
      ...
      do jc=y1start(2),y1end(2)    
        do ic=y1start(1),y1end(1)
          ...
          uxf= Fcoe(1)*ux(im,jc,km) + Fcoe(2)        *ux(ic,jc,km) +Fcoe(3)*ux(ip,jc,km) + &
               Fcoe(4)*ux(im,jc,kc) +(Fcoe(5)-1.0_RK)*ux(ic,jc,kc) +Fcoe(6)*ux(ip,jc,kc) + &
    ...
    call update_halo(Ktemp, mb1, hi_K)
    DO kc=y1start(3),y1end(3)
      do jc=y1start(2),y1end(2)
        do ic=y1start(1),y1end(1)
          ip=ic+1
          Kes(ic,jc,kc)= 0.5_RK*(Ktemp(ic,jc,kc) +Ktemp(ip,jc,kc))
        enddo
      enddo
    ENDDO
      ...
      do jc=1,nyp
        do ic=y1start(1),y1end(1)
          ...
          uyf= Fcoe(1)*uy(im,jc,km) + Fcoe(2)        *uy(ic,jc,km) +Fcoe(3)*uy(ip,jc,km) + &
               Fcoe(4)*uy(im,jc,kc) +(Fcoe(5)-1.0_RK)*uy(ic,jc,kc) +Fcoe(6)*uy(ip,jc,kc) + &
    ...
    DO kc=y1start(3),y1end(3)
      do jc=1,nyc
        jp=jc+1
        do ic=y1start(1),y1end(1)
          Kes(ic,jc,kc)= Kes(ic,jc,kc)+ 0.5_RK*(Ktemp(ic,jc,kc) +Ktemp(ic,jp,kc))
        enddo
      enddo
    ENDDO
      ...
      do jc=y1start(2),y1end(2)    
        do ic=y1start(1),y1end(1)
          ...
          uzf= Fcoe(1)*uz(im,jc,km) + Fcoe(2)        *uz(ic,jc,km) +Fcoe(3)*uz(ip,jc,km) + &
               Fcoe(4)*uz(im,jc,kc) +(Fcoe(5)-1.0_RK)*uz(ic,jc,kc) +Fcoe(6)*uz(ip,jc,kc) + &
    ...
    DO kc=y1start(3),y1end(3)
      ...
      do jc=y1start(2),y1end(2)
        do ic=y1start(1),y1end(1)
          Kes(ic,jc,kc)= Kes(ic,jc,kc)+ 0.5_RK*(Ktemp(ic,jc,kc) +Ktemp(ic,jc,kp))
        enddo
      enddo
    ENDDO
    ...
    DO kc=y1start(3),y1end(3)
      do jc=y1start(2),y1end(2)
        rdelta = 1.0_RK/DeltaCell(jc)
        do ic=y1start(1),y1end(1)
          Ksgs= Kes(ic,jc,kc)
          Ts = sqrt(Ksgs)*rdelta + strain_cha(ic,jc,kc)/10.0_RK
    ...
    DO kc=y1start(3),y1end(3)
      ...
      do jc=y1start(2),y1end(2)
        ...
        do ic=y1start(1),y1end(1)
          ...
          st4(ic,jc,kc)=0.125_RK*((uy(ip,jp,kc) -uy(im,jp,kc) +uy(ip,jc,kc) -uy(im,jc,kc))*rdx  &
          ...
          st5(ic,jc,kc)=0.125_RK*((ux(ip,jc,kp) +ux(ic,jc,kp) -ux(ip,jc,km) -ux(ic,jc,km))*rdz  &  
          ...
          st6(ic,jc,kc)=0.125_RK*((uy(ic,jp,kp) +uy(ic,jc,kp) -uy(ic,jp,km) -uy(ic,jc,km))*rdz  &
      ...
      do jc=y1start(2),y1end(2)
        ...
        do ic=y1start(1),y1end(1)
          ...
          st6=0.125_RK*((uy(ic,jp,kp) +uy(ic,jc,kp) -uy(ic,jp,km) -uy(ic,jc,km))*rdz  &
                       ...
                       +(uz(ic,jc,kp) +uz(ic,jc,kc))*cac12                            &
                       -(uz(ic,jm,kp) +uz(ic,jm,kc))*cac1                             )
          strain_cha(ic,jc,kc)= sqrt(2.0_RK*(st1*st1+ st2*st2+ st3*st3)+ 4.0_RK*(st4*st4 + st5*st5 + st6*st6))
        enddo
      enddo
    ENDDO
  ...
  subroutine SetBC_and_UpdateHalo_SGS(nut)
    ...
    CASE(BC_NoSlip)           
      nut(y1start(1):y1end(1), nyp, y1start(3):y1end(3)) =  2.0_RK*xnu-nut(y1start(1):y1end(1), nyc, y1start(3):y1end(3)) 
    CASE(BC_FreeSlip) 
    ...
    CASE(BC_NoSlip)        
      nut(y1start(1):y1end(1), 0, y1start(3):y1end(3)) =  2.0_RK*xnu-nut(y1start(1):y1end(1), 1, y1start(3):y1end(3)) 
    CASE(BC_FreeSlip)
    ...
    CASE(BC_NoSlip)           
      nut(nxp, 0:nyp,y1start(3):y1end(3))= 2.0_RK*xnu-nut(nxc, 0:nyp,y1start(3):y1end(3))
    CASE(BC_FreeSlip) 
    ...
    CASE(BC_NoSlip)        
      nut(0,0:nyp,y1start(3):y1end(3)) = 2.0_RK*xnu-nut(1,0:nyp,y1start(3):y1end(3))       
    CASE(BC_FreeSlip)
    ...
    CASE(BC_NoSlip)         
      nut(y1start(1):y1end(1), 0:nyp, nzp) = 2.0_RK*xnu-nut(y1start(1):y1end(1), 0:nyp, nzc)
    CASE(BC_FreeSlip)
    ...
    CASE(BC_NoSlip)         
      nut(y1start(1):y1end(1), 0:nyp, 0) = 2.0_RK*xnu-nut(y1start(1):y1end(1), 0:nyp, 1)
    CASE(BC_FreeSlip) 
      nut(y1start(1):y1end(1), 0:nyp, 0) =         nut(y1start(1):y1end(1), 0:nyp, 1)
    END SELECT     

    call update_halo(nut, mb1, hi_nut)
