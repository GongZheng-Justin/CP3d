...
!   InitATPStatistics
!********************************************************************************
  subroutine InitATPStatistics(chFile)
    implicit none
    ...
    if(ierror/=0) call ATPLogInfo%CheckForError(ErrT_Abort,"InitATPStatistics", "Cannot open file: "//trim(chFile))
    read(nUnit, nml=ParticleStatisticOption)
    if(nrank==0)write(ATPLogInfo%nUnit, nml=ParticleStatisticOption)
    ...
    if(nrank/=0) return
    write(filename,'(A,I10.10)') trim(ATP_Opt%ResultsDir)//'shannon',ilast
    open(newunit=nUnit,file=filename,status='replace',form='formatted',IOSTAT=ierror)
    if(ierror /= 0) call ATPLogInfo%CheckForError(ErrT_Abort,"InitATPStatistics","Cannot open file: "//trim(filename))
    close(nUnit,IOSTAT=ierror)
    do j=1,npType
      write(filename,"(A,I10.10,A,I4.4)") trim(ATP_Opt%ResultsDir)//'MeanSquare',ilast,'_set',j
      open(newunit=nUnit,file=filename,status='replace',form='formatted',IOSTAT=ierror)
      if(ierror /= 0) call ATPLogInfo%CheckForError(ErrT_Abort,"InitATPStatistics","Cannot open file: "//trim(filename))
      close(nUnit,IOSTAT=ierror)    
    enddo
  ...
  subroutine ClcATPStatistics()
    ...
    integer::pid,nlocal,itype,js,je,jc,ierror,nUnit,nBin
    ...
    IF(nrank==0) THEN
      DO itype=1,npType
        nBin=ATPProperty%nPrtcl_in_Bin(itype)
        do jc=1,nShannon
          pse = real(npsslotR(itype,jc),RK)/real(nBin,RK)
          if(pse > 0.0_RK) then
      ...
      if(ierror /= 0) then
        call ATPLogInfo%CheckForError(ErrT_Pass,"ClcATPStatistics","Cannot open file: "//trim(filename))
      else  
        write(nUnit,'(I10,10ES24.15)')itime,shannon_entropy
      endif
      close(nUnit,IOSTAT=ierror)
    ENDIF
    ...
    ! Mean square
    block
      real(RK),dimension(:,:),allocatable::SumDist,SumDistR
      allocate(SumDist(6,npType),SumDistR(6,npType))
      SumDist=0.0_RK; SumDistR=0.0_RK
      do pid=1,nlocal
        pos  = GPrtcl_MoveDistance(pid)
        itype= GPrtcl_pType(pid)
        SumDist(1,itype)=SumDist(1,itype)+pos%x
        SumDist(2,itype)=SumDist(2,itype)+pos%x*pos%x
        SumDist(3,itype)=SumDist(3,itype)+pos%y
        SumDist(4,itype)=SumDist(4,itype)+pos%y*pos%y        
        SumDist(5,itype)=SumDist(5,itype)+pos%z
        SumDist(6,itype)=SumDist(6,itype)+pos%z*pos%z
      enddo
      call MPI_REDUCE(SumDist,SumDistR, npType*6,real_type,MPI_SUM,0,MPI_COMM_WORLD,ierror)
      IF(nrank==0) THEN
        DO itype=1,npType
          nBin=ATPProperty%nPrtcl_in_Bin(itype)
          do jc=1,6
            SumDistR(jc,itype)=SumDistR(jc,itype)/real(nBin,RK)
          enddo
          do jc=1,3
            js=2*jc-1; je=js+1  
            SumDistR(je,itype)=SumDistR(je,itype)-SumDistR(js,itype)*SumDistR(js,itype)
          enddo
          write(filename,"(A,I10.10,A,I4.4)") trim(ATP_Opt%ResultsDir)//'MeanSquare',ilast,'_set',itype
          open(newunit=nUnit,file=filename,status='old',position='append',form='formatted',IOSTAT=ierror)
          if(ierror /= 0) then
            call ATPLogInfo%CheckForError(ErrT_Pass,"ClcATPStatistics","Cannot open file: "//trim(filename))
          else  
            write(nUnit,'(I10,6ES24.15)')itime,SumDistR(:,itype)
          endif
          close(nUnit,IOSTAT=ierror)        
        ENDDO
      ENDIF      
      deallocate(SumDist,SumDistR)
    end block
    ...
    if(nrank==0) then
      ...
      do itype=1,npType
        write(filename,"(A,I10.10,A,I4.4)") trim(ATP_Opt%ResultsDir)//'pstats',itime,'_set',itype
