...
contains
  ...
  subroutine PC_Comm_For_Exchange(this)
    ...
    call MPI_SENDRECV(nsend, 2, int_type, ATP_decomp%ProcNgh(3), 9, &
                      nrecv, 2, int_type, ATP_decomp%ProcNgh(4), 9, MPI_COMM_WORLD,SRstatus,ierror)
    if(nrecv(2)>0) then
      allocate(buf_recv(nrecv(2)))
      call MPI_IRECV(buf_recv,nrecv(2),real_type,ATP_decomp%ProcNgh(4),10,MPI_COMM_WORLD,request(1),ierror)
    endif
    if(nsend(2)>0) then
      allocate(buf_send(nsend(2)))
      nlocalp= nlocal;  nlocal = nlocalp - nsend(1)
      call this%pack_Exchange(buf_send,nsend(1),nlocalp,xp_axis)
      call MPI_SEND(buf_send,nsend(2),real_type,ATP_decomp%ProcNgh(3),10,MPI_COMM_WORLD,ierror)
    endif
    if(nrecv(2)>0) then
    ...
    call MPI_SENDRECV(nsend, 2, int_type, ATP_decomp%ProcNgh(4),11, &
                      nrecv, 2, int_type, ATP_decomp%ProcNgh(3),11, MPI_COMM_WORLD,SRstatus,ierror)
    if(nrecv(2)>0) then
      allocate(buf_recv(nrecv(2)))
      call MPI_IRECV(buf_recv,nrecv(2),real_type,ATP_decomp%ProcNgh(3),12,MPI_COMM_WORLD,request(2),ierror)
    endif
    if(nsend(2)>0) then
      allocate(buf_send(nsend(2)))
      nlocalp= nlocal;  nlocal = nlocalp - nsend(1)
      call this%pack_Exchange(buf_send,nsend(1),nlocalp,xm_axis)
      call MPI_SEND(buf_send,nsend(2),real_type,ATP_decomp%ProcNgh(4),12,MPI_COMM_WORLD,ierror)
    endif
    if(nrecv(2)>0) then
    ...
    call MPI_SENDRECV(nsend, 2, int_type, ATP_decomp%ProcNgh(1),13, &
                      nrecv, 2, int_type, ATP_decomp%ProcNgh(2),13, MPI_COMM_WORLD,SRstatus,ierror)
    if(nrecv(2)>0) then
      allocate(buf_recv(nrecv(2)))
      call MPI_IRECV(buf_recv,nrecv(2),real_type,ATP_decomp%ProcNgh(2),14,MPI_COMM_WORLD,request(3),ierror)
    endif
    if(nsend(2)>0) then
      allocate(buf_send(nsend(2)))
      nlocalp= nlocal;  nlocal = nlocalp - nsend(1)
      call this%pack_Exchange(buf_send,nsend(1),nlocalp,zp_axis)
      call MPI_SEND(buf_send,nsend(2),real_type,ATP_decomp%ProcNgh(1),14,MPI_COMM_WORLD,ierror)
    endif
    if(nrecv(2)>0) then
    ...
    call MPI_SENDRECV(nsend, 2, int_type, ATP_decomp%ProcNgh(2),15, &
                      nrecv, 2, int_type, ATP_decomp%ProcNgh(1),15, MPI_COMM_WORLD,SRstatus,ierror)
    if(nrecv(2)>0) then
      allocate(buf_recv(nrecv(2)))
      call MPI_IRECV(buf_recv,nrecv(2),real_type,ATP_decomp%ProcNgh(1),16,MPI_COMM_WORLD,request(4),ierror)
    endif
    if(nsend(2)>0) then
      allocate(buf_send(nsend(2)))
      nlocalp= nlocal;  nlocal = nlocalp - nsend(1)
      call this%pack_Exchange(buf_send,nsend(1),nlocalp,zm_axis)
      call MPI_SEND(buf_send,nsend(2),real_type,ATP_decomp%ProcNgh(2),16,MPI_COMM_WORLD,ierror)
    endif
    if(nrecv(2)>0) then
  ...
  function PC_ISInThisProc(this,buf_recv,m,dir) result(res)
    ...
    call ATPLogInfo%OutInfo(" Exchange direction  is :"//trim(num2str(dir)  ),3)
    call ATPLogInfo%OutInfo("   The particle id   is :"//trim(num2str( nint(buf_recv(m)  ))),3)
    call ATPLogInfo%OutInfo("       particle type is :"//trim(num2str( nint(buf_recv(m+1)))),3)
    call ATPLogInfo%OutInfo("       x-coordinate  is :"//trim(num2str( buf_recv(m+3) )), 3)
    call ATPLogInfo%OutInfo("       y-coordinate  is :"//trim(num2str( buf_recv(m+4) )), 3)
    call ATPLogInfo%OutInfo("       z-coordinate  is :"//trim(num2str( buf_recv(m+5) )), 3)
    call ATPLogInfo%OutInfo(" Present processor   is :"//trim(num2str(nrank)), 3)
