...
contains
  ...
  subroutine ChannelACM_Iterate()
    ...
    integer::ns,iForcingExtra,idem_start,idem_end,idem,iCountACM
    ...
    iCountACM=0
    if(DEM_Opt%RestartFlag)iCountACM=1
    DO itime=ifirst, ilast
      ...
      do ns=1,iadvance
        ...
        ELSE
          ...
          call IntegrateFluidPrtclForce_imp(iCountACM,uxStar,uyStar,uzStar)
        ...
        IF(IBM_Scheme<2) THEN
          call IntegrateFluidPrtclForce(iCountACM,ux,uy,uz)
          do idem=idem_start,idem_end
            call DEM%iterate(idem)
          enddo
        ENDIF
