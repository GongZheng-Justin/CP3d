  ...
  subroutine unpack_Comm_FluidIndicator(buf_recv,n1,n2)
    ...
    m=1
    do i=n1,n2
      itype              =int(buf_recv(m)+0.2); m=m+1 ! 01
  ...
  subroutine Init_IBPFix()
    ...
    ! locals
    type(real3):: Cposition,IbpPos,PosDiff
  ...
  real(RK) function delta_fun(ratio_in)
    ...
    elseif(ratio> 0.50_RK ) then
      delta_fun= 0.166666666666666666667_RK*(five-three*ratio-sqrt(one-three*(one-ratio)**2))
    else
      delta_fun= 0.333333333333333333333_RK*(one+sqrt(one-three*ratio*ratio))
    endif
