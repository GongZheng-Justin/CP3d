  ...
  subroutine Gather_Halo_IBMForce(VolForce_x,VolForce_y,VolForce_z)
    ...
    do k=mb1%zmm,mb1%zpm
      do i=mb1%xmm,mb1%xpm
        VolForce_x(i,  1,k)=VolForce_x(i,  1,k)+VolForce_x(i,    0,k);  VolForce_x(i,  0,  k)=zero
        VolForce_y(i,  1,k)=VolForce_y(i,  1,k)+VolForce_y(i,    0,k);  VolForce_y(i,  0,  k)=zero
        VolForce_z(i,  1,k)=VolForce_z(i,  1,k)+VolForce_z(i,    0,k);  VolForce_z(i,  0,  k)=zero
        VolForce_x(i,nyc,k)=VolForce_x(i,nyc,k)+VolForce_x(i,  nyp,k);  VolForce_x(i,nyp,  k)=zero
        VolForce_y(i,nyp,k)=VolForce_y(i,nyp,k)+VolForce_y(i,nyp+1,k);  VolForce_y(i,nyp+1,k)=zero
        VolForce_z(i,nyc,k)=VolForce_z(i,nyc,k)+VolForce_z(i,  nyp,k);  VolForce_z(i,nyp,  k)=zero
      enddo
    enddo
#endif
