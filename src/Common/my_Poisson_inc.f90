#include "definitions_inc.f90"
  ...
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine Initialize_ReduceMatrix(startIn,endIn,x_or_z); implicit none
  ...
  ! clcPPE_x1
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine clcPPE_x1(prsrc, prphiHalo); implicit none
  ...
  ! clcPPE_x1_reduce
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine clcPPE_x1_reduce(prsrc, prphiHalo); implicit none
    real(RK),dimension(y1start(1):y1end(1),y1start(2):y1end(2),y1start(3):y1end(3)),intent(inout)::prsrc
    real(RK),dimension(mb1%xmm:mb1%xpm,mb1%ymm:mb1%ypm,mb1%zmm:mb1%zpm),intent(out)::prphiHalo
  ...
  ! clcPPE_z1
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine clcPPE_z1(prsrc, prphiHalo); implicit none
    real(RK),dimension(y1start(1):y1end(1),y1start(2):y1end(2),y1start(3):y1end(3)),intent(inout)::prsrc
    real(RK),dimension(mb1%xmm:mb1%xpm,mb1%ymm:mb1%ypm,mb1%zmm:mb1%zpm),intent(out)::prphiHalo
  ...
  ! clcPPE_z1_reduce
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine clcPPE_z1_reduce(prsrc, prphiHalo); implicit none
    real(RK),dimension(y1start(1):y1end(1),y1start(2):y1end(2),y1start(3):y1end(3)),intent(inout)::prsrc
    real(RK),dimension(mb1%xmm:mb1%xpm,mb1%ymm:mb1%ypm,mb1%zmm:mb1%zpm),intent(out)::prphiHalo
  ...
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90  
  subroutine InverseTridiagonal_reduce(aj,cj,fj,m,n); implicit none
  ...
  ! clcPPE_x1_periodic
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine clcPPE_x1_periodic(prsrc, prphiHalo); implicit none
    real(RK),dimension(y1start(1):y1end(1),y1start(2):y1end(2),y1start(3):y1end(3)),intent(inout)::prsrc
    real(RK),dimension(mb1%xmm:mb1%xpm,mb1%ymm:mb1%ypm,mb1%zmm:mb1%zpm),intent(out)::prphiHalo
  ...
  ! clcPPE_z1_periodic
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine clcPPE_z1_periodic(prsrc, prphiHalo); implicit none
    real(RK),dimension(y1start(1):y1end(1),y1start(2):y1end(2),y1start(3):y1end(3)),intent(inout)::prsrc
    real(RK),dimension(mb1%xmm:mb1%xpm,mb1%ymm:mb1%ypm,mb1%zmm:mb1%zpm),intent(out)::prphiHalo
...
#ifdef my_Poisson_inc_add_UniformYmesh
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  ! clcPPE_x1_UniformYmesh_periodic
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine clcPPE_x1_UniformYmesh_periodic(prsrc, prphiHalo); implicit none
    real(RK),dimension(y1start(1):y1end(1),y1start(2):y1end(2),y1start(3):y1end(3)),intent(inout)::prsrc
    real(RK),dimension(mb1%xmm:mb1%xpm,mb1%ymm:mb1%ypm,mb1%zmm:mb1%zpm),intent(out)::prphiHalo
    ...
    integer::ic,jc,kc
    real(RK),dimension(x1size(1),x1size(2),x1size(3))::arrx
    real(RK),dimension(z2start(1):z2end(1),z2start(2):z2end(2),z2start(3):z2end(3))::arrz
    real(RK),dimension(z2start(1):z2end(1),z2start(3):z2end(3))::tridmk,tridck,tridpk,tridfk

    call my_execute_FFTW_r2r_y(fwd_plan_y,y1size(1),y1size(2),y1size(3),prsrc)
    call transpose_y1_to_x1(prsrc,arrx)
    call my_execute_FFTW_r2r_x(fwd_plan_x,x1size(1),x1size(2),x1size(3),arrx)
    call transpose_x1_to_z2(arrx,arrz)
    ...
    do kc=z2start(3),z2end(3)
      do ic=z2start(1),z2end(1)
        tridmk(ic,kc) = am3Pr(kc)
        tridpk(ic,kc) = ap3Pr(kc)
      enddo
    enddo
    IF(nrank==0) THEN
      do jc=2,z2end(2)
        do kc=1, nzc
          do ic=1,z2end(1)
            tridck(ic,kc) = ac3Pr(kc) +WaveNumX(ic) +WaveNumY(jc)
            tridfk(ic,kc) = arrz(ic,jc,kc)
          enddo
        enddo
        call InversePeriodicTridiagonal(tridmk,tridck,tridpk,tridfk,z2size(1),nzc)
        do kc=1, nzc
          do ic=1, z2end(1)      
            arrz(ic,jc,kc) = tridfk(ic,kc)
          enddo
        enddo      
      enddo    
    ...
    call my_execute_FFTW_r2r_x(bwd_plan_x,x1size(1),x1size(2),x1size(3),arrx)
    call transpose_x1_to_y1(arrx,prsrc)
    call my_execute_FFTW_r2r_y(bwd_plan_y,y1size(1),y1size(2),y1size(3),prsrc)
    do kc=y1start(3),y1end(3)
      do jc=y1start(2),y1end(2)
        do ic=y1start(1),y1end(1)
          prphiHalo(ic,jc,kc) = prsrc(ic,jc,kc)*normfft
      ...
      enddo
    enddo
  end subroutine clcPPE_x1_UniformYmesh_periodic
  
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  ! clcPPE_z1_UniformYmesh_periodic
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine clcPPE_z1_UniformYmesh_periodic(prsrc, prphiHalo); implicit none
    real(RK),dimension(y1start(1):y1end(1),y1start(2):y1end(2),y1start(3):y1end(3)),intent(inout)::prsrc
    real(RK),dimension(mb1%xmm:mb1%xpm,mb1%ymm:mb1%ypm,mb1%zmm:mb1%zpm),intent(out)::prphiHalo
    ...
    integer::ic,jc,kc
    real(RK),dimension(z1size(1),z1size(2),z1size(3))::arrz
    real(RK),dimension(x2start(1):x2end(1),x2start(2):x2end(2),x2start(3):x2end(3))::arrx
    real(RK),dimension(x2start(2):x2end(2),x2start(1):x2end(1))::tridmi,tridci,tridpi,tridfi
    ...
    do jc=x2start(2),x2end(2)
      do ic=x2start(1),x2end(1)
        tridmi(jc,ic) = am1Pr(ic)
        tridpi(jc,ic) = ap1Pr(ic)
      enddo
    enddo
    IF(nrank==0) THEN
      do kc=2,x2end(3)
        do jc=x2start(2),x2end(2)
          do ic=1, nxc
            tridci(jc,ic) = ac1Pr(ic) +WaveNumY(jc)+WaveNumZ(kc)
            tridfi(jc,ic) = arrx(ic,jc,kc)
          enddo
        enddo
        call InversePeriodicTridiagonal(tridmi,tridci,tridpi,tridfi,x2size(2),nxc)
        do jc=x2start(2),x2end(2)
          do ic=1, nxc
            arrx(ic,jc,kc) = tridfi(jc,ic)
          enddo
        enddo          
      enddo
    ...
    do kc=y1start(3),y1end(3)
      ...
      enddo
    enddo
  end subroutine clcPPE_z1_UniformYmesh_periodic
  
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  ! clcPPE_x1_UniformYmesh
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine clcPPE_x1_UniformYmesh(prsrc, prphiHalo); implicit none
    real(RK),dimension(y1start(1):y1end(1),y1start(2):y1end(2),y1start(3):y1end(3)),intent(inout)::prsrc
    real(RK),dimension(mb1%xmm:mb1%xpm,mb1%ymm:mb1%ypm,mb1%zmm:mb1%zpm),intent(out)::prphiHalo
    ...
    IF(nrank==0) THEN
      do jc=2,z2end(2)
        do kc=1, nzc
          ...
          enddo
        enddo
        call InverseTridiagonal(tridmk,tridck,tridpk,tridfk,z2size(1),nzc)
        do kc=1, nzc
          do ic=1, z2end(1)      
            arrz(ic,jc,kc) = tridfk(ic,kc)
          enddo
    ...
    do kc=y1start(3),y1end(3)
      ...
      enddo
    enddo
  end subroutine clcPPE_x1_UniformYmesh
  
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  ! clcPPE_z1_UniformYmesh
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine clcPPE_z1_UniformYmesh(prsrc, prphiHalo); implicit none
    real(RK),dimension(y1start(1):y1end(1),y1start(2):y1end(2),y1start(3):y1end(3)),intent(inout)::prsrc
    real(RK),dimension(mb1%xmm:mb1%xpm,mb1%ymm:mb1%ypm,mb1%zmm:mb1%zpm),intent(out)::prphiHalo
    ...
    IF(nrank==0) THEN
      do kc=2,x2end(3)
        do jc=x2start(2),x2end(2)
          ...
          enddo
        enddo
        call InverseTridiagonal(tridmi,tridci,tridpi,tridfi,x2size(2),nxc)
        do jc=x2start(2),x2end(2)
          do ic=1, nxc
            arrx(ic,jc,kc) = tridfi(jc,ic)
          enddo
    ...
    do kc=y1start(3),y1end(3)
      ...
      enddo
    enddo
  end subroutine clcPPE_z1_UniformYmesh
! end if my_Poisson_inc_add_UniformYmesh
#endif
