...
contains
  ...
  subroutine decomp_info_init(nx,ny,nz,decomp,np_row_col,initialize)
    ...
    ! locals
    integer::k,ksize,ierror,p_row,p_col
    ...
    if(.not.IsInitTopologyFinished) then
      ...
      call MPI_COMM_SPLIT(MPI_COMM_WORLD,pcoord(1),nrank,DECOMP_2D_COMM_ROW,ierror)       
      call MPI_COMM_SPLIT(MPI_COMM_WORLD,pcoord(2),nrank,DECOMP_2D_COMM_COL,ierror)
    endif
  ...
  ! findfactor
  !******************************************************************
  subroutine findfactor(num, factors, ifact, nfact)
    implicit none
  ...
  ! update_halo
  !**********************************************************************
  subroutine update_halo(mat,mb,hi)
    implicit none
    ...
    integer::jc,data_type,halo_type,ierror,ProcNgh(4) 
    integer::icount,ilength,ijump,SRstatus(MPI_STATUS_SIZE)
  
    data_type=real_type
    if(mb%pencil /= hi%pencil ) then
      ierror = 14
      call decomp_2d_abort(ierror,'update_halo wrong: pencil error, Gong Zheng')
    endif
    if(mb%xme<hi%xmh .or. mb%yme<hi%ymh .or. mb%zme<hi%zmh .or.  &
      ...
      ierror = 14
      call decomp_2d_abort(ierror,'update_halo wrong: HaloSize error, Gong Zheng')
    endif
      ...
      IF(hi%xmh > 0) THEN
        ...
        if(ProcNgh(3)==nrank) then  ! neighbour is nrank itself, and ProcNgh(4)==nrank also !!!
          ...
          call MPI_SENDRECV(mat(is1,js1,ks1),1,halo_type,ProcNgh(3),1, &
                            mat(ir1,jr1,kr1),1,halo_type,ProcNgh(4),1, MPI_COMM_WORLD,SRstatus,ierror)
      ...
      IF(hi%xph > 0) THEN
        ...
        if(ProcNgh(4)==nrank) then  ! neighbour is nrank itself, and ProcNgh(3)==nrank also !!!
          ...
          call MPI_SENDRECV(mat(is1,js1,ks1),1,halo_type,ProcNgh(4),2, &
                            mat(ir1,jr1,kr1),1,halo_type,ProcNgh(3),2, MPI_COMM_WORLD,SRstatus,ierror)
          call MPI_TYPE_FREE(halo_type, ierror)
        endif
      ENDIF
      ...
      IF(hi%zmh > 0) THEN
        ...
        if(ProcNgh(1)==nrank) then  ! neighbour is nrank itself, and ProcNgh(2)==nrank also !!!
          ...
          call MPI_SENDRECV(mat(is1,js1,ks1),icount,data_type,ProcNgh(1),3, &
                            mat(ir1,jr1,kr1),icount,data_type,ProcNgh(2),3, MPI_COMM_WORLD,SRstatus,ierror)
        endif
      ENDIF
      ...
      IF(hi%zph > 0) THEN
        ...
        if(ProcNgh(2)==nrank) then  ! neighbour is nrank itself, and ProcNgh(1)==nrank also !!!
          ...
          call MPI_SENDRECV(mat(is1,js1,ks1),icount,data_type,ProcNgh(2),4, &
                            mat(ir1,jr1,kr1),icount,data_type,ProcNgh(1),4, MPI_COMM_WORLD,SRstatus,ierror)
        endif
      ENDIF
    CASE(3)  ! z-pencil
      ! To DO
  ...
  subroutine sum_halo(mat,mb,hi)
    ...
    if(mb%pencil /= hi%pencil) then
      ierror = 14
      call decomp_2d_abort(ierror,'sum_halo wrong: pencil error, Gong Zheng')
    endif
    if(mb%xme<hi%xmh .or. mb%yme<hi%ymh .or. mb%zme<hi%zmh .or.  &
       mb%xpe<hi%xph .or. mb%ype<hi%yph .or. mb%zpe<hi%zph .or.  &
       hi%xmh<0      .or. hi%ymh<0      .or. hi%zmh<0      .or.  &
       hi%xph<0      .or. hi%yph<0      .or. hi%zph<0       ) then
      ierror = 14
      call decomp_2d_abort(ierror,'sum_halo wrong: HaloSize error, Gong Zheng')
    endif
      ...
      IF(hi%zph > 0) THEN
        ...
        if(ProcNgh(2)==nrank) then  ! neighbour is nrank itself, and ProcNgh(1)==nrank also !!!
          ...
          call MPI_SENDRECV(mat(is1,js1,ks1),icount,data_type,ProcNgh(1),5, &
                            Mat3HaloR,       icount,data_type,ProcNgh(2),5, MPI_COMM_WORLD,SRstatus,ierror)
        endif
      ...
      IF(hi%zmh > 0) THEN
        ...
        if(ProcNgh(1)==nrank) then  ! neighbour is nrank itself, and ProcNgh(2)==nrank also !!!
          ...
          call MPI_SENDRECV(mat(is1,js1,ks1),icount,data_type,ProcNgh(2),6, &
                            Mat3HaloR,       icount,data_type,ProcNgh(1),6, MPI_COMM_WORLD,SRstatus,ierror)
        endif
        mat(ir1:ir2, jr1:jr2, kr1:kr2)=mat(ir1:ir2, jr1:jr2, kr1:kr2)+Mat3HaloR
        deallocate(Mat3HaloR)        
      ENDIF    
      ...
      IF(hi%xph > 0) THEN
        ...
        if(ProcNgh(4)==nrank) then  ! neighbour is nrank itself, and ProcNgh(3)==nrank also !!!
          ...
          call MPI_SENDRECV(mat(is1,js1,ks1),1,halo_type,ProcNgh(3),7, &
                      Mat3HaloR,icount*ilength,data_type,ProcNgh(4),7, MPI_COMM_WORLD,SRstatus,ierror)
          call MPI_TYPE_FREE(halo_type, ierror)
        endif
        mat(ir1:ir2, jr1:jr2, kr1:kr2)=mat(ir1:ir2, jr1:jr2, kr1:kr2)+Mat3HaloR
        deallocate(Mat3HaloR)
      ...
      IF(hi%xmh > 0) THEN       
        ...
        if(ProcNgh(3)==nrank) then  ! neighbour is nrank itself, and ProcNgh(4)==nrank also !!!
          ...
          call MPI_SENDRECV(mat(is1,js1,ks1),1,halo_type,ProcNgh(4),8, &
                      Mat3HaloR,icount*ilength,data_type,ProcNgh(3),8, MPI_COMM_WORLD,SRstatus,ierror)
          call MPI_TYPE_FREE(halo_type, ierror)
        endif 
        mat(ir1:ir2, jr1:jr2, kr1:kr2)=mat(ir1:ir2, jr1:jr2, kr1:kr2)+Mat3HaloR
        deallocate(Mat3HaloR)
