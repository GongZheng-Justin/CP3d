...
#define DECOMP_TRANSPOSE_Z2X1 6
...
#ifdef SAVE_SINGLE
  integer,parameter,public:: mytype_save = 4
  ...
  logical::IsInitTopologyFinished=.false.
  ...
  public::decomp_2d_init,decomp_2d_finalize,decomp_info_init,myallocate,    &
          ...
          decomp_2d_write_plane_3d, decomp_2d_write_plane_reduce,           &
          decomp_2d_write_every, update_halo, sum_halo,                     &                           
...
contains
  ...
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine decomp_2d_init(nx,ny,nz,iproc,p_row,p_col,mainPencil,BcOptions); implicit none
    integer,intent(INOUT)::p_row,p_col
    integer,intent(in)::nx,ny,nz,mainPencil,iproc,BcOptions(6)

    ! locals
    integer::errorcode,ierr,nrow,ncol

    nrow=p_row
    ncol=p_col
    nproc=iproc
    IsInitTopologyFinished=.false.
    ...
    call MPI_TYPE_SIZE(real_type,mytype_bytes,ierr)
  ...
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine decomp_info_init(nx,ny,nz,decomp,np_row_col,initialize); implicit none
    ...
    ! locals
    integer::k,ksize,ierr,p_row,p_col
    ...
    if(present(np_row_col)) then
      if(IsInitTopologyFinished) then
        call decomp_2d_abort(1,'Initializing topology has finished. New np_row_col is forbidden !!!')      
      endif
      p_row=np_row_col(1)
      p_col=np_row_col(2)
    else
    ...
    decomp%nrow=p_row
    decomp%ncol=p_col
    if(.not.IsInitTopologyFinished) then
      pcoord(1)= int(nrank / p_col)
      pcoord(2)= mod(nrank,  p_col)
      call MPI_COMM_SPLIT(MPI_COMM_WORLD,pcoord(1),nrank,DECOMP_2D_COMM_ROW,ierr)       
      call MPI_COMM_SPLIT(MPI_COMM_WORLD,pcoord(2),nrank,DECOMP_2D_COMM_COL,ierr)
    endif
    ...
    if(decomp%initialize(DECOMP_TRANSPOSE_X1Y1)) then
      if(min(nx,ny)<p_row .or. nz<p_col) ierr=1
    endif
    if(decomp%initialize(DECOMP_TRANSPOSE_Y1Z1)) then
      if(min(ny,nz)<p_col .or. nx<p_row) ierr=2  
    endif
    if(decomp%initialize(DECOMP_TRANSPOSE_Z1X2)) then
      if(min(nz,nx)<p_row .or. ny<p_col) ierr=3     
    endif    
    if(decomp%initialize(DECOMP_TRANSPOSE_X2Y2)) then
      if(min(nx,ny)<p_col .or. nz<p_row) ierr=4 
    endif
    if(decomp%initialize(DECOMP_TRANSPOSE_Y2Z2)) then
      if(min(ny,nz)<p_row .or. nx<p_col) ierr=5   
    endif
    if(decomp%initialize(DECOMP_TRANSPOSE_Z2X1)) then
      if(min(nz,nx)<p_col .or. ny<p_row) ierr=6
    endif            
    if(ierr/=0) then
      call decomp_2d_abort(ierr,'Invalid 2D processor grid !!!')
    endif
    ...
    ! x1 <-> y1
    if(decomp%initialize(DECOMP_TRANSPOSE_X1Y1)) then
      allocate(decomp%xtype_x1y1(p_row),stat=ierr)
      if(ierr/=0) call decomp_2d_abort(ierr,'decomp_info_init- Allocate Wrong 2 !!!')
      sizes=decomp%x1sz
      ...
      do k=1,p_row
        ...
        call MPI_TYPE_CREATE_SUBARRAY(3,sizes,subsizes,starts,MPI_ORDER_FORTRAN, &
          real_type,decomp%xtype_x1y1(k),ierr)
        call MPI_TYPE_COMMIT(decomp%xtype_x1y1(k),ierr)      
      enddo
      allocate(decomp%ytype_x1y1(p_row),stat=ierr)
      if(ierr/=0) call decomp_2d_abort(ierr,'decomp_info_init- Allocate Wrong 3 !!!')
      sizes=decomp%y1sz
      ...
      do k=1,p_row
        ...
        istart=istart+ksize
        call MPI_TYPE_CREATE_SUBARRAY(3,sizes,subsizes,starts,MPI_ORDER_FORTRAN, &
          real_type,decomp%ytype_x1y1(k),ierr)
        call MPI_TYPE_COMMIT(decomp%ytype_x1y1(k),ierr) 
      enddo
    endif
    
    ! y1 <-> z1
    if(decomp%initialize(DECOMP_TRANSPOSE_Y1Z1)) then
      allocate(decomp%ytype_y1z1(p_col),stat=ierr)
      if(ierr/=0) call decomp_2d_abort(ierr,'decomp_info_init- Allocate Wrong 4 !!!')
      sizes=decomp%y1sz
      subsizes=decomp%y1sz
      starts=0; istart=0
      do k=1,p_col
        ...
        istart=istart+ksize
        call MPI_TYPE_CREATE_SUBARRAY(3,sizes,subsizes,starts,MPI_ORDER_FORTRAN, &
          real_type,decomp%ytype_y1z1(k),ierr)
        call MPI_TYPE_COMMIT(decomp%ytype_y1z1(k),ierr) 
      enddo
      allocate(decomp%ztype_y1z1(p_col),stat=ierr)
      if(ierr/=0) call decomp_2d_abort(ierr,'decomp_info_init- Allocate Wrong 5 !!!')
      sizes=decomp%z1sz
      ...
      do k=1,p_col
        ...
        istart=istart+ksize
        call MPI_TYPE_CREATE_SUBARRAY(3,sizes,subsizes,starts,MPI_ORDER_FORTRAN, &
          real_type,decomp%ztype_y1z1(k),ierr)
        call MPI_TYPE_COMMIT(decomp%ztype_y1z1(k),ierr)       
      enddo
    endif 
        
    ! z1 <-> x2
    if(decomp%initialize(DECOMP_TRANSPOSE_Z1X2)) then
      allocate(decomp%ztype_z1x2(p_row),stat=ierr)
      if(ierr/=0) call decomp_2d_abort(ierr,'decomp_info_init- Allocate Wrong 6 !!!')
      sizes=decomp%z1sz
      subsizes=decomp%z1sz
      starts=0; istart=0
      do k=1,p_row
        ...
        istart=istart+ksize  
        call MPI_TYPE_CREATE_SUBARRAY(3,sizes,subsizes,starts,MPI_ORDER_FORTRAN, &
          real_type,decomp%ztype_z1x2(k),ierr)
        call MPI_TYPE_COMMIT(decomp%ztype_z1x2(k),ierr) 
      enddo
      allocate(decomp%xtype_z1x2(p_row),stat=ierr)
      if(ierr/=0) call decomp_2d_abort(ierr,'decomp_info_init- Allocate Wrong 7 !!!')
      sizes=decomp%x2sz
      ...
      do k=1,p_row
        ...
        istart=istart+ksize
        call MPI_TYPE_CREATE_SUBARRAY(3,sizes,subsizes,starts,MPI_ORDER_FORTRAN, &
          real_type,decomp%xtype_z1x2(k),ierr)
        call MPI_TYPE_COMMIT(decomp%xtype_z1x2(k),ierr)      
      enddo
    endif
    
    ! x2 <-> y2
    if(decomp%initialize(DECOMP_TRANSPOSE_X2Y2)) then
      allocate(decomp%xtype_x2y2(p_col),stat=ierr)
      if(ierr/=0) call decomp_2d_abort(ierr,'decomp_info_init- Allocate Wrong 8 !!!')
      sizes=decomp%x2sz
      subsizes=decomp%x2sz
      starts=0; istart=0
      do k=1,p_col
        ...
        istart=istart+ksize
        call MPI_TYPE_CREATE_SUBARRAY(3,sizes,subsizes,starts,MPI_ORDER_FORTRAN, &
          real_type,decomp%xtype_x2y2(k),ierr)
        call MPI_TYPE_COMMIT(decomp%xtype_x2y2(k),ierr)       
      enddo
      allocate(decomp%ytype_x2y2(p_col),stat=ierr)
      if(ierr/=0) call decomp_2d_abort(ierr,'decomp_info_init- Allocate Wrong 9 !!!')
      sizes=decomp%y2sz
      ...
      do k=1,p_col
        ...
        istart=istart+ksize
        call MPI_TYPE_CREATE_SUBARRAY(3,sizes,subsizes,starts,MPI_ORDER_FORTRAN, &
          real_type,decomp%ytype_x2y2(k),ierr)
        call MPI_TYPE_COMMIT(decomp%ytype_x2y2(k),ierr)       
      enddo 
    endif
       
    ! y2 <-> z2
    if(decomp%initialize(DECOMP_TRANSPOSE_Y2Z2)) then
      allocate(decomp%ytype_y2z2(p_row),stat=ierr)
      if(ierr/=0) call decomp_2d_abort(ierr,'decomp_info_init- Allocate Wrong 10 !!!')
      sizes=decomp%y2sz
      subsizes=decomp%y2sz
      starts=0; istart=0  
      do k=1,p_row
        ...
        istart=istart+ksize
        call MPI_TYPE_CREATE_SUBARRAY(3,sizes,subsizes,starts,MPI_ORDER_FORTRAN, &
          real_type,decomp%ytype_y2z2(k),ierr)
        call MPI_TYPE_COMMIT(decomp%ytype_y2z2(k),ierr)       
      enddo
      allocate(decomp%ztype_y2z2(p_row),stat=ierr)
      if(ierr/=0) call decomp_2d_abort(ierr,'decomp_info_init- Allocate Wrong 11 !!!')
      sizes=decomp%z2sz
      ...
      do k=1,p_row
        ...
        istart=istart+ksize
        call MPI_TYPE_CREATE_SUBARRAY(3,sizes,subsizes,starts,MPI_ORDER_FORTRAN, &
          real_type,decomp%ztype_y2z2(k),ierr)
        call MPI_TYPE_COMMIT(decomp%ztype_y2z2(k),ierr)       
      enddo 
    endif   
            
    ! z2 <-> x1
    if(decomp%initialize(DECOMP_TRANSPOSE_Z2X1)) then
      allocate(decomp%ztype_z2x1(p_col),stat=ierr)
      if(ierr/=0) call decomp_2d_abort(ierr,'decomp_info_init- Allocate Wrong 12 !!!')
      sizes=decomp%z2sz
      subsizes=decomp%z2sz
      starts=0; istart=0  
      do k=1,p_col
        ...
        istart=istart+ksize
        call MPI_TYPE_CREATE_SUBARRAY(3,sizes,subsizes,starts,MPI_ORDER_FORTRAN, &
          real_type,decomp%ztype_z2x1(k),ierr)
        call MPI_TYPE_COMMIT(decomp%ztype_z2x1(k),ierr)       
      enddo    
      allocate(decomp%xtype_z2x1(p_col),stat=ierr)
      if(ierr/=0) call decomp_2d_abort(ierr,'decomp_info_init- Allocate Wrong 13 !!!')
      sizes=decomp%x1sz
      subsizes=decomp%x1sz
      starts=0; istart=0   
      do k=1,p_col
        ...
        istart=istart+ksize
        call MPI_TYPE_CREATE_SUBARRAY(3,sizes,subsizes,starts,MPI_ORDER_FORTRAN, &
          real_type,decomp%xtype_z2x1(k),ierr)
        call MPI_TYPE_COMMIT(decomp%xtype_z2x1(k),ierr)       
      enddo
    endif
  ...
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine decomp_2d_finalize(); implicit none
    IsInitTopologyFinished=.false.
  ...
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine decomp_info_finalize(decomp); implicit none
    type(decomp_info),intent(INOUT)::decomp

    ! locals
    integer::k,p_row,p_col,ierr
    ...
    if(decomp%initialize(DECOMP_TRANSPOSE_X1Y1)) then
      do k=1,p_row
        call MPI_TYPE_FREE(decomp%xtype_x1y1(k),ierr) 
        call MPI_TYPE_FREE(decomp%ytype_x1y1(k),ierr) 
      enddo
    ...
    if(decomp%initialize(DECOMP_TRANSPOSE_Y1Z1)) then
      do k=1,p_col
        call MPI_TYPE_FREE(decomp%ytype_y1z1(k),ierr) 
        call MPI_TYPE_FREE(decomp%ztype_y1z1(k),ierr)
      enddo
    ...
    if(decomp%initialize(DECOMP_TRANSPOSE_Z1X2)) then
      do k=1,p_row
        call MPI_TYPE_FREE(decomp%ztype_z1x2(k),ierr)
        call MPI_TYPE_FREE(decomp%xtype_z1x2(k),ierr)     
      enddo
    ...
    if(decomp%initialize(DECOMP_TRANSPOSE_X2Y2)) then
      do k=1,p_col
        call MPI_TYPE_FREE(decomp%xtype_x2y2(k),ierr)       
        call MPI_TYPE_FREE(decomp%ytype_x2y2(k),ierr)       
      enddo 
    ...
    if(decomp%initialize(DECOMP_TRANSPOSE_Y2Z2)) then
      do k=1,p_row
        call MPI_TYPE_FREE(decomp%ytype_y2z2(k),ierr)
        call MPI_TYPE_FREE(decomp%ztype_y2z2(k),ierr)       
      enddo 
    ...
    if(decomp%initialize(DECOMP_TRANSPOSE_Z2X1)) then
      do k=1,p_col
        call MPI_TYPE_FREE(decomp%ztype_z2x1(k),ierr)
        call MPI_TYPE_FREE(decomp%xtype_z2x1(k),ierr)       
      enddo
      deallocate(decomp%ztype_z2x1,decomp%xtype_z2x1)
    endif

    if(.not.IsInitTopologyFinished) then
      call MPI_COMM_FREE(DECOMP_2D_COMM_ROW,ierr)
      call MPI_COMM_FREE(DECOMP_2D_COMM_COL,ierr)   
    endif
  ...
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine distribute(ndata,mproc,istart,iend); implicit none
  ...
  ! Error handling
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine decomp_2d_abort(errorcode,msg); implicit none
  ...
  ! findfactor
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine findfactor(num, factors, ifact, nfact); implicit none
    integer,intent(in)::num,ifact    
    integer,intent(out)::nfact,factors(ifact)
    ...
    enddo
    nfact=nfact-1
    if(factors(nfact)*factors(nfact)/=num) then
  ...
  ! transpose_x1_to_y1
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90  
  subroutine transpose_x1_to_y1(src, dst, opt_decomp); implicit none
    ...
    np=decomp%nrow
    allocate(tran_count(np),tran_disp(np),stat=ierr)
    if(ierr/=0) call decomp_2d_abort(1, 'transpose_x1_to_y1-allocate wrong')
    tran_count=1; tran_disp=0
    tran_count(pcoord(1)+1)=0
    
    call MPI_ALLTOALLW(src,tran_count,tran_disp,decomp%xtype_x1y1, &
                       dst,tran_count,tran_disp,decomp%ytype_x1y1, DECOMP_2D_COMM_COL, ierr)
  ...
  ! transpose_y1_to_x1
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine transpose_y1_to_x1(src, dst, opt_decomp); implicit none
    real(mytype),dimension(:,:,:),intent(in) ::src
    real(mytype),dimension(:,:,:),intent(out)::dst
    type(decomp_info),intent(in),optional,target::opt_decomp
    ...
    np=decomp%nrow
    allocate(tran_count(np),tran_disp(np),stat=ierr)
    if(ierr/=0) call decomp_2d_abort(2, 'transpose_y1_to_x1-allocate wrong')
    tran_count=1; tran_disp=0
    tran_count(pcoord(1)+1)=0
    
    call MPI_ALLTOALLW(src,tran_count,tran_disp,decomp%ytype_x1y1, &
                       dst,tran_count,tran_disp,decomp%xtype_x1y1, DECOMP_2D_COMM_COL, ierr)
  ...
  ! transpose_y1_to_z1
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine transpose_y1_to_z1(src, dst, opt_decomp); implicit none
    real(mytype),dimension(:,:,:),intent(in) ::src
    real(mytype),dimension(:,:,:),intent(out)::dst
    type(decomp_info),intent(in),optional,target::opt_decomp
    ...
    np=decomp%ncol 
    allocate(tran_count(np),tran_disp(np),stat=ierr)
    if(ierr/=0) call decomp_2d_abort(3, 'transpose_y1_to_z1-allocate wrong')
    tran_count=1; tran_disp=0
    tran_count(pcoord(2)+1)=0
    
    call MPI_ALLTOALLW(src,tran_count,tran_disp,decomp%ytype_y1z1, &
                       dst,tran_count,tran_disp,decomp%ztype_y1z1, DECOMP_2D_COMM_ROW, ierr)
  ...
  ! transpose_z1_to_y1
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine transpose_z1_to_y1(src, dst, opt_decomp); implicit none
    real(mytype),dimension(:,:,:),intent(in) ::src
    real(mytype),dimension(:,:,:),intent(out)::dst
    type(decomp_info),intent(in),optional,target::opt_decomp
    ...
    np=decomp%ncol
    allocate(tran_count(np),tran_disp(np),stat=ierr)
    if(ierr/=0) call decomp_2d_abort(4, 'transpose_z1_to_y1-allocate wrong')
    tran_count=1; tran_disp=0
    tran_count(pcoord(2)+1)=0
    
    call MPI_ALLTOALLW(src,tran_count,tran_disp,decomp%ztype_y1z1, &
                       dst,tran_count,tran_disp,decomp%ytype_y1z1, DECOMP_2D_COMM_ROW, ierr)
  ...
  ! transpose_z1_to_x2
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine transpose_z1_to_x2(src, dst, opt_decomp); implicit none
    real(mytype),dimension(:,:,:),intent(in) ::src
    real(mytype),dimension(:,:,:),intent(out)::dst
    type(decomp_info),intent(in),optional,target::opt_decomp
    ...
    np=decomp%nrow
    allocate(tran_count(np),tran_disp(np),stat=ierr)
    if(ierr/=0) call decomp_2d_abort(5, 'transpose_z1_to_x2-allocate wrong')
    tran_count=1; tran_disp=0
    tran_count(pcoord(1)+1)=0
    
    call MPI_ALLTOALLW(src,tran_count,tran_disp,decomp%ztype_z1x2, &
                       dst,tran_count,tran_disp,decomp%xtype_z1x2, DECOMP_2D_COMM_COL, ierr)
  ...
  ! transpose_x2_to_z1
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine transpose_x2_to_z1(src, dst, opt_decomp); implicit none
    real(mytype),dimension(:,:,:),intent(in) ::src
    real(mytype),dimension(:,:,:),intent(out)::dst
    type(decomp_info),intent(in),optional,target::opt_decomp
    ...
    np=decomp%nrow
    allocate(tran_count(np),tran_disp(np),stat=ierr)
    if(ierr/=0) call decomp_2d_abort(6, 'transpose_x2_to_z1-allocate wrong')
    tran_count=1; tran_disp=0
    tran_count(pcoord(1)+1)=0
    
    call MPI_ALLTOALLW(src,tran_count,tran_disp,decomp%xtype_z1x2, &
                       dst,tran_count,tran_disp,decomp%ztype_z1x2, DECOMP_2D_COMM_COL, ierr)
  ...
  ! transpose_x2_to_y2
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine transpose_x2_to_y2(src, dst, opt_decomp); implicit none
    real(mytype),dimension(:,:,:),intent(in) ::src
    real(mytype),dimension(:,:,:),intent(out)::dst
    type(decomp_info),intent(in),optional,target::opt_decomp
    ...
    np=decomp%ncol
    allocate(tran_count(np),tran_disp(np),stat=ierr)
    if(ierr/=0) call decomp_2d_abort(7, 'transpose_x2_to_y2-allocate wrong')
    tran_count=1; tran_disp=0
    tran_count(pcoord(2)+1)=0
    
    call MPI_ALLTOALLW(src,tran_count,tran_disp,decomp%xtype_x2y2, &
                       dst,tran_count,tran_disp,decomp%ytype_x2y2, DECOMP_2D_COMM_ROW, ierr)
  ...
  ! transpose_y2_to_x2
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine transpose_y2_to_x2(src, dst, opt_decomp); implicit none
    real(mytype),dimension(:,:,:),intent(in) ::src
    real(mytype),dimension(:,:,:),intent(out)::dst
    type(decomp_info),intent(in),optional,target::opt_decomp
    ...
    np=decomp%ncol
    allocate(tran_count(np),tran_disp(np),stat=ierr)
    if(ierr/=0) call decomp_2d_abort(8, 'transpose_y2_to_x2-allocate wrong')
    tran_count=1; tran_disp=0
    tran_count(pcoord(2)+1)=0
    
    call MPI_ALLTOALLW(src,tran_count,tran_disp,decomp%ytype_x2y2, &
                       dst,tran_count,tran_disp,decomp%xtype_x2y2, DECOMP_2D_COMM_ROW, ierr)
  ...
  ! transpose_y2_to_z2
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine transpose_y2_to_z2(src, dst, opt_decomp); implicit none
    real(mytype),dimension(:,:,:),intent(in) ::src
    real(mytype),dimension(:,:,:),intent(out)::dst
    type(decomp_info),intent(in),optional,target::opt_decomp
    ...
    np=decomp%nrow
    allocate(tran_count(np),tran_disp(np),stat=ierr)
    if(ierr/=0) call decomp_2d_abort(9, 'transpose_y2_to_z2-allocate wrong')
    tran_count=1; tran_disp=0
    tran_count(pcoord(1)+1)=0
    
    call MPI_ALLTOALLW(src,tran_count,tran_disp,decomp%ytype_y2z2, &
                       dst,tran_count,tran_disp,decomp%ztype_y2z2, DECOMP_2D_COMM_COL, ierr)
  ...
  ! transpose_z2_to_y2
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine transpose_z2_to_y2(src, dst, opt_decomp); implicit none
    real(mytype),dimension(:,:,:),intent(in) ::src
    real(mytype),dimension(:,:,:),intent(out)::dst
    type(decomp_info),intent(in),optional,target::opt_decomp
    ...
    np=decomp%nrow
    allocate(tran_count(np),tran_disp(np),stat=ierr)
    if(ierr/=0) call decomp_2d_abort(10, 'transpose_z2_to_y2-allocate wrong')
    tran_count=1; tran_disp=0
    tran_count(pcoord(1)+1)=0
    
    call MPI_ALLTOALLW(src,tran_count,tran_disp,decomp%ztype_y2z2, &
                       dst,tran_count,tran_disp,decomp%ytype_y2z2, DECOMP_2D_COMM_COL, ierr)
  ...
  ! transpose_z2_to_x1
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine transpose_z2_to_x1(src, dst, opt_decomp); implicit none
    real(mytype),dimension(:,:,:),intent(in) ::src
    real(mytype),dimension(:,:,:),intent(out)::dst
    type(decomp_info),intent(in),optional,target::opt_decomp
    ...
    np=decomp%ncol
    allocate(tran_count(np),tran_disp(np),stat=ierr)
    if(ierr/=0) call decomp_2d_abort(11, 'transpose_z2_to_x1-allocate wrong')
    tran_count=1; tran_disp=0
    tran_count(pcoord(2)+1)=0
    
    call MPI_ALLTOALLW(src,tran_count,tran_disp,decomp%ztype_z2x1, &
                       dst,tran_count,tran_disp,decomp%xtype_z2x1, DECOMP_2D_COMM_ROW, ierr)
  ...
  ! transpose_x1_to_z2
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine transpose_x1_to_z2(src, dst, opt_decomp); implicit none
    real(mytype),dimension(:,:,:),intent(in) ::src
    real(mytype),dimension(:,:,:),intent(out)::dst
    type(decomp_info),intent(in),optional,target::opt_decomp
    ...
    np=decomp%ncol
    allocate(tran_count(np),tran_disp(np),stat=ierr)
    if(ierr/=0) call decomp_2d_abort(12, 'transpose_x1_to_z2-allocate wrong')
    tran_count=1; tran_disp=0
    tran_count(pcoord(2)+1)=0
    
    call MPI_ALLTOALLW(src,tran_count,tran_disp,decomp%xtype_z2x1, &
                       dst,tran_count,tran_disp,decomp%ztype_z2x1, DECOMP_2D_COMM_ROW, ierr)
  ...
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine decomp_2d_write_var(fh,disp,ipencil,var,opt_decomp); implicit none
    ...
    ! locals
    type(decomp_info),pointer::decomp
    integer::ierr,data_type,newtype
    ...
    call MPI_FILE_SET_VIEW(fh,disp,data_type,newtype,'native',MPI_INFO_NULL,ierr)
    call MPI_FILE_WRITE_ALL(fh,var,subsizes(1)*subsizes(2)*subsizes(3),data_type,MPI_STATUS_IGNORE,ierr)
  ...
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine decomp_2d_read_var(fh,disp,ipencil,var,opt_decomp); implicit none
    integer,intent(in)::fh
    integer,intent(in)::ipencil
    integer(MPI_OFFSET_KIND),intent(INOUT)::disp
    ...
    call MPI_TYPE_COMMIT(newtype,ierr)
    call MPI_FILE_SET_VIEW(fh,disp,data_type,newtype,'native',MPI_INFO_NULL,ierr)
    call MPI_FILE_READ_ALL(fh,var,subsizes(1)*subsizes(2)*subsizes(3),data_type,MPI_STATUS_IGNORE,ierr)
    call MPI_TYPE_FREE(newtype,ierr)
    disp=disp + int(sizes(1),8)*int(sizes(2),8)*int(sizes(3),8)*int(mytype_bytes,8)  ! Update displacement
  end subroutine decomp_2d_read_var

  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  ! decomp_2d_write_plane_reduce
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine decomp_2d_write_plane_reduce(var,ipencil,iplane,n3,reduce_type,filename,opt_decomp); implicit none
    real(mytype),dimension(:,:,:),intent(inout) :: var
    integer,intent(in)::ipencil     ! x1-pencil=1; y1-pencil=2; z1-pencil=3; x2-pencil=4; y2-pencil=5; z2-pencil=6
    integer,intent(in)::iplane      ! x-plane= 1; y-plane= 2; z-plane= 3
    integer,intent(in)::n3          ! size of 3rd dimension 
    integer,intent(in)::reduce_type ! MPI_MAX, MPI_MIN, MPI_SUM
    ...
    integer::sizes(3),subsizes(3),starts(3), newtype
    integer::comm_reduce, comm_write, nrank_rc,iUnit, ierr

    if(iplane<1 .or. iplane>3) then
      call decomp_2d_abort(17, 'decomp_2d_write_plane_reduce: iplane wrong')
    endif    
    if(ipencil<1 .or. ipencil>6) then
      call decomp_2d_abort(17, 'decomp_2d_write_plane_reduce: ipencil wrong')
    endif
    if(reduce_type /= MPI_MAX .and. reduce_type /= MPI_MIN .and. reduce_type /= MPI_SUM) then
      call decomp_2d_abort(17, 'decomp_2d_write_plane_reduce: reduce_type wrong')
    endif
    ...
    subsizes = 0
    starts   = 0
    comm_reduce = DECOMP_2D_COMM_ROW
    comm_write  = DECOMP_2D_COMM_COL
    SELECT CASE(ipencil)
    CASE(1) ! x1-pencil
      ! TODO
    CASE(2) ! y1-pencil
      IF(iplane==1) THEN
        comm_reduce = DECOMP_2D_COMM_COL
        comm_write  = DECOMP_2D_COMM_ROW
        sizes    = [decomp%y1sz(2), decomp%z1sz(3),   n3]
        subsizes = [decomp%y1sz(2), decomp%y1sz(3),   n3]
        starts   = [0,              decomp%y1st(3)-1,  0]
      ELSEIF(iplane==2) THEN
        comm_reduce = MPI_COMM_NULL
        comm_write  = MPI_COMM_WORLD
        sizes    = [decomp%x1sz(1),   decomp%z1sz(3), n3]
        subsizes = [decomp%y1sz(1),   decomp%y1sz(3), n3]
        starts   = [decomp%y1st(1)-1, decomp%y1st(3)-1,0]
      ELSEIF(iplane==3) THEN
        comm_reduce = DECOMP_2D_COMM_ROW
        comm_write  = DECOMP_2D_COMM_COL
        sizes    = [decomp%x1sz(1),   decomp%y1sz(2), n3]
        subsizes = [decomp%y1sz(1),   decomp%y1sz(2), n3]
        starts   = [decomp%y1st(1)-1, 0,               0]
      ENDIF
    CASE(3) ! z1-pencil
      ! TODO
    CASE(4) ! x2-pencil
    ...
    if(size(var,1) /= subsizes(1) .or. size(var,2) /= subsizes(2) .or. size(var,3) /= subsizes(3)) then
      call decomp_2d_abort(17, 'decomp_2d_write_plane_reduce: var size wrong')
    endif
    
    nrank_rc = 0
    IF(comm_reduce /= MPI_COMM_NULL) THEN
      allocate(wk2d(subsizes(1), subsizes(2)),stat=ierr)
      if(ierr/=0) call decomp_2d_abort(17, 'decomp_2d_write_plane_reduce: allocate wrong')
      do iUnit=1,n3
        call MPI_REDUCE(var(:,:,iUnit),wk2d, subsizes(1)*subsizes(2), real_type,reduce_type,0,comm_reduce,ierr)
        var(:,:,iUnit) = wk2d
      enddo      
      deallocate(wk2d, stat=ierr)
      call MPI_COMM_RANK(comm_reduce, nrank_rc, ierr)
    ENDIF
    IF(nrank_rc == 0) THEN
      call MPI_FILE_OPEN(comm_write, filename, MPI_MODE_CREATE+MPI_MODE_WRONLY, MPI_INFO_NULL, iUnit, ierr)
      call MPI_BARRIER(comm_write, ierr)
      call my_mpi_file_set_size(iUnit,0_MPI_OFFSET_KIND,ierr)  ! guarantee overwriting
      call MPI_BARRIER(comm_write, ierr)
      call MPI_TYPE_CREATE_SUBARRAY(3,sizes,subsizes,starts,MPI_ORDER_FORTRAN, real_type, newtype,ierr)
      call MPI_TYPE_COMMIT(newtype,ierr)
      call MPI_FILE_SET_VIEW(iUnit,0_MPI_OFFSET_KIND,real_type,newtype,'native',MPI_INFO_NULL,ierr)  
      call MPI_FILE_WRITE_ALL(iUnit, var, subsizes(1)*subsizes(2)*subsizes(3), real_type, MPI_STATUS_IGNORE,ierr)
      call MPI_TYPE_FREE(newtype,ierr)
      call MPI_FILE_CLOSE(iUnit,ierr)
    ENDIF
    call MPI_BARRIER(MPI_COMM_WORLD, ierr)
  end subroutine decomp_2d_write_plane_reduce
  ...
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine decomp_2d_write_plane_3d(var,ipencil,iplane,n,filename,opt_decomp); implicit none
    ...
    integer::i,j,k,ierr,newtype,fh,data_type
    real(mytype),allocatable,dimension(:,:,:)::wk3d
    real(mytype_save),allocatable,dimension(:,:,:)::wk2d
    
    if(iplane<1 .or. iplane>3) then
      call decomp_2d_abort(13, 'decomp_2d_write_plane_3d: iplane wrong')
    endif
    if(ipencil<1 .or. ipencil>6) then
      call decomp_2d_abort(13, 'decomp_2d_write_plane_3d: ipencil wrong')
    endif
    ...
    IF(iplane==1) THEN
      IF(ipencil==1) THEN ! x1-pencil
        allocate(wk2d(1,decomp%x1sz(2),decomp%x1sz(3)),stat=ierr)
        if(ierr/=0) call decomp_2d_abort(13, 'decomp_2d_write_plane_3d: allocate wrong-01')
      ...
      ELSEIF(ipencil==2) THEN ! y1-pencil
        allocate(wk2d(1,decomp%x1sz(2),decomp%x1sz(3)),stat=ierr)
        if(ierr/=0) call decomp_2d_abort(13, 'decomp_2d_write_plane_3d: allocate wrong-02')        
        allocate(wk3d(decomp%x1sz(1),decomp%x1sz(2),decomp%x1sz(3)),stat=ierr)
        if(ierr/=0) call decomp_2d_abort(13, 'decomp_2d_write_plane_3d: allocate wrong-03')
        ...
        subsizes=[1,decomp%x1sz(2),decomp%x1sz(3)]
        starts  =[1,decomp%x1st(2),decomp%x1st(3)]-1
      ELSEIF(ipencil==3) THEN ! z1-pencil
        allocate(wk2d(1,decomp%x2sz(2),decomp%x2sz(3)),stat=ierr)
        if(ierr/=0) call decomp_2d_abort(13, 'decomp_2d_write_plane_3d: allocate wrong-04')      
        allocate(wk3d(decomp%x2sz(1),decomp%x2sz(2),decomp%x2sz(3)),stat=ierr)
        if(ierr/=0) call decomp_2d_abort(13, 'decomp_2d_write_plane_3d: allocate wrong-05')
      ...
      ELSEIF(ipencil==4) THEN ! x2-pencil
        allocate(wk2d(1,decomp%x2sz(2),decomp%x2sz(3)),stat=ierr)
        if(ierr/=0) call decomp_2d_abort(13, 'decomp_2d_write_plane_3d: allocate wrong-06')
        do k=1,decomp%x2sz(3)
          do j=1,decomp%x2sz(2)
            wk2d(1,j,k)=real(var(n,j,k),mytype_save)
          enddo
        ...
        subsizes=[1,decomp%x2sz(2),decomp%x2sz(3)]
        starts  =[1,decomp%x2st(2),decomp%x2st(3)]-1
      ELSEIF(ipencil==5) THEN ! y2-pencil
        allocate(wk2d(1,decomp%x2sz(2),decomp%x2sz(3)),stat=ierr)
        if(ierr/=0) call decomp_2d_abort(13, 'decomp_2d_write_plane_3d: allocate wrong-07')        
        allocate(wk3d(decomp%x2sz(1),decomp%x2sz(2),decomp%x2sz(3)),stat=ierr)
        if(ierr/=0) call decomp_2d_abort(13, 'decomp_2d_write_plane_3d: allocate wrong-08')
        call transpose_y2_to_x2(var,wk3d,decomp)
        do k=1,decomp%x2sz(3)
          do j=1,decomp%x2sz(2)
            wk2d(1,j,k)=real(wk3d(n,j,k),mytype_save)
          enddo
        ...
        subsizes=[1,decomp%x2sz(2),decomp%x2sz(3)]
        starts  =[1,decomp%x2st(2),decomp%x2st(3)]-1
      ELSEIF(ipencil==6) THEN ! z2-pencil
        allocate(wk2d(1,decomp%x1sz(2),decomp%x1sz(3)),stat=ierr)
        if(ierr/=0) call decomp_2d_abort(13, 'decomp_2d_write_plane_3d: allocate wrong-09')      
        allocate(wk3d(decomp%x1sz(1),decomp%x1sz(2),decomp%x1sz(3)),stat=ierr)
        if(ierr/=0) call decomp_2d_abort(13, 'decomp_2d_write_plane_3d: allocate wrong-10')
        call transpose_z2_to_x1(var,wk3d,decomp)
        do k=1,decomp%x1sz(3)
          do j=1,decomp%x1sz(2)
            wk2d(1,j,k)=real(wk3d(n,j,k),mytype_save)
          enddo
    ...
    ELSEIF(iplane==2) THEN
      IF(ipencil==1) THEN ! x1-pencil
        allocate(wk2d(decomp%y1sz(1),1,decomp%y1sz(3)),stat=ierr)
        if(ierr/=0) call decomp_2d_abort(13, 'decomp_2d_write_plane_3d: allocate wrong-11')
        allocate(wk3d(decomp%y1sz(1),decomp%y1sz(2),decomp%y1sz(3)),stat=ierr)
        if(ierr/=0) call decomp_2d_abort(13, 'decomp_2d_write_plane_3d: allocate wrong-12')
      ...
      ELSEIF(ipencil==2) THEN ! y1-pencil
        allocate(wk2d(decomp%y1sz(1),1,decomp%y1sz(3)),stat=ierr)
        if(ierr/=0) call decomp_2d_abort(13, 'decomp_2d_write_plane_3d: allocate wrong-13')
        do k=1,decomp%y1sz(3)
          do i=1,decomp%y1sz(1)
      ...
      ELSEIF(ipencil==3) THEN ! z1-pencil
        allocate(wk2d(decomp%y1sz(1),1,decomp%y1sz(3)),stat=ierr)
        if(ierr/=0) call decomp_2d_abort(13, 'decomp_2d_write_plane_3d: allocate wrong-14')
        allocate(wk3d(decomp%y1sz(1),decomp%y1sz(2),decomp%y1sz(3)),stat=ierr)
        if(ierr/=0) call decomp_2d_abort(13, 'decomp_2d_write_plane_3d: allocate wrong-15')
        ...
        starts  =[decomp%y1st(1),1,decomp%y1st(3)]-1
      ELSEIF(ipencil==4) THEN ! x2-pencil
        allocate(wk2d(decomp%y2sz(1),1,decomp%y2sz(3)),stat=ierr)
        if(ierr/=0) call decomp_2d_abort(13, 'decomp_2d_write_plane_3d: allocate wrong-16')
        allocate(wk3d(decomp%y2sz(1),decomp%y2sz(2),decomp%y2sz(3)),stat=ierr)
        if(ierr/=0) call decomp_2d_abort(13, 'decomp_2d_write_plane_3d: allocate wrong-17')
        call transpose_x2_to_y2(var,wk3d,decomp)
        do k=1,decomp%y2sz(3)
          do i=1,decomp%y2sz(1)
            wk2d(i,1,k)=real(wk3d(i,n,k),mytype_save)
          enddo
        enddo
        deallocate(wk3d)
        sizes   =[decomp%x1sz(1),1,decomp%z1sz(3)]
        subsizes=[decomp%y2sz(1),1,decomp%y2sz(3)]
        starts  =[decomp%y2st(1),1,decomp%y2st(3)]-1
      ELSEIF(ipencil==5) THEN ! y2-pencil
        allocate(wk2d(decomp%y2sz(1),1,decomp%y2sz(3)),stat=ierr)
        if(ierr/=0) call decomp_2d_abort(13, 'decomp_2d_write_plane_3d: allocate wrong-18')
        do k=1,decomp%y2sz(3)
          do i=1,decomp%y2sz(1)
            wk2d(i,1,k)=real(var(i,n,k),mytype_save)
          enddo
        ...
        subsizes=[decomp%y2sz(1),1,decomp%y2sz(3)]
        starts  =[decomp%y2st(1),1,decomp%y2st(3)]-1
      ELSEIF(ipencil==3) THEN ! z2-pencil
        allocate(wk2d(decomp%y2sz(1),1,decomp%y2sz(3)),stat=ierr)
        if(ierr/=0) call decomp_2d_abort(13, 'decomp_2d_write_plane_3d: allocate wrong-19')
        allocate(wk3d(decomp%y2sz(1),decomp%y2sz(2),decomp%y2sz(3)),stat=ierr)
        if(ierr/=0) call decomp_2d_abort(13, 'decomp_2d_write_plane_3d: allocate wrong-20')
        call transpose_z2_to_y2(var,wk3d,decomp)
        do k=1,decomp%y2sz(3)
          do i=1,decomp%y2sz(1)
            wk2d(i,1,k)=real(wk3d(i,n,k),mytype_save)
          enddo
    ...
    ELSEIF(iplane==3) THEN ! iplane=3
      IF(ipencil==1) THEN  ! x1-pencil
        allocate(wk2d(decomp%z2sz(1),decomp%z2sz(2),1),stat=ierr)
        if(ierr/=0) call decomp_2d_abort(13, 'decomp_2d_write_plane_3d: allocate wrong-21')
        allocate(wk3d(decomp%z2sz(1),decomp%z2sz(2),decomp%z2sz(3)),stat=ierr)
        if(ierr/=0) call decomp_2d_abort(13, 'decomp_2d_write_plane_3d: allocate wrong-22')
      ...
      ELSEIF(ipencil==2) THEN  ! y1-pencil
        allocate(wk2d(decomp%z1sz(1),decomp%z1sz(2),1),stat=ierr)
        if(ierr/=0) call decomp_2d_abort(13, 'decomp_2d_write_plane_3d: allocate wrong-23')
        allocate(wk3d(decomp%z1sz(1),decomp%z1sz(2),decomp%z1sz(3)),stat=ierr)
        if(ierr/=0) call decomp_2d_abort(13, 'decomp_2d_write_plane_3d: allocate wrong-24')
      ...
      ELSEIF(ipencil==3) THEN  ! z1-pencil
        allocate(wk2d(decomp%z1sz(1),decomp%z1sz(2),1),stat=ierr)
        if(ierr/=0) call decomp_2d_abort(13, 'decomp_2d_write_plane_3d: allocate wrong-25')
        do j=1,decomp%z1sz(2)
          do i=1,decomp%z1sz(1) 
      ...
      ELSEIF(ipencil==4) THEN  ! x2-pencil
        allocate(wk2d(decomp%z1sz(1),decomp%z1sz(2),1),stat=ierr)
        if(ierr/=0) call decomp_2d_abort(13, 'decomp_2d_write_plane_3d: allocate wrong-26')
        allocate(wk3d(decomp%z1sz(1),decomp%z1sz(2),decomp%z1sz(3)),stat=ierr)
        if(ierr/=0) call decomp_2d_abort(13, 'decomp_2d_write_plane_3d: allocate wrong-27')
        call transpose_x2_to_z1(var,wk3d,decomp)
        do j=1,decomp%z1sz(2)
          do i=1,decomp%z1sz(1) 
            wk2d(i,j,1)=real(wk3d(i,j,n),mytype_save)
          enddo
      ...
      ELSEIF(ipencil==5) THEN  ! y2-pencil
        allocate(wk2d(decomp%z2sz(1),decomp%z2sz(2),1),stat=ierr)
        if(ierr/=0) call decomp_2d_abort(13, 'decomp_2d_write_plane_3d: allocate wrong-28')
        allocate(wk3d(decomp%z2sz(1),decomp%z2sz(2),decomp%z2sz(3)),stat=ierr)
        if(ierr/=0) call decomp_2d_abort(13, 'decomp_2d_write_plane_3d: allocate wrong-29')
        call transpose_y2_to_z2(var,wk3d,decomp)
        do j=1,decomp%z2sz(2)
          do i=1,decomp%z2sz(1) 
            wk2d(i,j,1)=real(wk3d(i,j,n),mytype_save)
          enddo
      ...
      ELSEIF(ipencil==6) THEN  ! z2-pencil
        allocate(wk2d(decomp%z2sz(1),decomp%z2sz(2),1),stat=ierr)
        if(ierr/=0) call decomp_2d_abort(13, 'decomp_2d_write_plane_3d: allocate wrong-30')
        do j=1,decomp%z2sz(2)
          do i=1,decomp%z2sz(1) 
            wk2d(i,j,1)=real(var(i,j,n),mytype_save)
          enddo
    ...
    call MPI_BARRIER(MPI_COMM_WORLD,ierr)
    call MPI_FILE_SET_VIEW(fh,0_MPI_OFFSET_KIND,data_type,newtype,'native',MPI_INFO_NULL,ierr)
    call MPI_FILE_WRITE_ALL(fh,wk2d,subsizes(1)*subsizes(2)*subsizes(3),data_type, MPI_STATUS_IGNORE, ierr)
    call MPI_FILE_CLOSE(fh,ierr)
    call MPI_TYPE_FREE(newtype,ierr)
    deallocate(wk2d)
  end subroutine decomp_2d_write_plane_3d

  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  ! Write 3D array data for every specified mesh point
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine decomp_2d_write_every(ipencil,var,iskip,jskip,kskip,filename,from1); implicit none
    integer,intent(in)::ipencil
    ...
    integer::i,j,k,id,jd,kd,ierr,newtype,fh,key,color,newcomm,data_type
    ...
    IF(ipencil==1)THEN
      allocate(wk(xst(1):xen(1),xst(2):xen(2),xst(3):xen(3)),stat=ierr)
      if(ierr/=0) call decomp_2d_abort(14, 'decomp_2d_write_every-allocate wrong-1')
      if(from1)then
        do k=xst(3),xen(3)
    ...
    ELSEIF(ipencil==2)THEN
      allocate(wk(yst(1):yen(1),yst(2):yen(2),yst(3):yen(3)),stat=ierr)
      if(ierr/=0) call decomp_2d_abort(14, 'decomp_2d_write_every-allocate wrong-2')
      if(from1)then
        do k=yst(3),yen(3)
    ...
    ELSEIF(ipencil==3)THEN
      allocate(wk(zst(1):zen(1),zst(2):zen(2),zst(3):zen(3)),stat=ierr)
      if(ierr/=0) call decomp_2d_abort(14, 'decomp_2d_write_every-allocate wrong-3')
      if(from1)then
        do k=zst(3),zen(3)
    ...
    call MPI_TYPE_CREATE_SUBARRAY(3,sizes,subsizes,starts,MPI_ORDER_FORTRAN, data_type, newtype, ierr)
    call MPI_TYPE_COMMIT(newtype,ierr)
    call MPI_FILE_OPEN(newcomm,filename,MPI_MODE_CREATE+MPI_MODE_WRONLY,MPI_INFO_NULL,fh,ierr)
    call MPI_BARRIER(newcomm,ierr)
    call my_mpi_file_set_size(fh,0_MPI_OFFSET_KIND,ierr)  ! guarantee overwriting
    call MPI_BARRIER(newcomm,ierr)
    call MPI_FILE_SET_VIEW(fh,0_MPI_OFFSET_KIND,data_type,newtype,'native',MPI_INFO_NULL,ierr)
    call MPI_FILE_WRITE_ALL(fh,wk,subsizes(1)*subsizes(2)*subsizes(3),data_type,MPI_STATUS_IGNORE,ierr)
    call MPI_FILE_CLOSE(fh,ierr)
    call MPI_TYPE_FREE(newtype,ierr)
  ...
  ! Init_ProcNeighbour
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine Init_ProcNeighbour(row,col,BcOpt); implicit none
    ...
    ! 
    !     The arrangement of the subomains(nrank) is as follow:
    !       y             x             x             y              x              x
    !       |       4 5   |       4 5   |       4 5   |              |              |
  ...
  ! myallocate
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine myallocate(var, mb, opt_decomp, opt_global); implicit none
    ...
    allocate(var(mb%xmm:mb%xpm,mb%ymm:mb%ypm,mb%zmm:mb%zpm),stat=ierr)
    if(ierr/=0) call decomp_2d_abort(15,'myallocate-allocate wrong')
  end subroutine myallocate

  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  ! best_2d_grid
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine best_2d_grid(nx,ny,nz,pencil,iproc,best_p_row,best_p_col,BcOptions); implicit none
    ...
    integer::nfact,i,k,row,col,ierr,errorcode
    ...
    allocate(factors(i),stat=ierr)
    if(ierr/=0) call decomp_2d_abort(16,'best_2d_grid-allocate wrong-1')
    call findfactor(iproc, factors,i, nfact)
    ...
    do i=1,nfact
      ...
      if(min(nx,ny,nz)>=row .and. min(ny,nz,nz)>=col) then
        ...
        allocate(Arr1(decomp%x1sz(1),decomp%x1sz(2),decomp%x1sz(3)),stat=ierr)
        if(ierr/=0) call decomp_2d_abort(16,'best_2d_grid-allocate wrong-2')
        allocate(Arr2(decomp%y1sz(1),decomp%y1sz(2),decomp%y1sz(3)),stat=ierr)
        if(ierr/=0) call decomp_2d_abort(16,'best_2d_grid-allocate wrong-3')
        call MPI_BARRIER(MPI_COMM_WORLD,ierr)
        t1 = MPI_WTIME()
        do k=1,5
        ...
        deallocate(Arr1)
        allocate(Arr1(decomp%z1sz(1),decomp%z1sz(2),decomp%z1sz(3)),stat=ierr)
        if(ierr/=0) call decomp_2d_abort(16,'best_2d_grid-allocate wrong-4')        
        call MPI_BARRIER(MPI_COMM_WORLD,ierr)
        t1 = MPI_WTIME()
        do k=1,5
        ...
        deallocate(Arr2)     
        allocate(Arr2(decomp%x2sz(1),decomp%x2sz(2),decomp%x2sz(3)),stat=ierr)
        if(ierr/=0) call decomp_2d_abort(16,'best_2d_grid-allocate wrong-5')
        call MPI_BARRIER(MPI_COMM_WORLD,ierr)
        t1 = MPI_WTIME()
        do k=1,5
        ...
        deallocate(Arr1)     
        allocate(Arr1(decomp%y2sz(1),decomp%y2sz(2),decomp%y2sz(3)),stat=ierr)
        if(ierr/=0) call decomp_2d_abort(16,'best_2d_grid-allocate wrong-6')
        call MPI_BARRIER(MPI_COMM_WORLD,ierr)
        t1 = MPI_WTIME()
        do k=1,5
        ...
        deallocate(Arr2)     
        allocate(Arr2(decomp%z2sz(1),decomp%z2sz(2),decomp%z2sz(3)),stat=ierr)
        if(ierr/=0) call decomp_2d_abort(16,'best_2d_grid-allocate wrong-7')
        call MPI_BARRIER(MPI_COMM_WORLD,ierr)
        t1 = MPI_WTIME()
        do k=1,5
        ...
        deallocate(Arr1)     
        allocate(Arr1(decomp%x1sz(1),decomp%x1sz(2),decomp%x1sz(3)),stat=ierr)
        if(ierr/=0) call decomp_2d_abort(16,'best_2d_grid-allocate wrong-8')
        call MPI_BARRIER(MPI_COMM_WORLD,ierr)
        t1 = MPI_WTIME()
        do k=1,5
        ...
        t1 = MPI_WTIME()
        do k=1,40
          call update_halo(Arr1,mb_test,hi_test)
        enddo
        t2 = MPI_WTIME() - t1 + t2
        
        deallocate(Arr1)
        call decomp_info_finalize(decomp)
        call MPI_ALLREDUCE(t2,t1,1,real_type,MPI_SUM,MPI_COMM_WORLD,ierr)
  ...
  ! update_halo
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine update_halo(mat,mb,hi); implicit none
    ...
    ! locals
    integer::is1,is2,js1,js2,ks1,ks2   ! index for halo sending
    integer::ir1,ir2,jr1,jr2,kr1,kr2   ! index for halo receiving
    integer::jc,data_type,halo_type,ierr,ProcNgh(4) 
    integer::icount,ilength,ijump,SRstatus(MPI_STATUS_SIZE)
  
    data_type=real_type
    if(mb%pencil /= hi%pencil ) then
      ierr = 14
      call decomp_2d_abort(ierr,'update_halo wrong: pencil error, Gong Zheng')
    endif
    if(mb%xme<hi%xmh .or. mb%yme<hi%ymh .or. mb%zme<hi%zmh .or.  &
      ...
      ierr = 14
      call decomp_2d_abort(ierr,'update_halo wrong: HaloSize error, Gong Zheng')
    endif

    do jc=1,4
      if(myProcNghBC(mb%pencil,jc)<0) then
        ProcNgh(jc)=MPI_PROC_NULL
      else
        ProcNgh(jc)=myProcNghBC(mb%pencil,jc)
      endif
    enddo
    ...
    CASE(2)  ! y-pencil
      ...
      IF(hi%ymh >0 ) THEN
        ir1=mb%xmr;             ir2=mb%xpr
        jr1=mb%ymr-hi%ymh;      jr2=mb%ymr-1  
        kr1=mb%zmr;             kr2=mb%zpr

        is1=ir1;                is2=ir2
        js1=mb%ypr-hi%ymh+1;    js2=mb%ypr    
        ks1=kr1;                ks2=kr2
        mat(ir1:ir2, jr1:jr2, kr1:kr2) = mat(is1:is2, js1:js2, ks1:ks2)
      ENDIF

      ! step2: receive from yp_dir, and send to ym_dir
      IF(hi%yph >0 ) THEN
        ir1=mb%xmr;             ir2=mb%xpr 
        jr1=mb%ypr+1;           jr2=mb%ypr+hi%yph  
        kr1=mb%zmr;             kr2=mb%zpr

        is1=ir1;                is2=ir2
        js1=mb%ymr;             js2=mb%ymr+hi%yph-1   
        ks1=kr1;                ks2=kr2
        mat(ir1:ir2, jr1:jr2, kr1:kr2) = mat(is1:is2, js1:js2, ks1:ks2)
      ENDIF
      ...
      IF(hi%xmh > 0) THEN
        ir1=mb%xmr-hi%xmh;      ir2=mb%xmr-1
        jr1=mb%ymm;             jr2=mb%ypm
        kr1=mb%zmr;             kr2=mb%zpr
        
        is1=mb%xpr-hi%xmh+1;    is2=mb%xpr
        js1=jr1;                js2=jr2    
        ks1=kr1;                ks2=kr2
        if(ProcNgh(3)==nrank) then  ! neighbour is nrank itself, and ProcNgh(4)==nrank also !!!
          mat(ir1:ir2, jr1:jr2, kr1:kr2) = mat(is1:is2, js1:js2, ks1:ks2)
        else
          icount =(mb%ypm-mb%ymm+1)*(mb%zpr-mb%zmr+1)
          ilength= hi%xmh
          ijump  = mb%xpm-mb%xmm+1
          call MPI_TYPE_VECTOR(icount,ilength,ijump, data_type, halo_type, ierr)
          call MPI_TYPE_COMMIT(halo_type, ierr)
          call MPI_SENDRECV(mat(is1,js1,ks1),1,halo_type,ProcNgh(3),1, &
                            mat(ir1,jr1,kr1),1,halo_type,ProcNgh(4),1, MPI_COMM_WORLD,SRstatus,ierr)
      ...
      IF(hi%xph > 0) THEN
        ir1=mb%xpr+1;           ir2=mb%xpr+hi%xph
        jr1=mb%ymm;             jr2=mb%ypm
        kr1=mb%zmr;             kr2=mb%zpr
        
        is1=mb%xmr;             is2=mb%xmr+hi%xph-1
        js1=jr1;                js2=jr2    
        ks1=kr1;                ks2=kr2
        if(ProcNgh(4)==nrank) then  ! neighbour is nrank itself, and ProcNgh(3)==nrank also !!!
          ...
          call MPI_TYPE_VECTOR(icount, ilength, ijump, data_type, halo_type, ierr)
          call MPI_TYPE_COMMIT(halo_type, ierr)
          call MPI_SENDRECV(mat(is1,js1,ks1),1,halo_type,ProcNgh(4),2, &
                            mat(ir1,jr1,kr1),1,halo_type,ProcNgh(3),2, MPI_COMM_WORLD,SRstatus,ierr)
          call MPI_TYPE_FREE(halo_type, ierr)
        endif
      ENDIF
      ...
      IF(hi%zmh > 0) THEN
        ir1=mb%xmm;             ir2=mb%xpm
        jr1=mb%ymm;             jr2=mb%ypm
        kr1=mb%zmr-hi%zmh;      kr2=mb%zmr-1
        
        is1=ir1;                is2=ir2
        js1=jr1;                js2=jr2    
        ks1=mb%zpr-hi%zmh+1;    ks2=mb%zpr
        if(ProcNgh(1)==nrank) then  ! neighbour is nrank itself, and ProcNgh(2)==nrank also !!!
          mat(ir1:ir2, jr1:jr2, kr1:kr2) = mat(is1:is2, js1:js2, ks1:ks2)
        else
          icount=(mb%xpm-mb%xmm+1)*(mb%ypm-mb%ymm+1)*hi%zmh
          call MPI_SENDRECV(mat(is1,js1,ks1),icount,data_type,ProcNgh(1),3, &
                            mat(ir1,jr1,kr1),icount,data_type,ProcNgh(2),3, MPI_COMM_WORLD,SRstatus,ierr)
        endif
      ENDIF
      ...
      IF(hi%zph > 0) THEN
        ir1=mb%xmm;             ir2=mb%xpm
        jr1=mb%ymm;             jr2=mb%ypm
        kr1=mb%zpr+1;           kr2=mb%zpr+hi%zph
        
        is1=ir1;                is2=ir2
        js1=jr1;                js2=jr2    
        ks1=mb%zmr;             ks2=mb%zmr+hi%zph-1
        if(ProcNgh(2)==nrank) then  ! neighbour is nrank itself, and ProcNgh(1)==nrank also !!!
          mat(ir1:ir2, jr1:jr2, kr1:kr2) = mat(is1:is2, js1:js2, ks1:ks2)
        else
          icount=(mb%xpm-mb%xmm+1)*(mb%ypm-mb%ymm+1)*hi%zph
          call MPI_SENDRECV(mat(is1,js1,ks1),icount,data_type,ProcNgh(2),4, &
                            mat(ir1,jr1,kr1),icount,data_type,ProcNgh(1),4, MPI_COMM_WORLD,SRstatus,ierr)
        endif
      ENDIF
    CASE(3)  ! z-pencil
      ! TODO
  ...
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine sum_halo(mat,mb,hi); implicit none
    type(MatBound),intent(in)::mb
    type(HaloInfo),intent(in)::hi
    real(mytype),dimension(mb%xmm:mb%xpm,mb%ymm:mb%ypm,mb%zmm:mb%zpm),intent(INOUT):: mat
    ...
    integer::ir1,ir2,jr1,jr2,kr1,kr2   ! index for halo receiving
    integer::jc,data_type,halo_type,ierr,ProcNgh(4)
    real(mytype),dimension(:,:,:),allocatable::Mat3HaloR
    integer::icount,ilength,ijump,SRstatus(MPI_STATUS_SIZE)
      
    data_type=real_type
    if(mb%pencil /= hi%pencil) then
      ierr = 14
      call decomp_2d_abort(ierr,'sum_halo wrong: pencil error, Gong Zheng')
    endif
    if(mb%xme<hi%xmh .or. mb%yme<hi%ymh .or. mb%zme<hi%zmh .or.  &
       mb%xpe<hi%xph .or. mb%ype<hi%yph .or. mb%zpe<hi%zph .or.  &
       hi%xmh<0      .or. hi%ymh<0      .or. hi%zmh<0      .or.  &
       hi%xph<0      .or. hi%yph<0      .or. hi%zph<0       ) then
      ierr = 14
      call decomp_2d_abort(ierr,'sum_halo wrong: HaloSize error, Gong Zheng')
    endif
    ...
    CASE(2)  ! y-pencil
      ...
      ! step6: send to zp_dir, and receive from zm_dir
      IF(hi%zph > 0) THEN
        ir1=mb%xmm;             ir2=mb%xpm
        jr1=mb%ymm;             jr2=mb%ypm
        kr1=mb%zmr;             kr2=mb%zmr+hi%zph-1
        
        is1=ir1;                is2=ir2
        js1=jr1;                js2=jr2
        ks1=mb%zpr+1;           ks2=mb%zpr+hi%zph
        allocate(Mat3HaloR(ir1:ir2, jr1:jr2, kr1:kr2)); Mat3HaloR= 0.0_mytype
        if(ProcNgh(2)==nrank) then  ! neighbour is nrank itself, and ProcNgh(1)==nrank also !!!
          Mat3HaloR = mat(is1:is2, js1:js2, ks1:ks2)
        else
          icount=(mb%xpm-mb%xmm+1)*(mb%ypm-mb%ymm+1)*hi%zph
          call MPI_SENDRECV(mat(is1,js1,ks1),icount,data_type,ProcNgh(1),5, &
                            Mat3HaloR,       icount,data_type,ProcNgh(2),5, MPI_COMM_WORLD,SRstatus,ierr)
        endif
        mat(ir1:ir2, jr1:jr2, kr1:kr2)=mat(ir1:ir2, jr1:jr2, kr1:kr2)+Mat3HaloR
        deallocate(Mat3HaloR)
      ENDIF

      ! step5: send to zm_dir, and receive from zp_dir
      IF(hi%zmh > 0) THEN
        ir1=mb%xmm;             ir2=mb%xpm
        jr1=mb%ymm;             jr2=mb%ypm
        kr1=mb%zpr-hi%zmh+1;    kr2=mb%zpr
        
        is1=ir1;                is2=ir2
        js1=jr1;                js2=jr2      
        ks1=mb%zmr-hi%zmh;      ks2=mb%zmr-1
        allocate(Mat3HaloR(ir1:ir2, jr1:jr2, kr1:kr2)); Mat3HaloR= 0.0_mytype
        if(ProcNgh(1)==nrank) then  ! neighbour is nrank itself, and ProcNgh(2)==nrank also !!!
          Mat3HaloR = mat(is1:is2, js1:js2, ks1:ks2)
        else
          icount=(mb%xpm-mb%xmm+1)*(mb%ypm-mb%ymm+1)*hi%zmh
          call MPI_SENDRECV(mat(is1,js1,ks1),icount,data_type,ProcNgh(2),6, &
                            Mat3HaloR,       icount,data_type,ProcNgh(1),6, MPI_COMM_WORLD,SRstatus,ierr)
        endif
        mat(ir1:ir2, jr1:jr2, kr1:kr2)=mat(ir1:ir2, jr1:jr2, kr1:kr2)+Mat3HaloR
        deallocate(Mat3HaloR)        
      ENDIF    
    
      ! step4: send to xp_dir, and receive from xm_dir
      IF(hi%xph > 0) THEN
        ir1=mb%xmr;             ir2=mb%xmr+hi%xph-1
        jr1=mb%ymm;             jr2=mb%ypm
        kr1=mb%zmr;             kr2=mb%zpr
                
        is1=mb%xpr+1;           is2=mb%xpr+hi%xph
        js1=jr1;                js2=jr2    
        ks1=kr1;                ks2=kr2
        allocate(Mat3HaloR(ir1:ir2, jr1:jr2, kr1:kr2)); Mat3HaloR= 0.0_mytype
        if(ProcNgh(4)==nrank) then  ! neighbour is nrank itself, and ProcNgh(3)==nrank also !!!
          ...
          call MPI_TYPE_VECTOR(icount, ilength, ijump, data_type, halo_type, ierr)
          call MPI_TYPE_COMMIT(halo_type, ierr)
          call MPI_SENDRECV(mat(is1,js1,ks1),1,halo_type,ProcNgh(3),7, &
                      Mat3HaloR,icount*ilength,data_type,ProcNgh(4),7, MPI_COMM_WORLD,SRstatus,ierr)
          call MPI_TYPE_FREE(halo_type, ierr)
        endif
        mat(ir1:ir2, jr1:jr2, kr1:kr2)=mat(ir1:ir2, jr1:jr2, kr1:kr2)+Mat3HaloR
        deallocate(Mat3HaloR)
      ...
      ! step3: send to xm_dir, and receive from xp_dir
      IF(hi%xmh > 0) THEN       
        ir1=mb%xpr-hi%xmh+1;    ir2=mb%xpr
        jr1=mb%ymm;             jr2=mb%ypm
        kr1=mb%zmr;             kr2=mb%zpr
        
        is1=mb%xmr-hi%xmh;      is2=mb%xmr-1
        js1=jr1;                js2=jr2    
        ks1=kr1;                ks2=kr2
        allocate(Mat3HaloR(ir1:ir2, jr1:jr2, kr1:kr2)); Mat3HaloR= 0.0_mytype
        if(ProcNgh(3)==nrank) then  ! neighbour is nrank itself, and ProcNgh(4)==nrank also !!!
          ...
          call MPI_TYPE_VECTOR(icount,ilength,ijump, data_type, halo_type, ierr)
          call MPI_TYPE_COMMIT(halo_type, ierr)
          call MPI_SENDRECV(mat(is1,js1,ks1),1,halo_type,ProcNgh(4),8, &
                      Mat3HaloR,icount*ilength,data_type,ProcNgh(3),8, MPI_COMM_WORLD,SRstatus,ierr)
          call MPI_TYPE_FREE(halo_type, ierr)
        endif 
        mat(ir1:ir2, jr1:jr2, kr1:kr2)=mat(ir1:ir2, jr1:jr2, kr1:kr2)+Mat3HaloR
        deallocate(Mat3HaloR)
      ...
      ! step2: send to yp_dir, and receive from ym_dir
      IF(hi%yph >0 ) THEN
        ir1=mb%xmr;             ir2=mb%xpr     
        jr1=mb%ymr;             jr2=mb%ymr+hi%yph-1  
        kr1=mb%zmr;             kr2=mb%zpr

        is1=ir1;                is2=ir2
        js1=mb%ypr+1;           js2=mb%ypr+hi%yph
        ks1=kr1;                ks2=kr2
        allocate(Mat3HaloR(ir1:ir2, jr1:jr2, kr1:kr2)); Mat3HaloR= 0.0_mytype
        Mat3HaloR = mat(is1:is2, js1:js2, ks1:ks2)
        mat(ir1:ir2, jr1:jr2, kr1:kr2)=mat(ir1:ir2, jr1:jr2, kr1:kr2)+Mat3HaloR
      ...
      ! step1: send to ym_dir, and receive from yp_dir
      IF(hi%ymh >0 ) THEN
        ir1=mb%xmr;             ir2=mb%xpr
        jr1=mb%ypr-hi%ymh+1;    jr2=mb%ypr
        kr1=mb%zmr;             kr2=mb%zpr

        is1=ir1;                is2=ir2
        js1=mb%ymr-hi%ymh;      js2=mb%ymr-1
        ks1=kr1;                ks2=kr2
        allocate(Mat3HaloR(ir1:ir2, jr1:jr2, kr1:kr2)); Mat3HaloR= 0.0_mytype
        Mat3HaloR = mat(is1:is2, js1:js2, ks1:ks2)
        mat(ir1:ir2, jr1:jr2, kr1:kr2)=mat(ir1:ir2, jr1:jr2, kr1:kr2)+Mat3HaloR
