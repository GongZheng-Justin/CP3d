...
module lp_Comm
  ...
  contains
    ...
    procedure,private:: IsInThisProc=> PC_IsInThisProc
...
contains
  ...
  subroutine PC_Comm_For_Exchange(this); implicit none
    ...
    call MPI_SENDRECV(nsend, 2, int_type, LPT_decomp%ProcNgh(3), 9, &
                      nrecv, 2, int_type, LPT_decomp%ProcNgh(4), 9, MPI_COMM_WORLD,SRstatus,ierr)
    if(nrecv(2)>0) then
      allocate(buf_recv(nrecv(2)))
      call MPI_IRECV(buf_recv,nrecv(2),real_type,LPT_decomp%ProcNgh(4),10,MPI_COMM_WORLD,request(1),ierr)
    endif
    if(nsend(2)>0) then
      allocate(buf_send(nsend(2)))
      nlocalp= nlocal;  nlocal = nlocalp - nsend(1)
      call this%pack_Exchange(buf_send,nsend(1),nlocalp,xp_axis)
      call MPI_SEND(buf_send,nsend(2),real_type,LPT_decomp%ProcNgh(3),10,MPI_COMM_WORLD,ierr)
    endif
    if(nrecv(2)>0) then
    ...
    call MPI_SENDRECV(nsend, 2, int_type, LPT_decomp%ProcNgh(4),11, &
                      nrecv, 2, int_type, LPT_decomp%ProcNgh(3),11, MPI_COMM_WORLD,SRstatus,ierr)
    if(nrecv(2)>0) then
      allocate(buf_recv(nrecv(2)))
      call MPI_IRECV(buf_recv,nrecv(2),real_type,LPT_decomp%ProcNgh(3),12,MPI_COMM_WORLD,request(2),ierr)
    endif
    if(nsend(2)>0) then
      allocate(buf_send(nsend(2)))
      nlocalp= nlocal;  nlocal = nlocalp - nsend(1)
      call this%pack_Exchange(buf_send,nsend(1),nlocalp,xm_axis)
      call MPI_SEND(buf_send,nsend(2),real_type,LPT_decomp%ProcNgh(4),12,MPI_COMM_WORLD,ierr)
    endif
    if(nrecv(2)>0) then
    ...
    call MPI_SENDRECV(nsend, 2, int_type, LPT_decomp%ProcNgh(1),13, &
                      nrecv, 2, int_type, LPT_decomp%ProcNgh(2),13, MPI_COMM_WORLD,SRstatus,ierr)
    if(nrecv(2)>0) then
      allocate(buf_recv(nrecv(2)))
      call MPI_IRECV(buf_recv,nrecv(2),real_type,LPT_decomp%ProcNgh(2),14,MPI_COMM_WORLD,request(3),ierr)
    endif
    if(nsend(2)>0) then
      allocate(buf_send(nsend(2)))
      nlocalp= nlocal;  nlocal = nlocalp - nsend(1)
      call this%pack_Exchange(buf_send,nsend(1),nlocalp,zp_axis)
      call MPI_SEND(buf_send,nsend(2),real_type,LPT_decomp%ProcNgh(1),14,MPI_COMM_WORLD,ierr)
    endif
    if(nrecv(2)>0) then
    ...
    call MPI_SENDRECV(nsend, 2, int_type, LPT_decomp%ProcNgh(2),15, &
                      nrecv, 2, int_type, LPT_decomp%ProcNgh(1),15, MPI_COMM_WORLD,SRstatus,ierr)
    if(nrecv(2)>0) then
      allocate(buf_recv(nrecv(2)))
      call MPI_IRECV(buf_recv,nrecv(2),real_type,LPT_decomp%ProcNgh(1),16,MPI_COMM_WORLD,request(4),ierr)
    endif
    if(nsend(2)>0) then
      allocate(buf_send(nsend(2)))
      nlocalp= nlocal;  nlocal = nlocalp - nsend(1)
      call this%pack_Exchange(buf_send,nsend(1),nlocalp,zm_axis)
      call MPI_SEND(buf_send,nsend(2),real_type,LPT_decomp%ProcNgh(2),16,MPI_COMM_WORLD,ierr)
    endif
    if(nrecv(2)>0) then
  ...
  function PC_IsInThisProc(this,buf_recv,m,dir) result(res); implicit none
    ...
    call LPTLogInfo%OutInfo(" Exchange direction  is :"//strip(num2str(dir)  ),3)
    call LPTLogInfo%OutInfo("   The particle id   is :"//strip(num2str( nint(buf_recv(m)  ))),3)
    call LPTLogInfo%OutInfo("       particle type is :"//strip(num2str( nint(buf_recv(m+1)))),3)
    call LPTLogInfo%OutInfo("       x-coordinate  is :"//strip(num2str( buf_recv(m+3) )), 3)
    call LPTLogInfo%OutInfo("       y-coordinate  is :"//strip(num2str( buf_recv(m+4) )), 3)
    call LPTLogInfo%OutInfo("       z-coordinate  is :"//strip(num2str( buf_recv(m+5) )), 3)
    call LPTLogInfo%OutInfo(" Present processor   is :"//strip(num2str(nrank)), 3)
  ...
  subroutine PC_reallocate_sendlist(this,ns); implicit none
    ...
    call LPTLogInfo%OutInfo("The present processor  is :"//strip(num2str(nrank)),3)
    call LPTLogInfo%OutInfo("Previous matirx length is :"//strip(num2str(sizep)),3)
    call LPTLogInfo%OutInfo("Updated  matirx length is :"//strip(num2str(sizen)),3)
