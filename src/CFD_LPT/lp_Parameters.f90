...
contains
  ...
  ! LO_ReadLPTOption
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine LO_ReadLPTOption(this, chFile); implicit none
    ...
    integer::numPrtcl,SaveVisuLPT,PI_Method,numPrtcl_Type,Cmd_LFile_Freq,LF_file_lvl,LF_cmdw_lvl,iUnit,ierr, len_str
