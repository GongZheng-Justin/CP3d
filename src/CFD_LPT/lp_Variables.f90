...
contains
  ...
  subroutine GL_AllocateAllVar(this); implicit none
    ...
    allocate(GPrtcl_VFluid(mlocal),            Stat=ierrTmp);ierr=ierr+abs(ierrTmp)
    if(ierr/=0) call LPTLogInfo%CheckForError(ErrT_Abort,"GL_AllocateAllVar","Allocation failed 1")

    allocate(GPrtcl_FpForce(mlocal),Stat=ierrTmp);ierr=ierr+abs(ierrTmp)
    if(ierr/=0) call LPTLogInfo%CheckForError(ErrT_Abort,"GL_AllocateAllVar: ","Allocation failed 2")    
  ...
  subroutine GL_MakingAllPrtcl(this, chFile); implicit none
    ...
    if(ierr/=0) call LPTLogInfo%CheckForError(ErrT_Abort,"MakingAllPrtcl", "Cannot open file: "//strip(chFile))
    read(iUnit, nml=ParticleMakingOption)
    if(nrank==0)write(LPTLogInfo%iUnit, nml=ParticleMakingOption)
