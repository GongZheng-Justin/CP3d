...
!   InitLPTStatistics
!^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine InitLPTStatistics(chFile); implicit none
    ...
    if(ierr/=0) call LPTLogInfo%CheckForError(ErrT_Abort,"InitLPTStatistics", "Cannot open file: "//strip(chFile))
    read(iUnit, nml=ParticleStatisticOption)
    if(nrank==0)write(LPTLogInfo%iUnit, nml=ParticleStatisticOption)
    ...
    if(nrank/=0) return
    filename = strip(LPT_Opt%ResultsDir)//'shannon'//int2str(ilast,10)
    open(newunit=iUnit,file=filename,status='replace',form='formatted',IOSTAT=ierr)
    if(ierr /= 0) call LPTLogInfo%CheckForError(ErrT_Abort,"InitLPTStatistics","Cannot open file: "//filename)
    close(iUnit,IOSTAT=ierr)    
  ...
  ! ClcLPTStatistics
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine ClcLPTStatistics(); implicit none
    ...
    IF(nrank==0) THEN
      ...
      if(ierr /= 0) then
        call LPTLogInfo%CheckForError(ErrT_Pass,"ClcLPTStatistics","Cannot open file: "//filename)
      else  
        write(iUnit,*)itime,shannon_entropy
      endif
      close(iUnit,IOSTAT=ierr)
    ENDIF
    ...
    if(nrank==0) then
      ...
      do itype=1,npType
        filename = strip(LPT_Opt%ResultsDir)//'pstats'//int2str(itime,10)//'_set'//int2str(itype,4)
        open(newunit=iUnit,file=filename,status='replace',form='formatted',IOSTAT=ierr)
        if(ierr /= 0) then
          call LPTLogInfo%CheckForError(ErrT_Pass,"ClcLPTStatistics","Cannot open file: "//filename)
        else              
