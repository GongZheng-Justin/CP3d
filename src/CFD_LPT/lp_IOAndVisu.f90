...
#include "Prtcl_Dump_MPI_inc.f90"
  ...
  subroutine PIO_Init_Visu(this,chFile,iStage); implicit none
    ...
    if(iStage==1) then
      ...
      if(ierr/=0)call LPTLogInfo%CheckForError(ErrT_Abort,"PIO_Init_Visu", "Cannot open file: "//strip(chFile))
      read(iUnit, nml=PrtclVisuOption)
      if(nrank==0)write(LPTLogInfo%iUnit, nml=PrtclVisuOption)
    ...
    ! Write XDMF file
    if(nrank/=0) return
    XdmfFile = strip(LPT_Opt%ResultsDir)//"PartVisuFor"//strip(LPT_Opt%RunName)//".xmf"
    open(newunit=iUnit, file=XdmfFile,status='replace',form='formatted',IOSTAT=ierr)
    if(ierr /= 0) call LPTLogInfo%CheckForError(ErrT_Abort,"PIO_Init_Visu","Cannot open file:  "//XdmfFile)
    ...
    ! Time series
    nfld = (LPT_Opt%ilast - LPT_Opt%ifirst +1)/LPT_Opt%SaveVisu  + 1
    ...
    do ifld =1, nfld
      if(mod(ifld,10)==0) then
        write(iUnit,'(I10)') (ifld-1)*LPT_Opt%SaveVisu + LPT_Opt%ifirst-1
        if(ifld < nfld) write(iUnit,'(A)',advance='no') '        '
      else
        write(iUnit,'(I9)',advance='no') (ifld-1)*LPT_Opt%SaveVisu + LPT_Opt%ifirst-1
      endif
    enddo
  ...
  subroutine PIO_Write_XDMF(itime); implicit none
    ...
    open(newunit=iUnit, file=XdmfFile,status='old',position='append',form='formatted',IOSTAT=ierr)
    if(ierr/=0) then
      call LPTLogInfo%CheckForError(ErrT_Abort,"PIO_Write_XDMF","Cannot open file: "//XdmfFile)
    endif

    disp = 0_MPI_OFFSET_KIND
    XdmfFile = "PartVisuFor"//strip(LPT_Opt%RunName)
    dims=3; iprec=RK
  ...
  subroutine PIO_Delete_Prev_Restart(itime); implicit none
    ...
    if(nrank/=0) return
    !
    chFile = strip(LPT_Opt%RestartDir)//"RestartFor"//strip(LPT_Opt%RunName)//int2str(Prev_BackUp_itime,10)
...
#define LPT_NumRead 2000
  subroutine PIO_Read_Restart(); implicit none
    ...
    chFile = strip(LPT_Opt%RestartDir)//"RestartFor"//strip(LPT_Opt%RunName)//int2str(itime,10)
    open(newunit=iUnit,file=chFile,status='old',form='unformatted',access='stream',action='read',IOSTAT=ierr)
    if(ierr/=0 .and. nrank==0) call LPTLogInfo%CheckForError(ErrT_Abort,"PIO_Read_Restart","Cannot open file: "//chFile)
    ...
    nreal3 = 2*tsize
    allocate(real3Vec(nreal3),PosVec(LPT_NumRead))
    ...
    call MPI_ALLREDUCE(nP_in_bin, LPTProperty%nPrtcl_in_Bin,LPT_Opt%numPrtcl_Type,int_type,MPI_SUM,MPI_COMM_WORLD,ierr)
...
#define LPT_NumRestart 200
  subroutine PIO_Write_Restart(itime); implicit none
    ...
    if(prank==0) then
      call MPI_FILE_WRITE_AT(fh,disp,LPT_Opt%np_InDomain,1,int_type,MPI_STATUS_IGNORE,ierr)
    endif
    disp = disp + int_byte
    call MPI_BARRIER(Prtcl_WORLD,ierr)
    ...
    allocate(real3Vec(LPT_NumRestart*nreal3))
    nLeft=nlocal; pid=0
    bgn_byte=disp+int(nreal3*real3_byte,8)*int(bgn_ind,8)
    DO
      nRestart=min(nLeft,LPT_NumRestart)
      k=0
      do i=1,nRestart
        pid=pid+1
...
#undef LPT_NumRestart
  ...
  subroutine PIO_Final_Visu(); implicit none
    ...
    xdmfFile = strip(LPT_Opt%ResultsDir)//"PartVisuFor"//strip(LPT_Opt%RunName)//".xmf"
    open(newunit=iUnit, file=XdmfFile,status='old',position='append',form='formatted',IOSTAT=ierr)
    if(ierr/=0 .and. nrank==0) call LPTLogInfo%CheckForError(ErrT_Abort,"PIO_Final_Visu","Cannot open file:  "//XdmfFile)
    ! XDMF/XMF Tail
    write(iUnit,'(A)') '    </Grid>'
    write(iUnit,'(A)') '</Domain>'
    write(iUnit,'(A)') '</Xdmf>'
  ...
  subroutine PIO_Dump_Visu(this, itime); implicit none
    ...
    ! begin to dump
    chFile = strip(LPT_Opt%ResultsDir)//"PartVisuFor"//strip(LPT_Opt%RunName)//int2str(itime,10)
    call MPI_FILE_OPEN(Prtcl_WORLD, chFile, MPI_MODE_CREATE+MPI_MODE_WRONLY, MPI_INFO_NULL, fh, ierr)
    call MPI_BARRIER(Prtcl_WORLD,ierr)
    call my_mpi_file_set_size(fh,0_8,ierr)  ! guarantee overwriting
    call MPI_BARRIER(Prtcl_WORLD,ierr)
