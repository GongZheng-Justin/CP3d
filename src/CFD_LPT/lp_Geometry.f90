...
contains
  ...
  subroutine G_InitAllocate(this); implicit none
    ...
    if(nrank/=0) return
    chFile = strip(LPT_opt%ResultsDir)//"WallsFor"//strip(LPT_opt%RunName)//".backup"
    open(newunit=iUnit, file=chfile,status='replace',form='formatted',IOSTAT=ierr)
    if(ierr/=0 .and. nrank==0) call LPTLogInfo%CheckForError(ErrT_Abort,"G_InitAllocate","Cannot open file: "//chFile)
  ...
  subroutine G_add_PlaneWall(this, p1, p2, p3, p4, user_id, prop_type, both, infinite, t_vel_t); implicit none
    ...
    IF(this%nPW_local == MaxWallSize) THEN
      ...
      if(ierr/=0) call LPTLogInfo%CheckForError(ErrT_Abort,"G_add_PlaneWall","Reallocation failed, 2 ")
    ...
    IF(wall%IsInDomain(pmin, pmax)) THEN
      ...
      if(ierr/=0 .and. nrank==0) call LPTLogInfo%CheckForError(ErrT_Abort,"G_add_PlaneWall","Cannot open file: "//chFile)
      write(iUnit,* ) p1
      write(iUnit,* ) p2
      write(iUnit,* ) p3
      write(iUnit,* ) p4
    ...
    ELSE
      ...
      call LPTLogInfo%OutInfo("   Point 1: "//strip(num2str(p1%x))//'  '//strip(num2str(p1%y))//'  '//strip(num2str(p1%z)), 3, .true.)
      call LPTLogInfo%OutInfo("   Point 2: "//strip(num2str(p2%x))//'  '//strip(num2str(p2%y))//'  '//strip(num2str(p2%z)), 3, .true.)
      call LPTLogInfo%OutInfo("   Point 3: "//strip(num2str(p3%x))//'  '//strip(num2str(p3%y))//'  '//strip(num2str(p3%z)), 3, .true.)
      call LPTLogInfo%OutInfo("   Point 4: "//strip(num2str(p4%x))//'  '//strip(num2str(p4%y))//'  '//strip(num2str(p4%z)), 3, .true.)
    ENDIF
  ...
  ! MakeGeometry
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90     
  subroutine G_MakeGeometry(this); implicit none
    class(Geometry)::this
