#include "definitions_inc.f90"
  ...
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine pack_Comm_FluidIndicator(buf_send,nsendg,nsend,dir); implicit none
    ...
    do i=1,nsendg
      id=sendlist(i)
      buf_send(m)=real(GhostP_pType(id),RK); m=m+1 ! 01
      buf_send(m)=GhostP_PosR(id)%x+pdx;     m=m+1 ! 02
      buf_send(m)=GhostP_PosR(id)%y+pdy;     m=m+1 ! 03
      buf_send(m)=GhostP_PosR(id)%z+pdz;     m=m+1 ! 04
    enddo
    do i=nsendg+1,nsend
      id=sendlist(i)
      buf_send(m)=real(GPrtcl_pType(id),RK); m=m+1 ! 01
      buf_send(m)=GPrtcl_PosR(id)%x+pdx;     m=m+1 ! 02
      buf_send(m)=GPrtcl_PosR(id)%y+pdy;     m=m+1 ! 03
  ...
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine unpack_Comm_FluidIndicator(buf_recv,n1,n2); implicit none
  ...
  ! Init_IBPFix
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine Init_IBPFix(); implicit none
...
#undef    clc_Point_indxyz_IBPFix
  ...
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  function clc_VoidFraction_IBM(CenterDiff,Radius) result(VoidFraction); implicit none
