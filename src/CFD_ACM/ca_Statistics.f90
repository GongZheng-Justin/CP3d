...
#define ClcMeanVelocity
contains
  
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  !   InitCAStatistics
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine InitCAStatistics(chFile); implicit none
    ...
    if(ierr/=0) call DEMLogInfo%CheckForError(ErrT_Abort,"InitCAStatistics ","Allocation failed")
    k=nyc/nslab
    do j=0,nslab
    ...
    if(nrank==0) then
      ...
      if(ierr/=0 .and. nrank==0) call DEMLogInfo%CheckForError(ErrT_Abort,"InitCAStatistics","Cannot open file: "//filename)
      close(iUnit,IOSTAT=ierr)
    endif
#endif
#ifdef ClcMeanVelocity
    if(nrank==0) then
      filename = strip(DEM_opt%ResultsDir) // "MeanVelocity" // int2str(ilast,10)
      open(newunit=iUnit,file=filename,status='replace',form='formatted',IOSTAT=ierr)
      if(ierr/=0 .and. nrank==0) call DEMLogInfo%CheckForError(ErrT_Abort,"InitCAStatistics","Cannot open file: "//filename)
      close(iUnit,IOSTAT=ierr)
    endif
  ...
  ! ClcCAStatistics
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine ClcCAStatistics(); implicit none
    ...
    real(RK)::irnpsum,inpstime,SumVel(3),SumVelR(3)
    ...
    do pid=1,nlocal
      itype = GPrtcl_pType(pid)
      SumVel(1)= SumVel(1)+ GPrtcl_linVel(1,pid)%x* DEMProperty%Prtcl_PureProp(itype)%Volume
    enddo
    ...
    if(nrank==0) then
      ...
      if(ierr /= 0) then
        call DEMLogInfo%CheckForError(ErrT_Pass,"ClcLCAStatistics: ","Cannot open file: "//filename)
      else
        write(iUnit,'(I7,ES24.15)')itime,SumVelR(1)/(xlx*zlz)
        close(iUnit,IOSTAT=ierr)
      endif
    endif
#endif
    ...
    SumVel=0.0_RK
    do pid=1,nlocal
      SumVel(1)= SumVel(1)+ GPrtcl_linVel(1,pid)%x
      SumVel(2)= SumVel(2)+ GPrtcl_linVel(1,pid)%y
      SumVel(3)= SumVel(3)+ GPrtcl_linVel(1,pid)%z
    enddo
    call MPI_REDUCE(SumVel,SumVelR,3,real_type,MPI_SUM,0,MPI_COMM_WORLD,ierr)
    if(nrank==0) then
      filename = strip(DEM_opt%ResultsDir) // "MeanVelocity" // int2str(ilast,10)
      open(newunit=iUnit,file=filename,status='old',form='formatted',position='append',IOSTAT=ierr)
      if(ierr /= 0) then
        ...
        write(iUnit,'(I7,3ES24.15)')itime,SumVelR/DEM_Opt%np_InDomain
        close(iUnit,IOSTAT=ierr)
      endif
    endif
#endif
    ...
    call MPI_REDUCE(sumStat,sumStatR,13*npType*nslab, real_type,MPI_SUM,0,MPI_COMM_WORLD,ierr)
    ...
    if(nrank==0) then
      ...
      do itype=1,npType
        ...
        open(newunit=iUnit,file=filename,status='replace',form='formatted',IOSTAT=ierr)
        if(ierr /= 0) then
          call DEMLogInfo%CheckForError(ErrT_Pass,"ClcCAStatistics","Cannot open file: "//filename)
        else              
          write(iUnit,'(a,I7,a,I7,a,I7)')'    The time step range for this particle statistics is ', &
                                       itime-(npstime-1)*ivstats, ':', ivstats, ':', itime
          write(iUnit,*)
          write(iUnit,'(A)')'  yp, np, up, vp, wp, upup, vpvp, wpwp, upvp, wx, wy, wz, wxwx, wywy, wzwz' 
          do jc=1,nslab
