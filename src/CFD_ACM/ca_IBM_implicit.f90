...
module ca_IBM_implicit
  ...
  use mc_TypeDef
  use mc_LogInfo
  use mc_Decomp2d,only:nrank,y1start,y1end    
  use f2_Parameters
  ...
  use sp_Variables
  use sp_Parameters
  use sp_Comm,only: DEM_Comm,sendlist,pbc,dx_pbc,dy_pbc,dz_pbc,reallocate_ghost_for_cntct
...
contains
#define ca_IBM_common_add_LS
#include "ca_IBM_common_inc.f90"

  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  ! Init_IBM_imp
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine Init_IBM_imp(chFile); implicit none
    character(len=*),intent(in)::chFile
    
    ! locals
    real(RK)::vol_tot,vol_local,maxR,dxyz,min_xz,max_dxyz, ForceAmplifyCoe
    integer:: i,ierr,ierrTmp, IBMForceScheme, nForcingExtra, deltaFunction_Scheme, iUnit
    Namelist/IBMForce_Options/ IBMForceScheme, ForceAmplifyCoe, nforcingextra, deltaFunction_Scheme
    ...
    allocate(GhostP_Direction(mGhostIBM),       Stat=ierrTmp); ierr =ierr +abs(ierrTmp)
    allocate(GhostP_FluidIntegrate(2,mGhostIBM),Stat=ierrTmp); ierr =ierr +abs(ierrTmp)
    if(ierr/=0)   call MainLog%CheckForError(ErrT_Abort,"Init_IBM","Allocation failed 1")
  ...
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine Update_FluidIndicator_imp(FluidIndicator); implicit none
  ...
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine PrepareIBM_interp_imp1(); implicit none

    ! locals
    integer:: i,j,itype,nPartition,nIbpSum,ierr
...
#undef    clc_Point_indxyz_IBPMove
      enddo
    ENDDO
    call MPI_REDUCE(nIBP,nIbpSum,1,int_type,MPI_SUM,0,MPI_COMM_WORLD,ierr)
    if(nrank==0) then
  ...
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine PrepareIBM_interp_imp2(); implicit none
...
#undef    clc_Point_indxyz_IBPMove
  ...
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine InterpolationIBM_imp(ux,uy,uz); implicit none
...
#include "clc_Point_Interpolation_inc.f90"
  ...
  ! SpreadIbpForce_imp
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90 
  subroutine SpreadIbpForce_imp(VolForce_x,VolForce_y,VolForce_z,Spreadflag); implicit none
...
#include "spreadIBM_force_inc.f90"
  ...
  ! UpdateRhsIBM_imp1
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine UpdateRhsIBM_imp1(RhsX,RhsY,RhsZ,VolForce_x,VolForce_y,VolForce_z); implicit none
  ...
  ! UpdateRhsIBM_imp2
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine UpdateRhsIBM_imp2(RhsX,RhsY,RhsZ,VolForce_x,VolForce_y,VolForce_z); implicit none
    real(RK),dimension(mb1%xmm:mb1%xpm,mb1%ymm:mb1%ypm,mb1%zmm:mb1%zpm),intent(inout)::RhsZ,VolForce_x
    real(RK),dimension(mb1%xmm:mb1%xpm,mb1%ymm:mb1%ypm,mb1%zmm:mb1%zpm),intent(in)::VolForce_y,VolForce_z
    real(RK),dimension(y1start(1):y1end(1),y1start(2):y1end(2),y1start(3):y1end(3)),intent(inout)::RhsX,RhsY
  ...
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine PComm_IBM_forward_imp1(); implicit none
    ...
    integer::i,ierr,request(4),SRstatus(MPI_STATUS_SIZE) 
    ...
    IF(pbc(2)) THEN
      do i=1,nlocalOld
        ...
        if(pyt +SMALL >yedCoord) then
          ...
          GhostP_Direction(ng)= THIS_PROC_YP      
          GhostP_PosR(ng)%y   = py -yly                
        endif
        ...
        if(pyt -SMALL <ystCoord) then
          ...
          GhostP_pType(ng)    = GPrtcl_pType(i)
          GhostP_PosR(ng)     = GPrtcl_PosR(i)
          GhostP_Direction(ng)= THIS_PROC_YM          
          GhostP_PosR(ng)%y   = py +yly
        endif        
      enddo    
    ENDIF    
    ...
    ELSEIF(DEM_decomp%ProcNgh(3) /= MPI_PROC_NULL) THEN
      do i=1,ngpp    ! consider the previous ghost particle firstly
        pxt= GhostP_PosR(i)%x+ GhostP_PosR(i)%w
        if(pxt +SMALL >xedCoord) then
          nsend=nsend+1
    ...
    call MPI_SENDRECV(nsend, 1, int_type, DEM_decomp%ProcNgh(3), 1, &
                      nrecv, 1, int_type, DEM_decomp%ProcNgh(4), 1, MPI_COMM_WORLD,SRstatus,ierr)
    if(nrecv>0) then
      nrecv2=nrecv*GIBM_size_forward_imp
      allocate(buf_recv(nrecv2))
      call MPI_IRECV(buf_recv,nrecv2,real_type,DEM_decomp%ProcNgh(4),2,MPI_COMM_WORLD,request(1),ierr)
    endif
    if(nsend>0) then
      nsend2=nsend*GIBM_size_forward_imp
      allocate(buf_send(nsend2))
      call pack_IBM_forward_imp1(buf_send,nsendg,nsend,xp_dir)
      call MPI_SEND(buf_send,nsend2,real_type,DEM_decomp%ProcNgh(3),2,MPI_COMM_WORLD,ierr)
    endif
    if(nrecv>0) then
    ...
    call MPI_SENDRECV(nsend, 1, int_type, DEM_decomp%ProcNgh(4), 3, &
                      nrecv, 1, int_type, DEM_decomp%ProcNgh(3), 3, MPI_COMM_WORLD,SRstatus,ierr)
    if(nrecv>0) then
      nrecv2=nrecv*GIBM_size_forward_imp
      allocate(buf_recv(nrecv2))
      call MPI_IRECV(buf_recv,nrecv2,real_type,DEM_decomp%ProcNgh(3),4,MPI_COMM_WORLD,request(2),ierr)
    endif
    if(nsend>0) then
      nsend2=nsend*GIBM_size_forward_imp
      allocate(buf_send(nsend2))
      call pack_IBM_forward_imp1(buf_send,nsendg,nsend,xm_dir)
      call MPI_SEND(buf_send,nsend2,real_type,DEM_decomp%ProcNgh(4),4,MPI_COMM_WORLD,ierr)
    endif
    if(nrecv>0) then
    ...
    call MPI_SENDRECV(nsend, 1, int_type, DEM_decomp%ProcNgh(1), 5, &
                      nrecv, 1, int_type, DEM_decomp%ProcNgh(2), 5, MPI_COMM_WORLD,SRstatus,ierr)
    if(nrecv>0) then
      nrecv2=nrecv*GIBM_size_forward_imp
      allocate(buf_recv(nrecv2))
      call MPI_IRECV(buf_recv,nrecv2,real_type,DEM_decomp%ProcNgh(2),6,MPI_COMM_WORLD,request(3),ierr)
    endif
    if(nsend>0) then
      nsend2=nsend*GIBM_size_forward_imp
      allocate(buf_send(nsend2))
      call pack_IBM_forward_imp1(buf_send,nsendg,nsend,zp_dir)
      call MPI_SEND(buf_send,nsend2,real_type,DEM_decomp%ProcNgh(1),6,MPI_COMM_WORLD,ierr)
    endif
    if(nrecv>0) then
    ...
    call MPI_SENDRECV(nsend, 1, int_type, DEM_decomp%ProcNgh(2), 7, &
                      nrecv, 1, int_type, DEM_decomp%ProcNgh(1), 7, MPI_COMM_WORLD,SRstatus,ierr)
    if(nrecv>0) then
      nrecv2=nrecv*GIBM_size_forward_imp
      allocate(buf_recv(nrecv2))
      call MPI_IRECV(buf_recv,nrecv2,real_type,DEM_decomp%ProcNgh(1),8,MPI_COMM_WORLD,request(4),ierr)
    endif
    if(nsend>0) then
      nsend2=nsend*GIBM_size_forward_imp
      allocate(buf_send(nsend2))
      call pack_IBM_forward_imp1(buf_send,nsendg,nsend,zm_dir)
      call MPI_SEND(buf_send,nsend2,real_type,DEM_decomp%ProcNgh(2),8,MPI_COMM_WORLD,ierr)
    endif
    if(nrecv>0) then
  ...
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine pack_IBM_forward_imp1(buf_send,nsendg,nsend,dir); implicit none
    ...
    do i=1,nsendg
      id=sendlist(i)
      buf_send(m)=real(nlocalOld+ id,RK);    m=m+1 ! 01
      buf_send(m)=real(GhostP_pType(id),RK); m=m+1 ! 02
      buf_send(m)=real(dir,RK);              m=m+1 ! 03
      buf_send(m)=GhostP_PosR(id)%x+pdx;     m=m+1 ! 04
      buf_send(m)=GhostP_PosR(id)%y+pdy;     m=m+1 ! 05
      buf_send(m)=GhostP_PosR(id)%z+pdz;     m=m+1 ! 06
    enddo
    do i=nsendg+1,nsend
      id=sendlist(i)
      buf_send(m)=real(id,RK);               m=m+1 ! 01
      buf_send(m)=real(GPrtcl_pType(id),RK); m=m+1 ! 02
      buf_send(m)=real(dir,RK);              m=m+1 ! 03
      buf_send(m)=GPrtcl_PosR(id)%x+pdx;     m=m+1 ! 04
      buf_send(m)=GPrtcl_PosR(id)%y+pdy;     m=m+1 ! 05
  ...
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine unpack_IBM_forward_imp1(buf_recv,n1,n2); implicit none
  ...
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine pack_IBM_forward_imp2(buf_send,nsendg,nsend,dir); implicit none
    real(RK),dimension(:),intent(out)::buf_send
    integer,intent(in)::nsendg,nsend,dir
    ...
    do i=nsendg+1,nsend
      id=sendlist(i)
      buf_send(m)=real(id,RK);                m=m+1 ! 01
      buf_send(m)=real(PIBM_pType(id),RK);    m=m+1 ! 02
      buf_send(m)=real(dir,RK);               m=m+1 ! 03
      buf_send(m)=GPrtcl_PosOld(id)%x+pdx;    m=m+1 ! 04
      buf_send(m)=GPrtcl_PosOld(id)%y+pdy;    m=m+1 ! 05
  ...
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine unpack_IBM_forward_imp2(buf_recv,n1,n2); implicit none
    real(RK),dimension(:),intent(in)::buf_recv
    integer,intent(in)::n1,n2
  ...
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine IntegrateFluidPrtclForce_imp(ux,uy,uz); implicit none
    real(RK),dimension(mb1%xmm:mb1%xpm,mb1%ymm:mb1%ypm,mb1%zmm:mb1%zpm),intent(in)::ux,uy,uz

    ! locals
    type(real3)::FpVolume,Cposition,PosDiff,CenterDiff,CellVel,PrGradVec
    real(RK),dimension(:),allocatable::buf_send,buf_recv
    real(RK)::CenterDistY,CenterDistZ,CenterDist,Radius,VoidFraction,MassL,InertiaL,DenAlpha,Vol
    integer::i,j,k,pid,idlocal,itype,idGhost,nsend,nrecv,nsend2,nrecv2,ierr,request(4)
    ...
    call MPI_SENDRECV(nsend, 1, int_type, DEM_decomp%ProcNgh(1), 9, &
                      nrecv, 1, int_type, DEM_decomp%ProcNgh(2), 9, MPI_COMM_WORLD,SRstatus,ierr)
    if(nrecv>0) then
      nrecv2=nrecv*GIBM_size_backward
      allocate(buf_recv(nrecv2))
      call MPI_IRECV(buf_recv,nrecv2,real_type,DEM_decomp%ProcNgh(2),10,MPI_COMM_WORLD,request(1),ierr)
    endif
    if(nsend>0) then
      nsend2=nsend*GIBM_size_backward
      allocate(buf_send(nsend2))
      call pack_IBM_Fpforce(buf_send,nsend)
      call MPI_SEND(buf_send,nsend2,real_type,DEM_decomp%ProcNgh(1),10,MPI_COMM_WORLD,ierr)
    endif
    if(nrecv>0) then
      call MPI_WAIT(request(1),SRstatus,ierr)
    ...
    call MPI_SENDRECV(nsend, 1, int_type, DEM_decomp%ProcNgh(2), 11, &
                      nrecv, 1, int_type, DEM_decomp%ProcNgh(1), 11, MPI_COMM_WORLD,SRstatus,ierr)
    if(nrecv>0) then
      nrecv2=nrecv*GIBM_size_backward
      allocate(buf_recv(nrecv2))
      call MPI_IRECV(buf_recv,nrecv2,real_type,DEM_decomp%ProcNgh(1),12,MPI_COMM_WORLD,request(2),ierr)
    endif
    if(nsend>0) then
      nsend2=nsend*GIBM_size_backward
      allocate(buf_send(nsend2))
      call pack_IBM_Fpforce(buf_send,nsend)
      call MPI_SEND(buf_send,nsend2,real_type,DEM_decomp%ProcNgh(2),12,MPI_COMM_WORLD,ierr)
    endif
    if(nrecv>0) then
      call MPI_WAIT(request(2),SRstatus,ierr)
      call unpack_IBM_Fpforce(buf_recv,nrecv)
    ...
    call MPI_SENDRECV(nsend, 1, int_type, DEM_decomp%ProcNgh(3), 13, &
                      nrecv, 1, int_type, DEM_decomp%ProcNgh(4), 13, MPI_COMM_WORLD,SRstatus,ierr)
    if(nrecv>0) then
      nrecv2=nrecv*GIBM_size_backward
      allocate(buf_recv(nrecv2))
      call MPI_IRECV(buf_recv,nrecv2,real_type,DEM_decomp%ProcNgh(4),14,MPI_COMM_WORLD,request(3),ierr)
    endif
    if(nsend>0) then
      nsend2=nsend*GIBM_size_backward
      allocate(buf_send(nsend2))
      call pack_IBM_Fpforce(buf_send,nsend)
      call MPI_SEND(buf_send,nsend2,real_type,DEM_decomp%ProcNgh(3),14,MPI_COMM_WORLD,ierr)
    endif
    if(nrecv>0) then
      call MPI_WAIT(request(3),SRstatus,ierr)
      call unpack_IBM_Fpforce(buf_recv,nrecv)
    ...
    call MPI_SENDRECV(nsend, 1, int_type, DEM_decomp%ProcNgh(4), 15, &
                      nrecv, 1, int_type, DEM_decomp%ProcNgh(3), 15, MPI_COMM_WORLD,SRstatus,ierr)
    if(nrecv>0) then
      nrecv2=nrecv*GIBM_size_backward
      allocate(buf_recv(nrecv2))
      call MPI_IRECV(buf_recv,nrecv2,real_type,DEM_decomp%ProcNgh(3),16,MPI_COMM_WORLD,request(4),ierr)
    endif
    if(nsend>0) then
      nsend2=nsend*GIBM_size_backward
      allocate(buf_send(nsend2))
      call pack_IBM_Fpforce(buf_send,nsend)
      call MPI_SEND(buf_send,nsend2,real_type,DEM_decomp%ProcNgh(4),16,MPI_COMM_WORLD,ierr)
    endif
    if(nrecv>0) then
      call MPI_WAIT(request(4),SRstatus,ierr)
      call unpack_IBM_Fpforce(buf_recv,nrecv)
    ...
    IF(pbc(2)) THEN
      DO i=1, nGhostIBM
        if(GhostP_Direction(i)/= THIS_PROC_YP .and. GhostP_Direction(i)/= THIS_PROC_YM) cycle
        idlocal= GhostP_id(i)
        GPrtcl_FpForce(idlocal) = GPrtcl_FpForce(idlocal) +GhostP_linVel(i)
        GPrtcl_FpTorque(idlocal)= GPrtcl_FpTorque(idlocal)+GhostP_rotVel(i)
        GPrtcl_FluidIntegrate(1,idlocal)= GPrtcl_FluidIntegrate(1,idlocal)+ GhostP_FluidIntegrate(1,i)
    ...
    PrGradVec= zero_r3
    if(IsAddFluidPressureGradient .and. IsUxConst) PrGradVec%x = FluidDensity *PrGradData(3) !PrGradData(2)
    do i=1, nlocalOld
      itype   = GPrtcl_pType(i)
      MassL   = PrtclIBMProp(itype)%MassIBL/pmAlpha
      InertiaL= PrtclIBMProp(itype)%InertiaIBL/pmAlpha
      Vol     = DEMProperty%Prtcl_PureProp(itype)%Volume
      GPrtcl_FpForce(i) = DenAlpha*(GPrtcl_FluidIntegrate(1,i)-GPrtcl_FluidIntOld(1,i)+GPrtcl_FpForce(i)) -MassL   *GPrtcl_linVel(1,i) +Vol*PrGradVec
      GPrtcl_FpTorque(i)= DenAlpha*(GPrtcl_FluidIntegrate(2,i)-GPrtcl_FluidIntOld(2,i)+GPrtcl_FpTorque(i))-InertiaL*GPrtcl_rotVel(1,i)
  ...
  subroutine pack_IBM_FpForce(buf_send,nsend); implicit none
    ...
    do i=1,nsend
      pid= sendlist(i)
      buf_send(m)= real(GhostP_id(pid),RK);        m=m+1
  ...
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine PComm_IBM_recover_imp(); implicit none

    ! locals
    real(RK)::px,pz
    real(RK),dimension(:),allocatable::buf_send,buf_recv
    integer:: i,ierr,nlocalNEW,nsend,nrecv,nsend2,nrecv2,request(4),nlocalp,itype,SRstatus(MPI_STATUS_SIZE)
    ...
    call MPI_SENDRECV(nsend, 1, int_type, DEM_decomp%ProcNgh(3), 17, &
                      nrecv, 1, int_type, DEM_decomp%ProcNgh(4), 17, MPI_COMM_WORLD,SRstatus,ierr)
    if(nrecv>0) then
      nrecv2=nrecv*GIBM_size_recover_imp
      allocate(buf_recv(nrecv2))
      call MPI_IRECV(buf_recv,nrecv2,real_type,DEM_decomp%ProcNgh(4),18,MPI_COMM_WORLD,request(1),ierr)
    endif
    if(nsend>0) then
      ...
      call MPI_SEND(buf_send,nsend2,real_type,DEM_decomp%ProcNgh(3),18,MPI_COMM_WORLD,ierr)
    endif
    if(nrecv>0) then
      call MPI_WAIT(request(1),SRstatus,ierr)
    ...
    call MPI_SENDRECV(nsend, 1, int_type, DEM_decomp%ProcNgh(4), 19, &
                      nrecv, 1, int_type, DEM_decomp%ProcNgh(3), 19, MPI_COMM_WORLD,SRstatus,ierr)
    if(nrecv>0) then
      nrecv2=nrecv*GIBM_size_recover_imp
      allocate(buf_recv(nrecv2))
      call MPI_IRECV(buf_recv,nrecv2,real_type,DEM_decomp%ProcNgh(3),20,MPI_COMM_WORLD,request(2),ierr)
    endif
    if(nsend>0) then
      nsend2=nsend*GIBM_size_recover_imp
      allocate(buf_send(nsend2))
      ...
      call MPI_SEND(buf_send,nsend2,real_type,DEM_decomp%ProcNgh(4),20,MPI_COMM_WORLD,ierr)
    endif
    if(nrecv>0) then
      call MPI_WAIT(request(2),SRstatus,ierr)
      nlocalp= nlocalNEW;  nlocalNEW= nlocalp+ nrecv
    ...
    call MPI_SENDRECV(nsend, 1, int_type, DEM_decomp%ProcNgh(1), 21, &
                      nrecv, 1, int_type, DEM_decomp%ProcNgh(2), 21, MPI_COMM_WORLD,SRstatus,ierr)
    if(nrecv>0) then
      nrecv2=nrecv*GIBM_size_recover_imp
      allocate(buf_recv(nrecv2))
      call MPI_IRECV(buf_recv,nrecv2,real_type,DEM_decomp%ProcNgh(2),22,MPI_COMM_WORLD,request(3),ierr)
    endif
    if(nsend>0) then
      nsend2=nsend*GIBM_size_recover_imp
      allocate(buf_send(nsend2))
      ...
      call MPI_SEND(buf_send,nsend2,real_type,DEM_decomp%ProcNgh(1),22,MPI_COMM_WORLD,ierr)
    endif
    if(nrecv>0) then
      call MPI_WAIT(request(3),SRstatus,ierr)
      nlocalp= nlocalNEW;  nlocalNEW= nlocalp+ nrecv
    ...
    call MPI_SENDRECV(nsend, 1, int_type, DEM_decomp%ProcNgh(2), 23, &
                      nrecv, 1, int_type, DEM_decomp%ProcNgh(1), 23, MPI_COMM_WORLD,SRstatus,ierr)
    if(nrecv>0) then
      nrecv2=nrecv*GIBM_size_recover_imp
      allocate(buf_recv(nrecv2))
      call MPI_IRECV(buf_recv,nrecv2,real_type,DEM_decomp%ProcNgh(1),24,MPI_COMM_WORLD,request(4),ierr)
    endif
    if(nsend>0) then
      nsend2=nsend*GIBM_size_recover_imp
      allocate(buf_send(nsend2))
      ...
      call MPI_SEND(buf_send,nsend2,real_type,DEM_decomp%ProcNgh(2),24,MPI_COMM_WORLD,ierr)
    endif
    if(nrecv>0) then
      call MPI_WAIT(request(4),SRstatus,ierr)
      nlocalp= nlocalNEW;  nlocalNEW= nlocalp+ nrecv
  ...
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine PComm_IBM_forward_imp2(); implicit none
    ...
    call MPI_SENDRECV(nsend, 1, int_type, DEM_decomp%ProcNgh(3), 25, &
                      nrecv, 1, int_type, DEM_decomp%ProcNgh(4), 25, MPI_COMM_WORLD,SRstatus,ierr)
    if(nrecv>0) then
      nrecv2=nrecv*GIBM_size_forward
      allocate(buf_recv(nrecv2))
      call MPI_IRECV(buf_recv,nrecv2,real_type,DEM_decomp%ProcNgh(4),26,MPI_COMM_WORLD,request(1),ierr)
    endif
    if(nsend>0) then
      nsend2=nsend*GIBM_size_forward
      allocate(buf_send(nsend2))
      call pack_IBM_forward_imp2(buf_send,nsendg,nsend,xp_dir)
      call MPI_SEND(buf_send,nsend2,real_type,DEM_decomp%ProcNgh(3),26,MPI_COMM_WORLD,ierr)
    endif
    if(nrecv>0) then
      call MPI_WAIT(request(1),SRstatus,ierr)
      ng=ng+nrecv
    ...
    call MPI_SENDRECV(nsend, 1, int_type, DEM_decomp%ProcNgh(4), 27, &
                      nrecv, 1, int_type, DEM_decomp%ProcNgh(3), 27, MPI_COMM_WORLD,SRstatus,ierr)
    if(nrecv>0) then
      nrecv2=nrecv*GIBM_size_forward
      allocate(buf_recv(nrecv2))
      call MPI_IRECV(buf_recv,nrecv2,real_type,DEM_decomp%ProcNgh(3),28,MPI_COMM_WORLD,request(2),ierr)
    endif
    if(nsend>0) then
      nsend2=nsend*GIBM_size_forward
      allocate(buf_send(nsend2))
      call pack_IBM_forward_imp2(buf_send,nsendg,nsend,xm_dir)
      call MPI_SEND(buf_send,nsend2,real_type,DEM_decomp%ProcNgh(4),28,MPI_COMM_WORLD,ierr)
    endif
    if(nrecv>0) then
      call MPI_WAIT(request(2),SRstatus,ierr)
      ng=ng+nrecv
    ...
    call MPI_SENDRECV(nsend, 1, int_type, DEM_decomp%ProcNgh(1), 29, &
                      nrecv, 1, int_type, DEM_decomp%ProcNgh(2), 29, MPI_COMM_WORLD,SRstatus,ierr)
    if(nrecv>0) then
      nrecv2=nrecv*GIBM_size_forward
      allocate(buf_recv(nrecv2))
      call MPI_IRECV(buf_recv,nrecv2,real_type,DEM_decomp%ProcNgh(2),30,MPI_COMM_WORLD,request(3),ierr)
    endif
    if(nsend>0) then
      nsend2=nsend*GIBM_size_forward
      allocate(buf_send(nsend2))
      call pack_IBM_forward_imp2(buf_send,nsendg,nsend,zp_dir)
      call MPI_SEND(buf_send,nsend2,real_type,DEM_decomp%ProcNgh(1),30,MPI_COMM_WORLD,ierr)
    endif
    if(nrecv>0) then
      call MPI_WAIT(request(3),SRstatus,ierr)
      ng=ng+nrecv
    ...
    call MPI_SENDRECV(nsend, 1, int_type, DEM_decomp%ProcNgh(2), 31, &
                      nrecv, 1, int_type, DEM_decomp%ProcNgh(1), 31, MPI_COMM_WORLD,SRstatus,ierr)
    if(nrecv>0) then
      nrecv2=nrecv*GIBM_size_forward
      allocate(buf_recv(nrecv2))
      call MPI_IRECV(buf_recv,nrecv2,real_type,DEM_decomp%ProcNgh(1),32,MPI_COMM_WORLD,request(4),ierr)
    endif
    if(nsend>0) then
      nsend2=nsend*GIBM_size_forward
      allocate(buf_send(nsend2))
      call pack_IBM_forward_imp2(buf_send,nsendg,nsend,zm_dir)
      call MPI_SEND(buf_send,nsend2,real_type,DEM_decomp%ProcNgh(2),32,MPI_COMM_WORLD,ierr)
    endif
    if(nrecv>0) then
      call MPI_WAIT(request(4),SRstatus,ierr)
      ng=ng+nrecv
  ...
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90 
  subroutine pack_IBM_recover_imp(buf_send,nsend,nlocalp,dir); implicit none
    real(RK),dimension(:),intent(out)::buf_send
    ...
    DO i=1,nsend
      ...
      buf_send(m)=GPrtcl_PosOld(id)%y+pdy; m=m+1 ! 02
      buf_send(m)=GPrtcl_PosOld(id)%z+pdz; m=m+1 ! 03
      buf_send(m)=real(PIBM_pType(id),RK); m=m+1 ! 04
  ...
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine unpack_IBM_recover_imp(buf_recv,nrecv,nlocalp,dir); implicit none
    real(RK),dimension(:),intent(in)::buf_recv
  ...
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine Reallocate_ghost_for_IBM(ng); implicit none
    integer,intent(in)::ng

    ! locals
    integer::sizep,ierrTemp,ierr
    integer,dimension(:),allocatable::IntVec
    
    ierr=0
    call reallocate_ghost_for_Cntct(ng)
    sizep= mGhostIBM; mGhostIBM= GPrtcl_list%mGhost_CS

    call move_alloc(GhostP_direction,IntVec)
    allocate(GhostP_direction(mGhostIBM),stat=ierrTemp);ierr=ierr+abs(ierrTemp)
    GhostP_direction(1:sizep)= IntVec
    deallocate(IntVec)
    deallocate(GhostP_FluidIntegrate)
    allocate(GhostP_FluidIntegrate(2,mGhostIBM),stat=ierrTemp);ierr=ierr+abs(ierrTemp)
  ...
  subroutine Reallocate_IbpVar(); implicit none
    ...
    ! locals
    integer:: sizep,sizen,ierrTemp,ierr=0
    integer,dimension(:),allocatable:: IntVec
  ...
  ! Reallocate_PIBMOld
  !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^80^^^^^^^^90
  subroutine Reallocate_PIBMOld(nlocalNEW); implicit none
    ...
    allocate(PIBM_pType(sizen),stat=ierrTemp);ierr=ierr+abs(ierrTemp)
    PIBM_pType(1:sizep)=IntVec
    deallocate(IntVec)  

    ! ======= real3 vercor part =======
    call move_alloc(PIBM_linVel,Real3Vec)
    allocate(PIBM_linVel(sizen),stat=ierrTemp);ierr=ierr+abs(ierrTemp)
    PIBM_linVel(1:sizep)=Real3Vec

    call move_alloc(PIBM_rotVel,Real3Vec)
    allocate(PIBM_rotVel(sizen),stat=ierrTemp);ierr=ierr+abs(ierrTemp)
    PIBM_rotVel(1:sizep)=Real3Vec
    deallocate(Real3Vec)

    ! ======= real4 vercor part =======  
    call move_alloc(GPrtcl_PosOld,Real4Vec)
    allocate(GPrtcl_PosOld(sizen),stat=ierrTemp);ierr=ierr+abs(ierrTemp)
...
#ifdef  ca_IBM_common_add_LS
#undef  ca_IBM_common_add_LS
#endif
