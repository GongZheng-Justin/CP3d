...
for k=1:nzhp
  Ratio=4.0;
  if(k==1 || k==nzhp);Ratio=1.0;end;
  jt=nyc+1-jForLCS(1); 
  SpecData=zeros(1,nych);
  for jc=1:nych
    jsc=nyc+1-jc;
    rTemp1=Ratio*SpectraZ(k,jc, iLCSR)^2/(SpectraZ(k,jForLCS(1),iSpeR)*SpectraZ(k,jc,iSpeR));     
    rTemp2=Ratio*SpectraZ(k,jsc,iLCSR)^2/(SpectraZ(k,jt,iSpeR)*SpectraZ(k,jsc,iSpeR));
...
for k=1:nzhp
  ...
  for jc=1:nych
    jsc=nyc+1-jc;
    rTemp1=Ratio*SpectraZ(k,jc, iLCSR2)^2/(SpectraZ(k,jForLCS(2),iSpeR)*SpectraZ(k,jc,iSpeR));     
    rTemp2=Ratio*SpectraZ(k,jsc,iLCSR2)^2/(SpectraZ(k,jt,iSpeR)*SpectraZ(k,jsc,iSpeR));
    SpecData(jc)=0.5*(rTemp1+rTemp2);
  end
  fprintf(fid,myformat,[2*pi/zlz*(k-1),SpecData]);
end
...
for k=1:nzhp
  SpecData=zeros(1,nych);
  waveNumber=2*pi/zlz*(k-1);
  for jc=1:nych
    jsc=nyc+1-jc;
    YMinusYr=YrDist(jc);
    rTemp1=-atan(SpectraZ(k,jc,iLCSI)/SpectraZ(k,jc,iLCSR));
    rTemp1=waveNumber*YMinusYr/rTemp1;
    rTemp1=atan(rTemp1)*180/pi;    
    rTemp2=-atan(SpectraZ(k,jsc,iLCSI)/SpectraZ(k,jsc,iLCSR));
