...
if [[ -n $1 ]]; then
  ...
  echo    "  Please type a EXE index(1, 2, 3, 4 5 or 6): "$id_exe >> $CompilingLog
  if [ "$id_exe" == 1 ]; then
    EXE="dem"
  elif [ "$id_exe" == 2 ]; then
    EXE="channel2nd"
