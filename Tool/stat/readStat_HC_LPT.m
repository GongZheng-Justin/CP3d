...
%   * LPT_statIn: The folder to store original data for LPT               %
%   * dir_statOut:The folder to dump the final statistic results          %
%   * yMesh_str:  Ymesh file name                                         %
%   * IsForceInCell: Is particle-to fluid force defined in cell-center?   %
%                                                                         %
...
%   * LumleyTri.txt: Lumley triangle datas (Xi and Eta)                   %
...
IsForceInCell=0; % 1=True; 0:False

dir_statIn = '../../CFD/Results/';
LPT_statIn = '../../LPT/Results/';
dir_statOut='../../StatOut/';
...
AccMag=utaufinal^3/xnu;
budMag=utaufinal^4/xnu;
utauSqr=utaufinal^2;

%% Profile first
dataPrf=zeros(nyc,30);
for k=1:nyc
  ...
  % fx, symmetry
  dataPrf(k,28)= dataE(k,36)/AccMag;
  
  % fy, antisymmetry
  if(IsForceInCell==0)
    dataPrf(k,29)= 0.5*(dataE(k,37)+dataE(k+1,37))/AccMag;
  else
    dataPrf(k,29)= dataE(k,37)/AccMag;
  end
  
  % fz
  dataPrf(k,30)= dataE(k,38)/AccMag;
  
end
myformat=[repmat('%24.15E',1,30),'\n'];
fid=fopen([dir_statOut,'Profile.txt'],'wt');
strout=' ybar yplus u v w p uRms vRms wRms u''v'' v''w'' u''w'' pRms u''p'' v''p'' w''p'' S(u) S(v) S(w) F(u) F(v) F(w) dudy dwdy wxRms wyRms wzRms fpx fpy fpz';
fprintf(fid,'%s\n',strout);
for k=1:nyc
...
for k=1:nyc
  ...
  RealT1= dataE(k,  39)-dataE(k,  1)*dataE(k,  36);
  uuBud(k,9)= 2.0*RealT1/budMag; % 2*fx*u  
end
myformat=[repmat('%24.15E',1,9),'\n'];
fid=fopen([dir_statOut,'uuBudget.txt'],'wt');
fprintf(fid,' ybar yplus P T D pi phi epsilon  ufx\n');
for k=1:nyc
...
for k=1:nyc
  ...
  % Fluid-particle interaction
  if(IsForceInCell==0)
    RealT1= dataE(k,  40)-dataE(k,  2)*dataE(k,  37);
    RealT2= dataE(k+1,40)-dataE(k+1,2)*dataE(k+1,37);
    vvBud(k,9)= (RealT1+RealT2)/budMag; % 2*fy*v
  else
    RealT1= dataE(k,  40)-dataE(k,  37)*(dataE(k,  2)+dataE(k+1,2))*0.5;
    vvBud(k,9)= 2.0*RealT1/budMag; % 2*fy*v  
  end  
end
myformat=[repmat('%24.15E',1,9),'\n'];
fid=fopen([dir_statOut,'vvBudget.txt'],'wt');
fprintf(fid,' ybar yplus P T D pi phi epsilon vfy\n');
for k=1:nyc
...
for k=1:nyc
  ...
  % Fluid-particle interaction
  RealT1= dataE(k,  41)-dataE(k,  3)*dataE(k,  38);
  wwBud(k,9)= 2.0*RealT1/budMag; % 2*fz*w 
end
myformat=[repmat('%24.15E',1,9),'\n'];
fid=fopen([dir_statOut,'wwBudget.txt'],'wt');
fprintf(fid,' ybar yplus P T D pi phi epsilon wfz\n');
for k=1:nyc
...
for k=1:nyc
  ...
  % Fluid-particle interaction
  if(IsForceInCell==0)
    RealT1= dataE(k,  42)-dataE(k,   1)*(dataE(k,  37)+dataE(k+1,37))*0.5...;
           -dataE(k,  36)*(dataE(k,  2)+ dataE(k+1, 2))*0.5;
    uvBud(k,9)= RealT1/budMag; % fy*u +fx*v 
 else
    RealT1= dataE(k,  42)-dataE(k,   1)* dataE(k,  37)...;
           -dataE(k,  36)*(dataE(k,  2)+ dataE(k+1, 2))*0.5;
    uvBud(k,9)= RealT1/budMag; % fy*u +fx*v  
 end
end
...
fprintf(fid,' ybar yplus P T D pi phi epsilon UfyVfx\n');
for k=1:nyc
...
%% k budget
kBud=zeros(nyc,9);
kBud(:,3:end)=0.5*(uuBud(:,3:end)+vvBud(:,3:end)+wwBud(:,3:end));
for k=1:nyc
...
fprintf(fid,' ybar yplus P T D pi phi epsilon VelFp\n');
for k=1:nyc
...
SpectraX=zeros(nxhp,nyc,nEnergySpec);
...
for k=1:nxhp
  fprintf(fid,myformat,[2*pi/xlx*(k-1),SpectraX(k,:,1)]);
end
fclose(fid);
...
for k=1:nxhp
  fprintf(fid,myformat,[2*pi/xlx*(k-1),SpectraX(k,2:end,2),0.0]);
end
fclose(fid);
...
fprintf(fid,myformat,[Retau,yplus_spec_center]);
for k=1:nxhp
  fprintf(fid,myformat,[2*pi/xlx*(k-1),SpectraX(k,:,3)]);
end
fclose(fid);
...
fprintf(fid,myformat,[Retau,yplus_spec_center]);
for k=1:nxhp
  fprintf(fid,myformat,[2*pi/xlx*(k-1),SpectraX(k,:,4)]);
end
fclose(fid);
...
fprintf(fid,myformat,[Retau,yplus_spec_center]);
for k=1:nxhp
  fprintf(fid,myformat,[2*pi/xlx*(k-1),SpectraX(k,:,5)]);
end
fclose(fid);
fid=fopen([dir_statOut,'specx_uv2.txt'],'wt');
fprintf(fid,myformat,[Retau,yplus_spec_center]);
for k=1:nxhp
  fprintf(fid,myformat,[2*pi/xlx*(k-1),SpectraX(k,:,6)]);
end
fclose(fid);
end
...
SpectraZ=zeros(nzhp,nyc,nEnergySpec);
...
fprintf(fid,myformat,[Retau,yplus_spec_center]);
for k=1:nzhp
  fprintf(fid,myformat,[2*pi/zlz*(k-1),SpectraZ(k,:,1)]);
end
fclose(fid);
...
fprintf(fid,myformat,[Retau,yplus_spec_node]);
for k=1:nzhp
  fprintf(fid,myformat,[2*pi/zlz*(k-1),SpectraZ(k,2:end,2),0.0]);
end
fclose(fid);
...
fprintf(fid,myformat,[Retau,yplus_spec_center]);
for k=1:nzhp
  fprintf(fid,myformat,[2*pi/zlz*(k-1),SpectraZ(k,:,3)]);
end
fclose(fid);
...
fprintf(fid,myformat,[Retau,yplus_spec_center]);
for k=1:nzhp
  fprintf(fid,myformat,[2*pi/zlz*(k-1),SpectraZ(k,:,4)]);
end
fclose(fid);
...
fprintf(fid,myformat,[Retau,yplus_spec_center]);
for k=1:nzhp
  fprintf(fid,myformat,[2*pi/zlz*(k-1),SpectraZ(k,:,5)]);
end
fclose(fid);
fid=fopen([dir_statOut,'specz_uv2.txt'],'wt');
fprintf(fid,myformat,[Retau,yplus_spec_center]);
for k=1:nzhp
  fprintf(fid,myformat,[2*pi/zlz*(k-1),SpectraZ(k,:,6)]);
end
fclose(fid);
end
