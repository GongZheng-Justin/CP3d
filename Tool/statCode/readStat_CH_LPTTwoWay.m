...
xlx=12.56637061435917;
zlz=4.188790204786391;
nxc=192;
nzc=128;
xnu=2.3310E-4;
iTimeSet=1250;%6000;
...
BodyForceX=1.760473764E-3;   % If IsUxConst=0, u_tau=sqrt(BodyForceX*height)

nEnergySpec1D=18;% Number of 1D energy spectra
jForLCS =[12 20];% Reference j-index for Linear coherent structure

dir_statIn = '../../CFD/Results1/';
dir_statOut='../../StatOut/LPTTwoWay1/';
yMesh_str=[dir_statIn,'yMeshForTowWayLee.txt'];
...
for k=1:nych
  ...
  % fx, symmetry
  dataPrf(k,28)= 0.5*(dataE(k,36)+dataE(ksc,36))/AccMag;
  
  % fy, antisymmetry
  dataPrf(k,29)= 0.25*(dataE(k,37)+dataE(k+1,37)-dataE(ksc,37)-dataE(ksp,37))/AccMag;
  
  % fz
  dataPrf(k,30)= 0.5*(dataE(k,38)+dataE(ksc,38))/AccMag;

  % fxRms
  dataPrf(k,31)=0.5*(sqrt(dataE(k,39)-dataE(k,36)^2)+sqrt(dataE(ksc,39)-dataE(ksc,36)^2));
  dataPrf(k,31)=dataPrf(k,31)/AccMag;
  
  % fyRms
  RealT1=sqrt(dataE(k,  40)-dataE(k,  37)^2);
  RealT2=sqrt(dataE(k+1,40)-dataE(k+1,37)^2);
  RealT3=sqrt(dataE(ksc,40)-dataE(ksc,37)^2);
  RealT4=sqrt(dataE(ksp,40)-dataE(ksp,37)^2);
  dataPrf(k,32)= 0.25*(RealT1+RealT2+RealT3+RealT4)/AccMag;

  % fzRms
  dataPrf(k,33)=0.5*(sqrt(dataE(k,41)-dataE(k,38)^2)+sqrt(dataE(ksc,41)-dataE(ksc,38)^2));
  dataPrf(k,33)=dataPrf(k,33)/AccMag;
end
myformat=[repmat('%24.15E',1,33),'\n'];
fid=fopen([dir_statOut,'Profile.txt'],'wt');
strout=' ybar yplus u v w p uRms vRms wRms u''v'' v''w'' u''w'' pRms u''p'' v''p'' w''p'' S(u) S(v) S(w) F(u) F(v) F(w) dudy dwdy wxRms wyRms wzRms fx fy fz fxRms fyRms fzRms';
fprintf(fid,'%s\n',strout);
for k=1:nych
...
for k=1:nych
  ...
  RealT1= dataE(k,  42)-dataE(k,  1)*dataE(k,  36);
  RealT2= dataE(ksc,42)-dataE(ksc,1)*dataE(ksc,36);
  uuBud(k,9)= (RealT1+RealT2)/budMag; % 2*fx*u
end
...
for k=1:nych
  ...
  % Fluid-particle interaction
  RealT1= dataE(k,  43)-dataE(k,  2)*dataE(k,  37);
  RealT2= dataE(k+1,43)-dataE(k+1,2)*dataE(k+1,37);
  RealT3= dataE(ksc,43)-dataE(ksc,2)*dataE(ksc,37);
  RealT4= dataE(ksp,43)-dataE(ksp,2)*dataE(ksp,37);
  vvBud(k,9)= 0.5*(RealT1+RealT2+RealT3+RealT4)/budMag; % 2*fy*v
end
myformat=[repmat('%24.15E',1,9),'\n'];
...
for k=1:nych
  ...
  % Fluid-particle interaction
  RealT1= dataE(k,  44)-dataE(k,  3)*dataE(k,  38);
  RealT2= dataE(ksc,44)-dataE(ksc,3)*dataE(ksc,38);
  wwBud(k,9)= (RealT1+RealT2)/budMag; % 2*fz*w
end
myformat=[repmat('%24.15E',1,9),'\n'];
...
for k=1:nych
  ...
  % Fluid-particle interaction
  RealT1= dataE(k,  45)-dataE(k,   1)*(dataE(k,  37)+dataE(k+1,37))*0.5...
         -dataE(k,  36)*(dataE(k,  2)+ dataE(k+1, 2))*0.5;
  RealT2= dataE(ksc,45)-dataE(ksc, 1)*(dataE(ksc,37)+dataE(ksp,37))*0.5...
         -dataE(ksc,36)*(dataE(ksc,2)+ dataE(ksp, 2))*0.5;
  uvBud(k,9)= 0.5*(RealT1-RealT2)/budMag; % fy*u +fx*v
end
myformat=[repmat('%24.15E',1,9),'\n'];
fid=fopen([dir_statOut,'uvBudget.txt'],'wt');
fprintf(fid,' ybar yplus P T D pi phi epsilon Ufy+Vfx\n');
for k=1:nych
