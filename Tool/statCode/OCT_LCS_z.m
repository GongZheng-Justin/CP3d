...
for k=1:nzhp
  ...
  for jc=1:nyc
    rTemp=Ratio*SpectraZ(k,jc, iLCSR)^2/(SpectraZ(k,jForLCS(1),iSpeR)*SpectraZ(k,jc,iSpeR));     
    SpecData(jc)=rTemp;
  end
...
for k=1:nzhp
  ...
  SpecData=zeros(1,nyc);
  for jc=1:nyc
    rTemp=Ratio*SpectraZ(k,jc, iLCSR2)^2/(SpectraZ(k,jForLCS(2),iSpeR)*SpectraZ(k,jc,iSpeR));     
    SpecData(jc)=rTemp;
  end
  fprintf(fid,myformat,[2*pi/zlz*(k-1),SpecData]);
end
...
for k=1:nzhp
  ...
  for jc=1:nyc
    YMinusYr=YrDist(jc);
    rTemp=-atan(SpectraZ(k,jc,iLCSI)/SpectraZ(k,jc,iLCSR));
